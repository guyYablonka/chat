	...
	channelsList(filter, channelType, limit, sort) {
			...
			if (hasPermission(userId, 'view-c-room')) {
				if (filter) {
					channels = channels.concat(
						Rooms.findByTypeAndNameContaining(
							'c',
							filter,
							options
						).fetch()
					...
					channels = channels.concat(
						Rooms.findByType('c', options).fetch()
					);
				}
				...
				if (filter) {
					channels = channels.concat(
						Rooms.findByTypeInIdsAndNameContaining(
							'c',
							roomIds,
							filter,
							options
					...
					channels = channels.concat(
						Rooms.findByTypeInIds('c', roomIds, options).fetch()
					);
				}
			}
		}
		...
		if (channelType !== 'public' && hasPermission(userId, 'view-p-room')) {
			...
			const groupByType =
				userPref !== undefined
					? userPref
					: settings.get('UI_Group_Channels_By_Type');
			...
			if (!groupByType) {
				...
				if (filter) {
					...
					channels = channels.concat(
						Rooms.findByTypeInIds('p', roomIds, options).fetch()
					);
				}
			}
		}
