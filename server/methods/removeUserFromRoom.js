...
import { getUserDestinations } from '../../app/lib/server/functions/getUserDestinations';
import { extractRoomMemberRemoval } from '../../app/lib/server/functions/extractions/extractRoomMemberRemoval';
...
Meteor.methods({
	removeUserFromRoom(data) {
		...
		if (
			...
			!roomTypes
				.getConfig(room.t)
				.allowMemberAction(room, RoomMemberActions.REMOVE_USER)
		) {
			throw new Meteor.Error('error-not-allowed', 'Not allowed', {
				method: 'removeUserFromRoom'
			});
		...
		const userDestinations = getUserDestinations(removedUser.username);
		...
		callbacks.run(
			...
			{ removedUser, userWhoRemoved: fromUser },
			room
		);

		data.extraData = data.extraData || {};
		data.extraData.room = room;
		data.extraData.removedUser = removedUser;
		...
		extractRoomMemberRemoval(room, removedUser, fromUser, data.extraData);
