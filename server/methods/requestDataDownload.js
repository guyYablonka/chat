...
Meteor.methods({
	requestDataDownload({ fullExport = false }) {
		...
		const lastOperation = ExportOperations.findLastOperationByUser(
			userId,
			fullExport
		);
		const requestDay = lastOperation ? lastOperation.createdAt : new Date();
		const pendingOperationsBeforeMyRequestCount = ExportOperations.findAllPendingBeforeMyRequest(
			requestDay
		).count();
			...
			if (lastOperation.createdAt > yesterday) {
				if (lastOperation.status === 'completed') {
					const file = lastOperation.fileId
						? UserDataFiles.findOneById(lastOperation.fileId)
						: UserDataFiles.findLastFileByUser(userId);
					if (file) {
						return {
							requested: false,
