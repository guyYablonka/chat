...
Meteor.methods({
	messageSearch(text, rid, limit) {
		...
		if (Object.keys(query).length > 0) {
			...
			const usernames = result.message.docs
				.filter(msg => msg.u?.username)
				.map(msg => msg.u.username);
			...
			result.message.docs.forEach(message => {
				if (!message.u) {
					return;
				}
				message.u.name =
					names.get(message.u.username) || message.u.username;
			});
		}
