...
Meteor.methods({
	saveUserProfile(settings, customFields) {
		...
		if (settings.email) {
			if (
				!compareUserPassword(user, { sha256: settings.typedPassword })
			) {
				throw new Meteor.Error(
		...
		if (canChangePasswordForOAuth || user.services?.password) {
			...
			if (
				...
				if (
					!compareUserPassword(user, {
						sha256: settings.typedPassword
					})
				) {
					throw new Meteor.Error(
						'error-invalid-password',
				...
				if (
					compareUserPassword(user, { plain: settings.newPassword })
				) {
					throw new Meteor.Error(
