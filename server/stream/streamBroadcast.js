...
function _authorizeConnection(instance) {
	...
	return connections[instance].call(
		...
		function (err, ok) {
			if (err != null) {
				return logger.auth.error(
					`broadcastAuth error ${instance} ${InstanceStatus.id()} ${
						connections[instance].instanceId
					}`,
					{ err }
				);
			...
			return logger.auth.info(`broadcastAuth with ${instance}`, {
				ok
			});
		}
	);
...
function startMatrixBroadcast() {
	...
	matrixBroadCastActions = {
		added(record) {
			...
			if (
				...
				logger.auth.info('prevent self connect', {
					instance
				});
				return;
			}
			...
			if (connections[instance] && connections[instance].instanceRecord) {
				if (
					connections[instance].instanceRecord._createdAt <
					record._createdAt
				) {
			...
			logger.connection.debug('all instance connections: ', {
				instanceInfo: connections[instance].instanceRecord
			});
		},
	...
	InstanceStatusModel.find(query, options)
		.fetch()
...
function startStreamCastBroadcast(value) {
	...
	logger.connection.debug('connecting in', { instance, value });
					...
					function (error, response) {
						...
						switch (response) {
							case 'self-not-authorized':
								...
								logger.stream.debug(
									`    -> connection authorized ${connection.broadcastAuth}`
								);
								logger.stream.debug(
									`    -> connection status: ${connection.status()}`
								);
								return logger.stream.debug(
									'    -> arguments'.red,
									{
										eventName,
										args
							...
							case 'stream-not-exists':
								...
								return logger.stream.debug('    -> arguments', {
									eventName,
									args
								});
						}
