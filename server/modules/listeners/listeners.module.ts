...
export class ListenersModule {
    constructor(service: IServiceClass, notifications: NotificationsModule) {
        ...
        service.onEvent('watch.messages', ({ message }) => {
            ...
            notifications.streamRoomMessage.emitWithoutBroadcast(
                message.rid,
                message
            );
        });
        ...
        service.onEvent(
            ...
            ({ clientAction, subscription }) => {
                ...
                subscription.t =
                    subscription.t ??
                    Rooms.findOneById(subscription.rid, {
                        fields: {
                            t: 1
                        }
                    })?.t;
                ...
                if (clientAction === 'removed') {
                    notifications.streamRoomMessage.__emit(
                        subscription.u._id,
                        clientAction,
                        subscription
                    );
                ...
                notifications.streamUser.__emit(
                    subscription.u._id,
                    clientAction,
                    subscription
                );
        ...
        service.onEvent(
            'watch.inquiries',
            async ({ clientAction, inquiry, diff }): Promise<void> => {
        ...
        service.onEvent(
            ...
            async ({ clientAction, setting }): Promise<void> => {
                ...
                if (clientAction !== 'removed') {
                    const result = await EnterpriseSettings.changeSettingValue(
                        setting
                    );
                    if (
                        result &&
        ...
        service.onEvent(
            'watch.users',
            ({ clientAction, data, diff, unset, id }): void => {
                switch (clientAction) {
                    case 'updated':
        ...
        service.onEvent(
            'watch.integrationHistory',
            ({ clientAction, data, diff, id }): void => {
                if (!data?.integration?._id) {
                    return;
                }
                switch (clientAction) {
                    case 'updated': {
                        notifications.streamIntegrationHistory.emitWithoutBroadcast(
                            data.integration._id,
                            { id, diff, type: clientAction }
                        );
                        break;
                    }
                    case 'inserted': {
        ...
        service.onEvent(
            'watch.livechatDepartmentAgents',
            ({ clientAction, data }): void => {
                const { agentId } = data;
                if (!agentId) {
                    return;
                }
