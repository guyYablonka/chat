...
export class NotificationsModule {
    ...
    constructor(private Streamer: IStreamerConstructor) {
        settings.get('Enable_Stream_Notification_Logs', (_key, value) => {
            this.debug = !!value;
        });
        ...
        this.streamLivechatQueueData = new this.Streamer(
    ...
    async configure({
        ...
        this.streamRoomMessage.allowEmit(
            '__my_messages__',
            async function (_eventName, { rid }) {
                if (!this.userId) {
                    return false;
                }
                ...
                try {
                    ...
                    const shouldAccess = await Authorization.shouldAccessRoom(
                        room,
                        {
                            _id: this.userId
                        }
                    ...
                    const roomParticipant =
                        await Subscriptions.countByRoomIdAndUserId(
                            room._id,
                            this.userId
                        );
        ...
        this.streamRoom.allowRead(async function (eventName, extraData) {
            ...
            if (extraData?.token) {
                ...
                return (
                    room && room.t === 'l' && room.v.token === extraData.token
                );
            }
            ...
            const subsCount = await Subscriptions.countByRoomIdAndUserId(
                rid,
                this.userId
            );
            return subsCount > 0;
        ...
        this.streamRoomUsers.allowWrite(async function (eventName, ...args) {
            ...
            if (
                ...
                const subscriptions: ISubscription[] =
                    await Subscriptions.findByRoomIdAndNotUserId(
                        roomId,
                        this.userId,
                        { projection: { 'u._id': 1, _id: 0 } }
                    ).toArray();
                subscriptions.forEach(subscription =>
                    notifyUser(subscription.u._id, e, ...args)
                );
            }
            return false;
        });
        ...
        this.streamLivechatQueueData.allowRead(async function () {
            return this.userId
                ? Authorization.hasPermission(this.userId, 'view-l-room')
                : false;
        });
            ...
            async (
                ...
                if (/rooms-changed/.test(eventName)) {
                    ...
                    const roomEvent = (...args: any[]): void => {
                        ...
                        const payload = streamer.changedPayload(
                            streamer.subscriptionName,
                            'id',
                            {
                    ...
                    const subscriptions: Pick<ISubscription, 'rid'>[] =
    ...
    notifyRoomInThisInstance(
        ...
        return this.streamRoom.emitWithoutBroadcast(
            `${room}/${eventName}`,
            ...args
        );
    }

    notifyUserInThisInstance(
        userId: string,
        eventName: string,
        ...args: any[]
    ): void {
        ...
        return this.streamUser.emitWithoutBroadcast(
            `${userId}/${eventName}`,
            ...args
        );
    }
