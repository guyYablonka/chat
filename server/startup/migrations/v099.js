...
function batch(arr, limit, fn) {
	...
	return Promise.all(arr.splice(0, limit).map(item => fn(item))).then(() =>
		batch(arr, limit, fn)
	);
}
...
Migrations.add({
	...
	up() {
				...
				Uploads.model.direct.update(
					{ _id: record._id },
					{
						$set: {
							...
							GoogleStorage: {
								path:
									record.googleCloudStorage.path + record._id
							}
						},
						$unset: {
		...
		const avatarsPathRecord = Settings.findOne({
			_id: 'Accounts_AvatarStorePath'
		});
		const avatarStoreTypeRecord = Settings.findOne({
			_id: 'Accounts_AvatarStoreType'
		});

		const avatarsPath = avatarsPathRecord
			? avatarsPathRecord.value
			: process.env.AVATARS_PATH;
		let avatarStoreType =
			avatarStoreTypeRecord && avatarStoreTypeRecord.value;
		...
		if (avatarStoreType == null) {
			...
			if (Match.test(count, Number) && count > 0) {
				...
				Match.test(avatarsPath, String) &&
				avatarsPath.length > 0
			) {
		...
		Meteor.startup(function () {
			Meteor.setTimeout(function () {
				...
				const users = Users.find(
					{ avatarOrigin: { $in: avatarOrigins } },
					{ avatarOrigin: 1, username: 1 }
				).fetch();
				...
				batch(users, 300, user => {
					...
					const gridFSAvatar =
						oldAvatarGridFS.getFileWithReadStream(id);
					...
					.then(() => {
						const avatarsFiles = new Mongo.Collection(
							'avatars.files'
						);
						const avatarsChunks = new Mongo.Collection(
							'avatars.chunks'
						);
						try {
							...
							console.warn(
								'Migration Error: avatars.files and avatars.chunks collections may not exist!'
							);
						}
