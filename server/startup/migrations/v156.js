...
const federationEventsCollection = new Mongo.Collection(
	'rocketchat_federation_events'
);
...
Migrations.add({
	...
	up() {
		...
		Settings.remove({
			$and: [{ _id: /FEDERATION/ }, { _id: { $nin: validSettings } }]
		});
		...
		Promise.await(
			Rooms.model.rawCollection().updateMany(
				{
		...
		Promise.await(
			Subscriptions.model.rawCollection().updateMany(
				{
					federation: { $exists: true }
				},
				{
		...
		Promise.await(
			Users.model.rawCollection().updateMany(
				{
					federation: { $exists: true }
				},
				{
		...
		Promise.await(
			Messages.model.rawCollection().updateMany(
				{
					federation: { $exists: true }
				},
				{
		...
		Promise.await(
			Uploads.model.rawCollection().updateMany(
				{
					federation: { $exists: true }
				},
				{
