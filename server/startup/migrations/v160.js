...
Migrations.add({
	version: 160,
	up() {
		const emailFieldSetting = Settings.findOne({
			_id: 'SAML_Custom_Default_email_field'
		});
		const usernameFieldSetting = Settings.findOne({
			_id: 'SAML_Custom_Default_username_field'
		});
		...
		const emailField =
			(emailFieldSetting && emailFieldSetting.value) || 'email';
		const usernameField =
			(usernameFieldSetting && usernameFieldSetting.value) || 'username';
