...
Migrations.add({
	...
	up() {
		...
		const LDAP_Domain_Search_User_ID = Settings.findOne({
		...
		const LDAP_Use_Custom_Domain_Search = Settings.findOne({
			_id: 'LDAP_Use_Custom_Domain_Search'
		});
		const LDAP_Custom_Domain_Search = Settings.findOne({
			_id: 'LDAP_Custom_Domain_Search'
		});

		const LDAP_Domain_Search_User = Settings.findOne({
			_id: 'LDAP_Domain_Search_User'
		});
		const LDAP_Domain_Search_Password = Settings.findOne({
			_id: 'LDAP_Domain_Search_Password'
		});
		const LDAP_Domain_Search_Filter = Settings.findOne({
			_id: 'LDAP_Domain_Search_Filter'
		});

		const LDAP_Domain_Search_Object_Class = Settings.findOne({
			_id: 'LDAP_Domain_Search_Object_Class'
		});
		const LDAP_Domain_Search_Object_Category = Settings.findOne({
		...
		if (LDAP_Use_Custom_Domain_Search) {
			if (LDAP_Use_Custom_Domain_Search.value === true) {
				...
				try {
					Custom_Domain_Search = JSON.parse(
						LDAP_Custom_Domain_Search.value
					);
				} catch (error) {
					throw new Error('Invalid Custom Domain Search JSON');
				}

				LDAP_Domain_Search_User.value =
					Custom_Domain_Search.userDN || '';
				LDAP_Domain_Search_Password.value =
					Custom_Domain_Search.password || '';
				...
				if (LDAP_Domain_Search_Object_Category.value !== '') {
					filter.push(
						`(objectCategory=${LDAP_Domain_Search_Object_Category.value})`
					);
				}

				if (LDAP_Domain_Search_Object_Class.value !== '') {
					filter.push(
						`(objectclass=${LDAP_Domain_Search_Object_Class.value})`
					);
				}
		...
		const LDAP_Sync_User_Data = Settings.findOne({
		...
		const LDAP_Import_Users = Settings.findOne({
