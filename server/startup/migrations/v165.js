...
Migrations.add({
	version: 165,
	up() {
		const manageIntegrationsPermission = Permissions.findOne(
			'manage-integrations'
		);
		const manageOwnIntegrationsPermission = Permissions.findOne(
		...
		if (manageIntegrationsPermission) {
			Permissions.upsert(
				...
				{ $set: { roles: manageIntegrationsPermission.roles } }
			);
			Permissions.upsert(
		...
		if (manageOwnIntegrationsPermission) {
			Permissions.upsert(
				...
				{ $set: { roles: manageOwnIntegrationsPermission.roles } }
			);
			Permissions.upsert(
