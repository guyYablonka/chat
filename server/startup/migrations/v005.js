...
Migrations.add({
	...
	up() {
		...
		Users.find({
			...
			console.log(
				`Adding: username ${newUserName} to all user ${user._id}`
			);
		...
		Rooms.findByType('d').forEach(function (room) {
			let newId = '';
			const id0 = Users.findOneByUsernameIgnoringCase(room.usernames[0])
				._id;
			const id1 = Users.findOneByUsernameIgnoringCase(room.usernames[1])
				._id;
			const ids = [id0, id1];
