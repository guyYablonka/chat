...
Migrations.add({
	version: 109,
	up() {
		const FileUpload_GoogleStorage_Proxy =
			(Settings.findOne({ _id: 'FileUpload_GoogleStorage_Proxy' }) || {})
				.value === true;
		const FileUpload_S3_Proxy =
			(Settings.findOne({ _id: 'FileUpload_S3_Proxy' }) || {}).value ===
			true;

		Settings.update(
			{ _id: 'FileUpload_GoogleStorage_Proxy_Avatars' },
			{ $set: { value: FileUpload_GoogleStorage_Proxy } }
		);
		Settings.update(
