...
const exitIfNotBypassed = (ignore, errorCode = 1) => {
	if (
		...
		['yes', 'true'].includes(ignore.toLowerCase())
	) {
		return;
	}
...
Meteor.startup(function () {
	...
	const desiredNodeVersion = semver.clean(
		fs
			.readFileSync(path.join(process.cwd(), '../../.node_version.txt'))
			.toString()
	);
	const desiredNodeVersionMajor = String(
		semver.parse(desiredNodeVersion).major
	);
	...
	return Meteor.setTimeout(function () {
		...
		if (!process.env.DISABLE_DB_WATCH && !oplogEnabled) {
			...
			SystemLogger.error(msg, 'SERVER ERROR');
		...
		SystemLogger.startup(msg, 'SERVER RUNNING');

		// Deprecation
		if (!semver.satisfies(semver.coerce(mongoVersion), '>=3.6.0')) {
			msg = [
				`YOUR CURRENT MONGODB VERSION (${mongoVersion}) IS DEPRECATED.`,
				'IT WILL NOT BE SUPPORTED ON ROCKET.CHAT VERSION 4.0.0 AND GREATER,',
				'PLEASE UPGRADE MONGODB TO VERSION 3.6 OR GREATER'
			].join('\n');
			SystemLogger.warn(msg, 'DEPRECATION');

			const id = `mongodbDeprecation_${mongoVersion.replace(
				/[^0-9]/g,
				'_'
			)}`;
			const title = 'MongoDB_Deprecated';
			const text =
				'MongoDB_version_s_is_deprecated_please_upgrade_your_installation';
