const getMongoVersion = async function ({ version, git }) {
	try {
		const workflows = await git.show([
			`${version}:.github/workflows/build_and_test.yml`
		]);
...
const getNodeNpmVersions = async function ({ version, git, request }) {
	try {
		...
		return {
			node_version: requestResult.match(
				/NODE_VERSION=((?:\d+\.){2}\d)/m
			)[1],
...
const getAppsEngineVersion = async function ({ version, git }) {
	try {
		...
		const { version: appsEngineVersion } = dependencies[
...
module.exports = async function ({ version, git, request }) {
	...
	const { node_version, npm_version } = await getNodeNpmVersions({
		version,
		git,
		request
	});
