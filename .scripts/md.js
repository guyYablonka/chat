...
const historyManualDataFile = path.join(
	__dirname,
...
function getTagDate(tag) {
	return execSync(`git tag -l --format="%(creatordate:short)" ${tag}`)
		.toString()
		.replace(/\n/, '');
}

function getLatestCommitDate() {
	return execSync("git log --date=short --format='%ad' -1")
		.toString()
		.replace(/\n/, '');
}
...
function getSummary(contributors, groupedPRs) {
	...
	Object.keys(groupedPRs).forEach(group => {
		if (groupedPRs[group].length) {
			summary.push(
				`${groupedPRs[group].length} ${SummaryNameEmoticons[group]}`
			);
		}
	});
	...
	if (contributors.length) {
		summary.push(
			`${contributors.length} ${SummaryNameEmoticons.contributor}`
		);
	}
...
function renderPRs(prs) {
	...
	Object.keys(groupedPRs).forEach(group => {
		...
		prs.forEach(pr => {
			let contributors = _.compact(
				_.difference(pr.contributors, nonContributors)
			)
				.sort()
				.map(
					contributor =>
			...
			const prInfo = pr.pr
				? ` ([#${pr.pr}](https://github.com/RocketChat/Rocket.Chat/pull/${pr.pr})${contributors})`
				: '';
	...
	const contributors = _.compact(
		_.difference(
			prs.reduce(
				(value, pr) => _.unique(value.concat(pr.contributors)),
				[]
			),
			nonContributors
		)
...
Object.keys(historyData)
	...
	.forEach(tag => {
		...
		const tagDate =
			tag === 'HEAD' ? getLatestCommitDate() : getTagDate(tag);
