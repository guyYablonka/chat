...
const commitRegexString =
	'(^Merge pull request #([0-9]+) from )|( \\(#([0-9]+)\\)$)';
...
function getPRInfo(number, commit) {
	...
	return promiseRetryRateLimit(() =>
		octokit.pullRequests.get({ owner, repo, number })
	)
		.catch(onError)
		.then(pr => {
			const info = {
		...
		return new Promise((resolve, reject) => {
			...
			function process() {
				...
				const promises = partItems.map(item =>
					getPRInfo(getPRNumeberFromMessage(item.message, item), item)
				);
