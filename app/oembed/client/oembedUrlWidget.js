...
const getDescription = function (self) {
	...
	const description =
		self.meta.ogDescription ||
		self.meta.twitterDescription ||
...
Template.oembedUrlWidget.helpers({
	...
	image() {
		...
		let url =
			...
			this.meta.msapplicationTileImage;
		if (url == null) {
			return;
		}
		url = new URL(url, `${this.parsedUrl.protocol}//${this.parsedUrl.host}`)
			.href;
		return url;
	},
	show() {
