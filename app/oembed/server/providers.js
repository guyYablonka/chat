...
providers.registerProvider({
	urls: [
		new RegExp('https?://www\\.youtube\\.com/\\S+'),
		new RegExp('https?://youtu\\.be/\\S+')
	],
...
providers.registerProvider({
	urls: [
		new RegExp('https?://www\\.rdio\\.com/\\S+'),
		new RegExp('https?://rd\\.io/\\S+')
	],
...
providers.registerProvider({
	...
	endPoint:
		'https://www.slideshare.net/api/oembed/2?format=json&maxheight=200'
});
...
providers.registerProvider({
	urls: [
		new RegExp(
			'https?://(play|open)\\.spotify\\.com/(track|album|playlist|show)/\\S+'
		)
	],
