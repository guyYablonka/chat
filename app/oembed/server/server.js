...
const getUrlContent = Meteor.wrapAsync(function (
	...
	const ignoredHosts =
		settings.get('API_EmbedIgnoredHosts').replace(/\s/g, '').split(',') ||
		[];
	if (
	...
	const safePorts =
		settings.get('API_EmbedSafePorts').replace(/\s/g, '').split(',') || [];
...
const insertMaxWidthInOembedHtml = oembedHtml =>
	oembedHtml?.replace('iframe', 'iframe style="max-width: 100%"');
