...
import { RoomTypeToLabel } from '../../lib/RoomTypeConfig';
import { hasAtLeastOnePermission } from '../../../authorization';
import { ChatRoom, ChatSubscription } from '../../../models';

export const roomTypes =
	new (class RocketChatRoomTypes extends RoomTypesCommon {
		...
		getIcon(roomData) {
			...
			return (
				(this.roomTypes[roomData.t].getIcon &&
					this.roomTypes[roomData.t].getIcon(roomData)) ||
				this.roomTypes[roomData.t].icon
			);
		}

		getRoomName(roomType, roomData) {
			return this.roomTypes[roomType]?.roomName?.(roomData);
		}

		getSecondaryRoomName(roomType, roomData) {
			return (
				this.roomTypes[roomType] &&
				typeof this.roomTypes[roomType].secondaryRoomName ===
					'function' &&
		...
		getIdentifiers(e) {
			...
			const list = _.reject(
				this.roomTypesOrder,
				t => except.indexOf(t.identifier) !== -1
			);
		...
		getRoomTypeLabel(roomId) {
			const type = this.getRoomType(roomId);

			return RoomTypeToLabel[type];
		}

		getUserStatusText(roomType, rid) {
			return (
				this.roomTypes[roomType] &&
				typeof this.roomTypes[roomType].getUserStatusText ===
					'function' &&
		...
		findRoom(roomType, identifier, user) {
			return (
				this.roomTypes[roomType] &&
				this.roomTypes[roomType].findRoom(identifier, user)
			);
		}
		...
		readOnly(rid, user) {
			...
			if (
				roomType &&
				this.roomTypes[roomType] &&
				this.roomTypes[roomType].readOnly
			) {
			...
			if (room) {
				if (
					...
					room.muted.indexOf(user.username) !== -1
				) {
					return true;
				}
		...
		verifyCanSendMessage(rid) {
			...
			if (
				this.roomTypes[roomType] &&
				this.roomTypes[roomType].canSendMessage
			) {
		...
		verifyShowJoinLink(rid) {
			...
			if (
				this.roomTypes[roomType] &&
				!this.roomTypes[roomType].showJoinLink
			) {
				return false;
			}
		...
		getNotSubscribedTpl(rid) {
			...
			if (
				this.roomTypes[roomType] &&
				!this.roomTypes[roomType].notSubscribedTpl
			) {
				return false;
			}
		...
		getReadOnlyTpl(rid) {
			...
			const roomType = room.t;
			return (
				this.roomTypes[roomType] && this.roomTypes[roomType].readOnlyTpl
			);
		}
		...
		openRouteLink(roomType, subData, queryParams) {
			...
			return FlowRouter.go(
				this.roomTypes[roomType].route.name,
				routeData,
				queryParams
			);
		}
