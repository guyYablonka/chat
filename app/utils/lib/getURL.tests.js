...
jest.mock('../../settings', () => ({
	settings: { get: () => 'https://go.rocket.chat' }
}));
...
const testPaths = (o, _processPath) => {
	...
	expect(_getURL('', o)).toEqual(processPath(''));
	expect(_getURL('/', o)).toEqual(processPath(''));
	expect(_getURL('//', o)).toEqual(processPath(''));
	expect(_getURL('///', o)).toEqual(processPath(''));
	expect(_getURL('/channel', o)).toEqual(processPath('/channel'));
	expect(_getURL('/channel/', o)).toEqual(processPath('/channel'));
	expect(_getURL('/channel//', o)).toEqual(processPath('/channel'));
	expect(_getURL('/channel/123', o)).toEqual(processPath('/channel/123'));
	expect(_getURL('/channel/123/', o)).toEqual(processPath('/channel/123'));
	expect(_getURL('/channel/123?id=456&name=test', o)).toEqual(
