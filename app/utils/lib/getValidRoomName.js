...
export const getValidRoomName = (displayName, rid = '', options = {}) => {
			...
			if (room && room._id !== rid) {
				if (room.archived) {
					throw new Meteor.Error(
						...
						`There's an archived channel with name ${displayName}`,
						{
					...
					throw new Meteor.Error(
						...
						`A channel with name '${displayName}' exists`,
						{
							function: 'RocketChat.getValidRoomName',
							channel_name: displayName
						}
	...
	if (options.nameValidationRegex) {
		...
		try {
			...
			nameValidation = new RegExp('^[\\-.\'"\\w א-ת]{1,40}$');
		}
	}
	...
	if (!nameValidation.test(slugifiedName)) {
		throw new Meteor.Error(
			...
			`${slugifiedName} is not a valid room name.`,
			{
				function: 'RocketChat.getValidRoomName',
	...
	if (options.allowDuplicates !== true) {
		...
		if (room && room._id !== rid) {
			if (settings.get('UI_Allow_room_names_with_special_chars')) {
				...
				throw new Meteor.Error(
					'error-archived-duplicate-name',
					`There's an archived channel with name ${slugifiedName}`,
					{
						function: 'RocketChat.getValidRoomName',
						channel_name: slugifiedName
					}
				...
				throw new Meteor.Error(
					'error-duplicate-channel-name',
					`A channel with name '${slugifiedName}' exists`,
					{
						function: 'RocketChat.getValidRoomName',
						channel_name: slugifiedName
					}
