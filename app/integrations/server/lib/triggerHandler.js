...
integrations.triggerHandler = new (class RocketChatIntegrationHandler {
	...
	updateHistory({
		...
		if (historyId) {
			Models.IntegrationHistory.update(
				{ _id: historyId },
				{ $set: history }
			);
			return historyId;
	...
	getIntegrationScript(integration) {
		...
		try {
			logger.outgoing.info(
				'Will evaluate script of Trigger',
				integration.name
			);
	...
	executeScript(integration, method, params, historyId) {
		...
		try {
			...
			logger.outgoing.debug(
				`Script method "${method}" result of the Integration "${integration.name}" is:`
			);
			...
			logger.outgoing.debug(
				integration.scriptCompiled.replace(/^/gm, '  ')
