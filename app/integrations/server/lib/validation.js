...
function _verifyRequiredFields(integration) {
	...
	if (
		integrations.outgoingEvents[integration.event].use.targetRoom &&
		!integration.targetRoom
	) {
		throw new Meteor.Error(
		...
		if (scopedChannels.includes(channel)) {
			if (channel === 'all_public_channels') {
				...
				throw new Meteor.Error(
					...
					{
						function:
							'validateOutgoing._verifyUserHasPermissionForChannels'
					}
				);
			}
		} else {
			...
			if (
				...
				!Subscriptions.findOneByRoomIdAndUserId(record._id, userId, {
					fields: { _id: 1 }
				})
			) {
				throw new Meteor.Error(
...
integrations.validateOutgoing = function _validateOutgoing(
	integration,
	userId
) {
	if (
	...
	if (integrations.outgoingEvents[integration.event].use.channel) {
		if (!Match.test(integration.channel, String)) {
			...
			channels = _.map(integration.channel.split(','), channel =>
				s.trim(channel)
			);
	...
	if (
		integrations.outgoingEvents[integration.event].use.triggerWords &&
		integration.triggerWords
	) {
		...
		integration.triggerWords = _.without(integration.triggerWords, [
			undefined
		]);
	} else {
	...
	if (
		...
		try {
			const babelOptions = Object.assign(
				Babel.getDefaultOptions({ runtime: false }),
