...
function getIntegrationScript(integration) {
	...
	try {
		logger.incoming.info(
			`Will evaluate script of Trigger ${integration.name}`
		);
	...
	Meteor.runAsUser(user._id, function () {
		switch (options.event) {
			case 'newMessageOnChannel':
				...
				if (
					options.data.channel_name != null &&
					options.data.channel_name.indexOf('#') === -1
				) {
...
function executeIntegrationRest() {
	...
	if (
		...
		try {
			const { sandbox } = buildSandbox(
				compiledScripts[this.integration._id].store
			);
			...
			if (!result) {
				logger.incoming.debug(
					'[Process Incoming Request result of Trigger',
			...
			logger.incoming.error(
				'[Error running Script in Trigger',
				this.integration.name,
				':]'
			);
			logger.incoming.error(
				this.integration.scriptCompiled.replace(/^/gm, '  ')
			);
			logger.incoming.error('[Stack:]');
			logger.incoming.error(stack.replace(/^/gm, '  '));
	...
	if (
		!this.bodyParams ||
		(_.isEmpty(this.bodyParams) && !this.integration.scriptEnabled)
	) {
	...
	try {
		const message = processWebhookMessage(
			this.bodyParams,
			this.user,
			defaultValues,
			this.integration
...
class WebHookAPI extends APIClass {
	...
	*/
	enforceRateLimit(objectForRateLimitMatch, request, response, userId) {
		...
		super.enforceRateLimit(
			integrationForRateLimitMatch,
			request,
			response,
			userId
		);
		...
		user() {
			...
			if (!this.integration) {
				logger.incoming.info(
					...
					this.request.params.integrationId,
					'or token',
					this.request.params.token
				);
