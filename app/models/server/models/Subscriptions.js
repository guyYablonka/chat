...
export class Subscriptions extends Base {
	...
	findNotificationPreferencesByRoom(
		query /* { roomId: rid, desktopFilter: desktopNotifications, mobileFilter: mobilePushNotifications, emailFilter: emailNotifications }*/
	) {
	...
	setMuteByRoomIdAndUserId(roomId, userId, disableNotifications) {
	...
	setUserDirectDisplayNameByUsername(username, directDisplayName) {
		const query = {
			name: username,
			t: 'd'
		};
	...
	addRoleById(_id, role, extraData = {}) {
		const query = { _id };
