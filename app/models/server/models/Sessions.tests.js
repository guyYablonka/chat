...
describe.skip('Sessions Aggregates', () => {
	let db;

	if (!process.env.MONGO_URL) {
		beforeAll(function () {
			this.timeout(120000);
			return mongoUnit.start({ version: '3.2.22' }).then(testMongoUrl => {
	...
	it('should have sessions_dates data saved', () => {
		...
		return collection
			...
			.toArray()
			.then(docs =>
				expect(docs.length).toEqual(DATA.sessions_dates.length)
			);
	});
	...
	it('should have sessions data saved', () => {
		...
		return collection
			.find()
			.toArray()
			.then(docs => expect(docs.length).toEqual(DATA.sessions.length));
	});
