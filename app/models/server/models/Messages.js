...
export class Messages extends Base {
	...
	addAttachmentTranslations = function (
		...
		Object.keys(translations).forEach(key => {
			const translation = translations[key];
			updateObj[`attachments.${attachmentIndex}.translations.${key}`] =
				translation;
		});
		return this.update({ _id: messageId }, { $set: updateObj });
	};
	...
	findOneByInfo(rid, username, msg, date) {
		...
		const flooredDate = new Date(
			Math.floor(ts / MILLIS_IN_SECOND) * MILLIS_IN_SECOND
		);
		const ceiledDate = new Date(
			Math.ceil(ts / MILLIS_IN_SECOND) * MILLIS_IN_SECOND
		);
		...
		const query = {
			...
			msg,
			'u.username': username,
			$or: [exactDateQuery, timeRangeQuery]
		};
	...
	createUserJoinWithRoomIdAndUser(roomId, user, extraData) {
		const message = getUserName(user);
		return this.createWithTypeRoomIdMessageAndUser(
			'uj',
			roomId,
			message,
	...
	createChangeDescriptionWithRoomIdAndUser(
		roomId,
		user,
		roomDescription,
		extraData
	...
	createChangeAnnouncementWithRoomIdAndUser(
		roomId,
		user,
		roomAnnouncement,
		extraData
	...
	createChangeTopicWithRoomIdAndUser(
		roomId,
		user,
		roomTopic,
		extraData
	...
	findUnreadMessagesByRoomAndDate(rid, after, options = {}) {
		const query = {
			unread: true,
			rid
		};
	...
	findUnreadMessagesByRoom(rid, options = {}) {
		const query = {
			unread: true,
			rid
		};
