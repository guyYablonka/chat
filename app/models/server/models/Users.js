...
import { buildQueryOfFindByUsers } from '../functions/buildQueryOfFindByUsers';

const queryStatusAgentOnline = (extraFilters = {}) => {
	if (settings.get('Livechat_enabled_when_agent_idle') === false) {
		extraFilters = Object.assign(extraFilters, {
...
export class Users extends Base {
	constructor(...args) {
		...
		this.tryEnsureIndex({ lastWhatsNewVersion: 1 });
	...
	getNextAgent(ignoreAgentId) {
		...
		const findAndModify = Meteor.wrapAsync(
	...
	findOneByEmailAddress(emailAddress, options) {
		...
		return (
			this.findOne(query, options) || this.findOne(regexQuery, options)
		);
	}
	...
	findByUsersExcept(
		...
		const query = buildQueryOfFindByUsers(
			false,
			searchTerm,
			exceptions,
			forcedSearchFields,
	...
	getLastWhatsNewVersionById(_id) {
		return this.findOneById(_id).lastWhatsNewVersion;
	}
	...
	// FIND ONLY FOR EXTERNAL SERVICES
	findOneByUsernameOrEmail(usernameOrEmail, options) {
		const query = {
			$or: [
				{
				...
				{
					'emails.address': new RegExp(
						`^${s.escapeRegExp(usernameOrEmail)}$`,
						'i'
					)
				}
			]
	...
	findByUsernamesOrEmails(usernamesOrEmails, options) {
		const usernamesOrEmailsInsensitive = usernamesOrEmails.map(
			usernameOrEmail => new RegExp(`^${usernameOrEmail}$`, 'i')
		);
		...
		const query = {
			$or: [
				{
					username: {
						$in: usernamesOrEmailsInsensitive
					}
				},
				{
					'emails.address': {
	...
	updateLastWhatsNewVersionById(_id, lastWhatsNewVersion) {
		const update = {
			$set: {
				lastWhatsNewVersion
			}
	...
	limitLoginTokens(_id) {
		const update = {
			$push: {
				'services.resume.loginTokens': {
					$each: [],
					$slice: -settings.get('Accounts_LoginTokenMax')
				}
			}
		};
