...
export class Rooms extends Base {
	...
	setReactionsInLastMessage(roomId, lastMessage) {
		return this.update(
			{ _id: roomId },
			{ $set: { 'lastMessage.reactions': lastMessage.reactions } }
		);
	}
	...
	setReadOnlyById(_id, readOnly, hasPermission) {
		if (!hasPermission) {
			throw new Error(
				'You must provide "hasPermission" function to be able to call this method'
			);
		}
		const query = {
			_id
	...
	getTypeByIdOrName(_idOrName) {
		const room = this.findOneByIdOrName(_idOrName);
	...
	findBySubscriptionUserId(userId, options) {
		const data = Subscriptions.findByUserId(userId, { fields: { rid: 1 } })
			.fetch()
			.map(item => item.rid);
	...
	setOldRoomNameById(_id, oldRoomName) {
		const query = { _id };
	...
	resetLastMessageById(_id, messageId) {
		const query = { _id };
		const lastMessage =
			Messages.getLastVisibleMessageSentWithNoTypeByRoomId(
				_id,
				messageId
			);

		let newTimestamp = lastMessage?.ts;

		if (!newTimestamp) {
			const roomData = this.findOneById(_id);
			if (roomData.t !== RoomTypes.DM) {
				newTimestamp = roomData.ts;
			}
		}
	...
	incDestinationsCount(rid, role, updateTime) {
		const query = {
			_id: rid
		};

		const update = {
			$inc: { [`destinations.${role}`]: 1 }
		};

		return this.update(query, update, {}, updateTime);
	}

	decDestinationsCount(rid, role, updateTime) {
		const room = this.findOneById(rid);

		if (!room.destinations[role]) {
			return;
		}
