...
export class AppsRestApi {
	...
	_handleMultipartFormData(request) {
		...
		return Meteor.wrapAsync(callback => {
			...
			busboy.on(
				'file',
				Meteor.bindEnvironment((fieldname, file) => {
					const fileData = [];
					file.on(
						'data',
					...
					file.on(
						...
						Meteor.bindEnvironment(() => {
							formFields[fieldname] = Buffer.concat(fileData);
						})
					);
				})
			);
			...
			busboy.on(
				'finish',
				Meteor.bindEnvironment(() => callback(undefined, formFields))
			);
			request.pipe(busboy);
		})();
	}
				...
				post() {
					...
					if (this.bodyParams.url) {
						...
						try {
							...
							permissionsGranted = this.bodyParams
								.permissionsGranted;
						} catch (err) {
						...
						if (
							...
							return API.v1.failure({
								error:
									'Direct installation of an App is disabled.'
							});
						}
					...
					if (!buff) {
						return API.v1.failure({
							error:
								'Failed to get a file to install for the App. '
						});
					}
			...
			{ authRequired: true },
			{
				post() {
					if (
						...
						return API.v1.failure({
							error:
								'Event and externalComponent must be provided.'
						});
					}
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				...
				post() {
					...
					const aff = Promise.await(
						manager.update(buff, this.bodyParams.permissionsGranted)
					);
					const info = aff.getAppInfo();
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				post() {
					...
					const [workspaceIdSetting] = Settings.findById(
						'Cloud_Workspace_Id'
					).fetch();
		...
		this.api.addRoute(
			...
			{ authRequired: false },
			{
				get() {
					...
					if (prl) {
						const languages =
							prl.getStorageItem().languageContent || {};
