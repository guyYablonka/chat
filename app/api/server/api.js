...
const logger = new Logger('API', {}, __filename);
const rateLimiterDictionary = {};
export const defaultRateLimiterOptions = {
	numRequestsAllowed: settings.get(
...
export class APIClass extends Restivus {
	constructor(properties) {
		...
		settings.get('Log_Trace_API_Responses', (key, value) => {
		...
		settings.get(
			...
			(key, value) => {
				try {
					this.excludedRoutesFromLog = JSON.parse(value);
				} catch (err) {
					this.excludedRoutesFromLog = [
						'settings.public',
						'commands.list',
	...
	isRouteLogDisabled() {
		const routeName = (httpContext.get('route') ?? '').split('/').pop();

		return this.excludedRoutesFromLog.some(route =>
			routeName.startsWith(route)
		);
	}
	...
	shouldAddRateLimitToRoute(options) {
		...
		return (
			...
			Boolean(
				defaultRateLimiterOptions.numRequestsAllowed &&
					defaultRateLimiterOptions.intervalTimeInMS
			)
		);
	}
	...
	success(result = {}) {
		...
		this.loggingEnabled &&
			!this.isRouteLogDisabled() &&
			logger.api('Response data', result);
	...
	notFound(msg) {
		...
		this.loggingEnabled && logger.api('Response data', result);
		return result;
	}
	...
	notImplemented(msg) {
		const result = {
			...
			body: {
				success: false,
				error: msg || 'Method not supported'
			}
		};
	...
	enforceRateLimit(objectForRateLimitMatch, request, response, userId) {
		if (
			!this.shouldVerifyRateLimit(objectForRateLimitMatch.route, userId)
		) {
			return;
		}

		rateLimiterDictionary[
			objectForRateLimitMatch.route
		].rateLimiter.increment(objectForRateLimitMatch);
		const attemptResult = rateLimiterDictionary[
			objectForRateLimitMatch.route
		].rateLimiter.check(objectForRateLimitMatch);
		const timeToResetAttempsInSeconds = Math.ceil(
			attemptResult.timeToReset / 1000
		);
		response.setHeader(
			'X-RateLimit-Limit',
			rateLimiterDictionary[objectForRateLimitMatch.route].options
				.numRequestsAllowed
		);
		response.setHeader(
			'X-RateLimit-Remaining',
			attemptResult.numInvocationsLeft
		);
		response.setHeader(
			'X-RateLimit-Reset',
			new Date().getTime() + attemptResult.timeToReset
		);
		...
		this._routes.forEach(route => {
			if (this.shouldAddRateLimitToRoute(route.options)) {
				this.addRateLimiterRuleForRoutes({
					routes: [route.path],
					rateLimiterOptions:
						route.options.rateLimiterOptions ||
						defaultRateLimiterOptions,
					endpoints: Object.keys(route.endpoints).filter(
	...
	addRateLimiterRuleForRoutes({
		...
		if (!rateLimiterOptions.numRequestsAllowed) {
			throw new Meteor.Error(
				'You must set "numRequestsAllowed" property in rateLimiter for REST API endpoint'
			);
		}
		if (!rateLimiterOptions.intervalTimeInMS) {
			throw new Meteor.Error(
				'You must set "intervalTimeInMS" property in rateLimiter for REST API endpoint'
			);
		}
	...
	namedRoutes(route, endpoints, apiVersion) {
		const routeActions = Array.isArray(endpoints)
			? endpoints
			: Object.keys(endpoints);

		return routeActions.map(action =>
			this.getFullRouteName(route, action, apiVersion)
		);
	}
			...
			Object.keys(endpoints).forEach(method => {
				...
				endpoints[method].action =
					function _internalRouteActionHandler() {
						const rocketchatRestApiEnd =
							metrics.rocketchatRestApi.startTimer({
								method,
								version,
								...
								entrypoint: route.startsWith('method.call')
									? decodeURIComponent(
											this.request._parsedUrl.pathname.slice(
												8
											)
									  )
									: route
						...
						try {
							contextData = {
								...
								username: httpContext.get('username')
							};
							...
							api.loggingEnabled &&
								logger.api('request params', {
									...contextData,
									bodyParams: this.request.body,
									queryParams: this.request.query,
									routeParams: this.request.params
								});
							...
							result = DDP._CurrentInvocation.withValue(
								invocation,
								() => originalAction.apply(this)
							);
						} catch (e) {
							logger.error(
								`api error in route ${route} -> ${e.message}`,
								{
									...e,
									route,
									method,
	...
	_initAuth() {
		...
		this.addRoute(
			...
			{
				post() {
					let args = loginCompatibility(
						this.bodyParams,
						this.request
					);
					const { serverAuthCode, codeVerifier } = args.mauth || {};

					if (serverAuthCode && codeVerifier) {
						const authResponse = Meteor.call('mobileAuthenticate', {
							serverAuthCode,
							codeVerifier
						});
						...
						if (
							...
							authResponse.response.statusCode !== statusCodes.ok
						) {
							let error, message;
							...
							if (
								...
								message =
									'Unexpected error in mobile authentication process';
							}

							logger.error('Error in mobile auth', {
								error,
								msg: message
							});
					...
					let auth;
					try {
						auth = DDP._CurrentInvocation.withValue(
							invocation,
							() => Meteor.call('login', args)
						);
					} catch (error) {
						let e = error;
		...
		this.addRoute(
			...
			{
				get() {
					const serverURL = process.env.WEB_AUTH_URL;

					logger.info(
						'For oidc returning to client auth url:',
						serverURL
					);
...
const defaultOptionsEndpoint = function _defaultOptionsEndpoint() {
	if (
		...
		this.request.headers['access-control-request-method']
	) {
		if (settings.get('API_Enable_CORS') === true) {
			this.response.writeHead(statusCodes.ok, {
			...
			this.response.writeHead(statusCodes.methodNotAllowed);
			this.response.write(
				'CORS not enabled. Go to "Admin > General > REST Api" to enable it.'
			);
		}
	} else {
		this.response.writeHead(statusCodes.notFound);
	}
	this.done();
};
...
const createApi = function _createApi(_api, options = {}) {
	...
	if (settings.get('API_Enable_CORS')) {
		...
		_api._config.defaultHeaders['Access-Control-Allow-Headers'] =
...
settings.get('Accounts_CustomFields', (key, value) => {
	...
	try {
		...
		const nonPublicCustomFields = Object.keys(customFields).filter(
			customFieldKey => customFields[customFieldKey].public !== true
		);
