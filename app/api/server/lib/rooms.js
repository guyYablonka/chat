...
export async function findAdminRooms({
	...
	const showTypes = Array.isArray(types)
		? types.filter(type => type !== 'discussions')
		: [];
	const options = {
		fields,
	...
	if (name && showTypes.length) {
		cursor = Rooms.findByNameContainingAndTypes(
			name,
			showTypes,
			discussion,
			options
...
export async function findChannelAndPrivateAutocomplete({ uid, selector }) {
	...
	const rooms = await Rooms.findChannelAndPrivateByNameStarting(
		selector.name,
		options
	).toArray();
