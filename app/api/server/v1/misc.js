...
import initiateRequestData from '../../../lib/server/lib/initiateRequestData';

const logger = new Logger('Misc', {}, __filename);
API.v1.addRoute(
	'info',
	{ authRequired: false },
	{
		get() {
			const warningMessage =
				'The endpoint "/v1/info" is deprecated and will be removed after version v3.0.0';
		...
		get() {
			...
			if (hideIcon && (!name || !name.trim())) {
				return API.v1.failure(
					'Name cannot be empty when icon is hidden'
				);
			}
			...
			switch (type) {
				...
				case 'channel':
					if (!channel) {
						return API.v1.failure(
							'Shield channel is required for type "channel"'
						);
					}
	...
	{ authRequired: true },
	{
		get() {
			...
			if (sort && Object.keys(sort).length > 1) {
				return API.v1.failure(
					'This method support only one "sort" parameter'
				);
			}
			const sortBy = sort ? Object.keys(sort)[0] : undefined;
			const sortDirection =
				sort && Object.values(sort)[0] === 1 ? 'asc' : 'desc';
...
const methodCall = () => ({
	post() {
		...
		if (!httpContext.get('requestId')) {
			initiateRequestData(this.request);
		}
