	...
	{ authRequired: true },
	{
		get() {
			const warningMessage =
				'The endpoint "emoji-custom" is deprecated and will be removed after version v3.0.0';
	...
	{ authRequired: true },
	{
		get() {
			...
			if (updatedSince) {
				...
				return API.v1.success({
					emojis: {
						update: EmojiCustom.find({
							...query,
							_updatedAt: { $gt: updatedSinceDate }
						}).fetch(),
						remove: EmojiCustom.trashFindDeletedAfter(
							updatedSinceDate
						).fetch()
					}
				});
					...
					busboy.on(
						...
						Meteor.bindEnvironment(() => {
							try {
								...
								const emojiToUpdate = EmojiCustom.findOneById(
									fields._id
								);
								if (!emojiToUpdate) {
									return callback(
