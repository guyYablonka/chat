	...
	{ authRequired: true },
	{
		get() {
			...
			return API.v1.success({
				roles: {
					update: Roles.findByUpdatedDate(new Date(updatedSince), {
						fields: API.v1.defaultFieldsToExclude
					}).fetch(),
					remove: Roles.trashFindDeletedAfter(
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			return API.v1.success({
				role: Roles.findOneByIdOrName(roleData.name, {
					fields: API.v1.defaultFieldsToExclude
				})
			});
		}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			return API.v1.success({
				role: Roles.findOneByIdOrName(this.bodyParams.roleName, {
					fields: API.v1.defaultFieldsToExclude
				})
			});
		}
