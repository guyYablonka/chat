...
const fetchSettings = (query, sort, offset, count, fields) => {
	const settings = Settings.find(query, {
		...
		fields: Object.assign(
			{ _id: 1, value: 1, enterprise: 1, invalidValue: 1, modules: 1 },
			fields
		)
	}).fetch();
...
API.v1.addRoute(
	...
	{ authRequired: false },
	{
		get() {
			...
			const ourfields = { ...fields, enterprise: 0 };
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			return API.v1.success(
				_.pick(
					Settings.findOneNotHiddenById(this.urlParams._id),
					'_id',
					'value'
				)
			);
		...
		post: {
			...
			action() {
				...
				const setting = Settings.findOneNotHiddenById(
					this.urlParams._id
				);
				if (
					setting.type === 'action' &&
...
API.v1.addRoute(
	'classificationLevels.get',
	{ authRequired: false },
	{
		get() {
			const classificationJson = settings.get('Classification_json');

			const classifications = JSON.parse(classificationJson);
