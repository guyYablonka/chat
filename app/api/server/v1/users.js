	...
	{ authRequired: true },
	{
		post() {
			check(this.bodyParams, {
				...
				createdFrom: Match.Maybe(String)
			});

			if (!hasPermission(this.userId, 'create-user')) {
			...
			if (typeof this.bodyParams.verified === 'undefined') {
				this.bodyParams.verified = true;
			}
			...
			return API.v1.success({
				user: Users.findOneById(newUserId, { fields })
			});
		}
	}
);
...
API.v1.addRoute(
	...
	{ authRequired: false },
	{
		get() {
			...
			const url = getURL(`/avatar/${user.username}`, {
				cdn: false,
				full: true
			});
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const count = Users.setActiveNotLoggedInAfterWithRole(
				lastLoggedIn,
				role,
				false
			);
	...
	{ authRequired: true },
	{
		get() {
			...
			if (
				fields.userRooms === 1 &&
				(myself ||
					hasPermission(this.userId, 'view-other-user-channels'))
			) {
...
API.v1.addRoute(
	...
	{
		get() {
			const { usernameOrId } = this.requestParams();
			const userData = Users.findOneByIdOrUsername(usernameOrId, {
				fields: {
					customFields: 1,
					name: 1
				}
			...
			const { customFields, name } = userData;
			let fullName = name?.split('/')?.pop();

			if (customFields?.firstName && customFields?.lastName) {
				fullName = customFields.firstName + ' ' + customFields.lastName;
			}
		...
		post() {
			if (this.userId) {
				return API.v1.failure(
					'Logged in users can not register again.'
				);
			}
			...
			Meteor.runAsUser(userId, () =>
				Meteor.call('setUsername', this.bodyParams.username)
			);
			const { fields } = this.parseJsonQuery();

			return API.v1.success({
				user: Users.findOneById(userId, { fields })
			});
		}
	}
);
			...
			Meteor.runAsUser(user._id, () => {
				if (this.bodyParams.avatarUrl) {
					...
					const getUserFromFormData = fields => {
						...
						if (fields.username) {
							return Users.findOneByUsernameIgnoringCase(
								fields.username,
								{ _id: 1 }
							);
						}
							...
							Meteor.bindEnvironment(
								(
									...
									file.on(
										'end',
										Meteor.bindEnvironment(() => {
											const sentTheUserByFormData =
												fields.userId ||
												fields.username;
											if (sentTheUserByFormData) {
												user =
													getUserFromFormData(fields);
												if (!user) {
													return callback(
														new Meteor.Error(
															'error-invalid-user',
...
API.v1.addRoute(
	'users.update',
	{ authRequired: true, twoFactorRequired: false },
	{
		post() {
			check(this.bodyParams, {
				userId: String,
			...
			const {
				userId,
				data: { username, name, customFields }
			} = this.bodyParams;
			...
			Meteor.runAsUser(this.userId, () =>
				saveUser(this.userId, userData)
			);

			if (customFields) {
				saveCustomFields(this.bodyParams.userId, customFields);

				const userToChange = Users.findOneById(userId);
			...
			return API.v1.success({
				user: Users.findOneById(this.bodyParams.userId, { fields })
			});
		}
	}
);
...
API.v1.addRoute(
	...
	{
		post() {
			this.validateUpdateByUsernameBodyParams();

			const { username, data } = this.bodyParams;

			const user = Users.findOneByUsernameOrEmail(username);
			...
			if (data.customFields) {
				saveCustomFields(user._id, data.customFields);
			}

			if (typeof data.active === 'boolean') {
				Meteor.runAsUser(this.userId, () => {
					Meteor.call('setUserActiveStatus', user._id, data.active);
				});
			}
...
API.v1.addRoute(
	...
	{
		post() {
			check(this.bodyParams, { phoneNumber: Match.Maybe(String) });
			const newPhoneNumber = this.bodyParams.phoneNumber;

			const userToUpdate = getUserById(
				this.userId,
				API.v1.defaultFieldsToExclude
			);

			if (userToUpdate.customFields.phoneNumber === newPhoneNumber) {
				return API.v1.success({
					user: userToUpdate
				});
			}

			updateUserPhoneNumber(this.userId, newPhoneNumber);

			return API.v1.success({
				user: getUserById(this.userId, API.v1.defaultFieldsToExclude)
			});
		}
	}
);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const userId = this.bodyParams.userId
				? this.bodyParams.userId
				: this.userId;
			...
			Meteor.runAsUser(userId, () =>
				Meteor.call('saveUserPreferences', this.bodyParams.data)
			);
	...
	{ authRequired: true },
	{
		get() {
			const result = Meteor.runAsUser(this.userId, () =>
				Meteor.call('getUsernameSuggestion')
			);
	...
	{ authRequired: true, twoFactorRequired: false },
	{
		post() {
			...
			const token = Meteor.runAsUser(this.userId, () =>
				Meteor.call('personalAccessTokens:generateToken', {
					tokenName,
					bypassTwoFactor
				})
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const loginTokens = Users.getLoginTokensByUserId(
				this.userId
			).fetch()[0];
...
API.v1.addRoute('users.2fa.sendEmailCode', {
	post() {
		...
		const method = emailOrUsername.includes('@')
			? 'findOneByEmailAddress'
			: 'findOneByUsername';
		const userId =
			this.userId ||
			Users[method](emailOrUsername, { fields: { _id: 1 } })?._id;

		if (!userId) {
			this.logger.error(
				'[2fa] User was not found when requesting 2fa email code'
			);
			return API.v1.success();
		}
	...
	{ authRequired: true },
	{
		get() {
			...
			if (from) {
				...
				if (diff < 10) {
					return API.v1.success({
						users: Users.findNotIdUpdatedFrom(
							this.userId,
							ts,
							options
						).fetch(),
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const result = Meteor.runAsUser(this.userId, () =>
				Meteor.call('requestDataDownload', {
