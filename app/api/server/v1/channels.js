		...
		post() {
			...
			if (!sub) {
				return API.v1.failure(
					`The user/callee is not in the channel "${findResult.name}.`
				);
			}

			if (!sub.open) {
				return API.v1.failure(
					`The channel, ${findResult.name}, is already closed to the sender`
				);
			}
...
function createChannelValidator(params) {
	...
	if (
		...
		!_.isArray(params.members.value)
	) {
		throw new Error(
...
function createChannel(userId, params) {
	const readOnly =
		typeof params.readOnly !== 'undefined' ? params.readOnly : false;
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			return API.v1.success(
				API.channels.create.execute(userId, bodyParams)
			);
		}
	}
);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			if (
				typeof this.queryParams.includeAllPublicChannels !== 'undefined'
			) {
				includeAllPublicChannels =
					this.queryParams.includeAllPublicChannels === 'true';
			}
...
API.v1.addRoute(
	['channels.info', 'channels.info.limited'],
	{ authRequired: true },
	{
		get() {
			return API.v1.success({
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			Meteor.runAsUser(this.userId, () => {
				Meteor.call('joinRoom', findResult._id);
			});
	...
	{ authRequired: true },
	{
		get: {
			...
			action() {
				const { offset, count } = this.getPaginationItems();
				const { sort, fields, query } = this.parseJsonQuery();
				const hasPermissionToSeeAllPublicChannels = hasPermission(
					this.userId,
					'view-c-room'
				);
				...
				if (!hasPermissionToSeeAllPublicChannels) {
					...
					const roomIds = Subscriptions.findByUserIdAndType(
						this.userId,
						'c',
						{
							fields: { rid: 1 }
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (sub.open) {
				return API.v1.failure(
					`The channel, ${findResult.name}, is already open to the sender`
				);
			}
...
API.v1.addRoute(
	['channels.rename', 'channels.rename.limited'],
	{ authRequired: true },
	{
		post() {
			...
			if (findResult.name === this.bodyParams.name) {
				return API.v1.failure(
					'The channel name is the same as what it would be renamed to.'
				);
			}
		...
		post() {
			...
			Meteor.runAsUser(this.userId, () => {
				Meteor.call(
					...
					'default',
					['true', '1'].includes(
						this.bodyParams.default.toString().toLowerCase()
					)
				);
			});
...
API.v1.addRoute(
	['channels.setReadOnly', 'channels.setReadOnly.limited'],
	{ authRequired: true },
	{
		post() {
	...
	{ authRequired: true },
	{
		post() {
			...
			if (findResult.topic === this.bodyParams.topic) {
				return API.v1.failure(
					'The channel topic is the same as what it would be changed to.'
				);
			}
	...
	{ authRequired: true },
	{
		post() {
			...
			if (findResult.t === this.bodyParams.type) {
				return API.v1.failure(
					'The channel type is the same as what it would be changed to.'
				);
			}
			...
			return API.v1.success({
				channel: this.composeRoomWithLastMessage(
					Rooms.findOneById(findResult._id, {
	...
	{ authRequired: true },
	{
		post() {
			...
			if (!findResult.archived) {
				return API.v1.failure(
					`The channel, ${findResult.name}, is not archived`
				);
			}
