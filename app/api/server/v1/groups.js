...
import { getDestinations } from '../../../lib/server/functions';
...
import { API } from '../api';
// import { settings } from '/app/livechat/server/api/lib/livechat';
import { settings } from '../../../settings/server';
import { mapEmailsToUsernames } from '../utils/mapEmailsToUsernames';

// Returns the private group subscription IF found otherwise it will return the failure of why it didn't. Check the `statusCode` property
export function findPrivateGroupByIdOrName({
	params,
	userId,
		...
		post() {
			...
			Meteor.runAsUser(this.userId, () => {
				Meteor.call(
					'addAllUserToRoom',
					findResult.rid,
					this.bodyParams.activeUsersOnly
				);
			});

			return API.v1.success({
				group: this.composeRoomWithLastMessage(
					Rooms.findOneById(findResult.rid, {
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			this.validateOwnerActionsAsUserBodyParams();

			const { originUsername: originUsernameOrEmail, extraData } =
				this.requestParams();

			const originUser = Users.findOneByUsernameOrEmail(
				originUsernameOrEmail
			);

			if (!originUser) {
				throw new Meteor.Error(
					'error-invalid-user',
					`User ${originUsernameOrEmail} does not exist.`
				);
			}
			...
			if (proxyRole) {
				this.validateCreatorOrigin(proxyRole, originUser);
			}
			...
			Meteor.runAsUser(originUser._id, () => {
				Meteor.call(
					'addRoomOwner',
					findResult.rid,
					user._id,
	...
	{ authRequired: true },
	{
		get() {
			...
			if (typeof subscription !== 'undefined' && subscription.open) {
				unreads =
					Messages.countVisibleByRoomIdBetweenTimestampsInclusive(
						subscription.rid,
	...
	{ authRequired: true },
	{
		post() {
			...
			if (
				...
				!_.isArray(this.bodyParams.members)
			) {
				return API.v1.failure(
					'Body param "members" must be an array if provided'
				);
			}

			if (
				this.bodyParams.customFields &&
				!(typeof this.bodyParams.customFields === 'object')
			) {
				return API.v1.failure(
					'Body param "customFields" must be an object if provided'
				);
			}

			const readOnly =
				typeof this.bodyParams.readOnly !== 'undefined'
					? this.bodyParams.readOnly
					: false;
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			if (
				typeof this.queryParams.includeAllPrivateGroups !== 'undefined'
			) {
				includeAllPrivateGroups =
					this.queryParams.includeAllPrivateGroups === 'true';
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			this.validateCreateAsUserBodyParams();

			const {
				name,
				creatorUsername: creatorUsernameOrEmail,
				extraData: proxyData
			} = this.bodyParams;

			const creator = Users.findOneByUsernameOrEmail(
				creatorUsernameOrEmail
			);

			if (!creator) {
				throw new Meteor.Error(
					'error-invalid-user',
					`User ${creatorUsernameOrEmail} does not exist.`
				);
			}

			if (!hasPermission(creator._id, 'create-p')) {
				return API.v1.unauthorized();
			}
			...
			if (proxyRole) {
				this.validateCreatorOrigin(proxyRole, creator);
			}

			const membersAsUsernamesOrEmails =
				mapEmailsToUsernames(this.bodyParams.members) || [];

			const destinations = getDestinations([
				creatorUsernameOrEmail,
				...membersAsUsernamesOrEmails
			]);
	...
	{ authRequired: true },
	{
		post() {
			...
			const {
				roomId = '',
				roomName = '',
				originUsername: originUsernameOrEmail,
				extraData
			} = this.requestParams();
			...
			Meteor.runAsUser(originUser._id, () =>
				Meteor.call('addUserToRoom', { rid, username, extraData })
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			this.validateLeaveAsUserBodyParams();

			const { leaver: leaverUsernameOrEmail, extraData } =
				this.requestParams();

			const leaverUser = Users.findOneByUsernameOrEmail(
				leaverUsernameOrEmail
			);

			if (!leaverUser) {
				throw new Meteor.Error(
					'error-invalid-user',
					`User ${leaverUsernameOrEmail} does not exist.`
				);
			}
			...
			if (proxyRole) {
				this.validateCreatorOrigin(proxyRole, leaverUser);
			}

			const findResult = findPrivateGroupByIdOrName({
				params: this.requestParams(),
				userId: leaverUser._id
			});

			Meteor.runAsUser(leaverUser._id, () => {
				Meteor.call('leaveRoom', findResult.rid, extraData);
			});
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			if (
				room.broadcast &&
				!hasPermission(this.userId, 'view-broadcast-member-list')
			) {
				return API.v1.unauthorized();
			}
	...
	{ authRequired: true },
	{
		post() {
			if (
				...
				return API.v1.failure(
					'The bodyParam "customFields" is required with a type like object.'
				);
			}
	...
	{ authRequired: true },
	{
		post() {
			if (!this.bodyParams.hasOwnProperty('description')) {
				return API.v1.failure(
					'The bodyParam "description" is required'
				);
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			this.validateRenameAsUserBodyParams();
			...
			const byUser = Users.findOneByUsernameOrEmail(byUsernameOrEmail);
			...
			if (proxyRole) {
				this.validateCreatorOrigin(proxyRole, byUser);
			}
	...
	{ authRequired: true },
	{
		post() {
			...
			if (findResult.t === this.bodyParams.type) {
				return API.v1.failure(
					'The private group type is the same as what it would be changed to.'
				);
			}
	...
	{ authRequired: true },
	{
		post() {
			if (!this.bodyParams.hasOwnProperty('announcement')) {
				return API.v1.failure(
					'The bodyParam "announcement" is required'
				);
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const moderators = Subscriptions.findByRoomIdAndRoles(
				findResult.rid,
				['moderator'],
				{ fields: { u: 1 } }
			)
	...
	{ authRequired: true },
	{
		post() {
			if (
				!Match.test(
					this.bodyParams,
					Match.ObjectIncluding({ encrypted: Boolean })
				)
			) {
...
API.v1.addRoute(
	'groups.setClassificationLevel.limited',
	{ authRequired: true },
	{
		post() {
			check(this.bodyParams.classificationLevel, Number);
			...
			const classificationJson = JSON.parse(
				settings.get('Classification_json')
			);

			if (
				!classificationJson.find(
					classification =>
						classification.id == this.bodyParams.classificationLevel
				)
			) {
				return API.v1.failure(
					'Body param "ClassificationLevel" value is not configured in the server'
				);
			}

			if (
				room.classificationLevel === this.bodyParams.classificationLevel
			) {
				return API.v1.failure(
					'The group classificationLevel is the same as what it would be changed to.'
				);
			}

			// TODO add updating the room's classification level in the DB here

			const finalRoom = Rooms.findOneById(findResult.rid, {
				fields: API.v1.defaultFieldsToExclude
			});
