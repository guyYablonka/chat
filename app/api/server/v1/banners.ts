        ...
        get() {
            ...
            if (!platform) {
                throw new Meteor.Error(
                    'error-missing-param',
                    'The required "platform" param is missing.'
                );
            }
            ...
            const banners = Promise.await(
                Banner.getNewBannersForUser(this.userId, platform, bannerId)
            );
    ...
    { authRequired: true },
    {
        post() {
            ...
            if (!bannerId || !bannerId.trim()) {
                throw new Meteor.Error(
                    'error-missing-param',
                    'The required "bannerId" param is missing.'
                );
            }
