				...
				busboy.on(
					...
					Meteor.bindEnvironment(
						(fieldname, file, filename, encoding, mimetype) => {
							const isValidAsset = Object.keys(
								RocketChatAssets.assets
							).includes(fieldname);
							if (!isValidAsset) {
								callback(
									new Meteor.Error(
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const isValidAsset = Object.keys(RocketChatAssets.assets).includes(
				assetName
			);
			if (!isValidAsset) {
				throw new Meteor.Error('error-invalid-asset');
			}
			Meteor.runAsUser(this.userId, () =>
