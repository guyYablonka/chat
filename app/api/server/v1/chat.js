...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			this.validateRemoveBodyParameters();

			const {
				fromUsername: fromUsernameOrEmail,
				message: msg,
				ts,
				extraData
			} = this.bodyParams;
			...
			const messageToDelete = Messages.findOneByInfo(
				room._id,
				fromUser.username,
				msg,
				date
			...
			Meteor.runAsUser(fromUser._id, () =>
				Meteor.call('deleteMessage', messageToDelete, extraData)
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const { senderUsername: fromUsernameOrEmail } = this.bodyParams;

			const sender = Users.findOneByUsernameOrEmail(fromUsernameOrEmail);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			let result;
			Meteor.runAsUser(this.userId, () => {
				result = Meteor.call('messageSearch', searchText, roomId, count)
					.message.docs;
			});
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (selfRoomId) {
				this.bodyParams.message.rid = selfRoomId;
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const [message] = normalizeMessagesForUser([sent], fromUser._id);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			this.validateUpdateMessageBodyParameters();
			...
			const date = new Date(ts);
			const originalMessage = Messages.findOneByInfo(
				room._id,
				fromUser.username,
				msg,
				date
			...
			Meteor.runAsUser(fromUser._id, () =>
				Meteor.call('updateMessage', updateMessage, extraData)
			);

			const [message] = normalizeMessagesForUser(
				[Messages.findOneById(updateMessage._id)],
				this.userId
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (selfRoomId) {
				this.bodyParams.roomId = selfRoomId;
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			ignore =
				typeof ignore === 'string' ? /true|1/.test(ignore) : ignore;
