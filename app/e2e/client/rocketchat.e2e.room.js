...
const reduce = (prev, next) => {
	...
	switch (prev) {
		...
		case E2E_ROOM_STATES.NOT_STARTED:
			return (
				[
					E2E_ROOM_STATES.ESTABLISHING,
					E2E_ROOM_STATES.PAUSED,
					E2E_ROOM_STATES.DISABLED,
					E2E_ROOM_STATES.KEYS_RECEIVED
				].includes(next) && next
			);
		case E2E_ROOM_STATES.READY:
			return (
				[E2E_ROOM_STATES.PAUSED, E2E_ROOM_STATES.DISABLED].includes(
					next
				) && next
			);
		...
		case E2E_ROOM_STATES.ESTABLISHING:
			return (
				[
					E2E_ROOM_STATES.READY,
					E2E_ROOM_STATES.KEYS_RECEIVED,
					E2E_ROOM_STATES.ERROR,
					E2E_ROOM_STATES.PAUSED,
					E2E_ROOM_STATES.DISABLED,
					E2E_ROOM_STATES.WAITING_KEYS
				].includes(next) && next
			);
		default:
			return next;
...
export class E2ERoom extends Emitter {
	...
	constructor(userId, roomId, t) {
		...
		this.once(E2E_ROOM_STATES.READY, () =>
			this.decryptPendingSubscription()
		);
	...
	pause() {
		![E2E_ROOM_STATES.PAUSED, E2E_ROOM_STATES.DISABLED].includes(
			...
			this.setState(
				this.state === E2E_ROOM_STATES.READY
					? E2E_ROOM_STATES.PAUSED
					: E2E_ROOM_STATES.DISABLED
			);
	}
	...
	enable() {
		![E2E_ROOM_STATES.READY].includes(this.state) &&
	...
	isReady() {
		return [E2E_ROOM_STATES.PAUSED, E2E_ROOM_STATES.READY].includes(
			this.state
		);
	}
	...
	async handshake() {
		switch (this.state) {
			...
			case E2E_ROOM_STATES.NOT_STARTED:
				...
				try {
					const groupKey = Subscriptions.findOne({ rid: this.roomId })
						.E2EKey;
					if (groupKey) {
