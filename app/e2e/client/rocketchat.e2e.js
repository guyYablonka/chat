...
class E2E {
	...
	async getInstanceByRoomId(roomId) {
		...
		const room = await waitUntilFind(() =>
			Rooms.findOne({
				_id: roomId
			})
		);
		...
		this.instancesByRoomId[roomId] =
			this.instancesByRoomId[roomId] ??
			new E2ERoom(Meteor.userId(), roomId, room.t);
	...
	async startClient() {
		...
		if (!private_key && this.db_private_key) {
			try {
				...
				this.openAlert({
					...
					html:
						'<div>Your encryption password seems wrong. Click here to try again.</div>',
		...
		const randomPassword = Meteor._localStorage.getItem(
			'e2e.randomPassword'
		);
		if (randomPassword) {
				...
				action: () => {
					modal.open(
						...
						confirm => {
							...
							Meteor._localStorage.removeItem(
								'e2e.randomPassword'
							);
							this.closeAlert();
						}
	...
	async loadKeys({ public_key, private_key }) {
		...
		try {
			this.privateKey = await importRSAKey(EJSON.parse(private_key), [
				'decrypt'
			]);
	...
	async getMasterKey(password) {
		...
		try {
			...
			return this.error(
				'Error creating a key based on user password: ',
				error
			);
		}
		...
		return new Promise(resolve => {
			...
			const showModal = () => {
				modal.open(
					{
						title: TAPi18n.__(
							'Enter_E2E_password_to_decode_your_key'
						),
						type: 'input',
						inputType: 'password',
						html: true,
...
Meteor.startup(function () {
	Tracker.autorun(function () {
		if (Meteor.userId()) {
			const adminEmbedded =
				Layout.isEmbedded() &&
				FlowRouter.current().path.startsWith('/admin');
	...
	Tracker.autorun(() => {
		...
		promises.add(
			...
			async function (message) {
				...
				return e2eRoom.encrypt(message).then(msg => {
					message.msg = msg;
					message.t = 'e2e';
					message.e2e = 'pending';
					return message;
