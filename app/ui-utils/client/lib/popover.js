...
Template.popover.onRendered(function () {
	...
	const position = _.throttle(() => {
		const direction =
			typeof this.data.direction === 'function'
				? this.data.direction()
				: this.data.direction;

		const verticalDirection = /top/.test(direction) ? 'top' : 'bottom';
		const rtlDirection =
			isRtl() ^ /inverted/.test(direction) ? 'left' : 'right';
		...
		const position =
			typeof this.data.position === 'function'
				? this.data.position()
				: this.data.position;
		...
		const offsetWidth =
			offsetHorizontal * (horizontalDirection === 'left' ? 1 : -1);
		const offsetHeight =
			offsetVertical * (verticalDirection === 'bottom' ? 1 : -1);

		const mousePositionHorizontalDiff = window.innerWidth - mousePosition.x;
		const horizontalPaddingDirection =
			mousePositionHorizontalDiff <= window.innerWidth / 2 &&
			mousePositionHorizontalDiff > 0
				? 'right'
				: 'left';
		const horizontalPaddingDiff =
			horizontalPaddingDirection === 'right'
				? -1 * (window.innerWidth - originalWidth)
				: window.innerWidth - originalWidth;

		if (position) {
			popoverContent.style.top = `${position.top}px`;
			popoverContent.style[horizontalPaddingDirection] = `${
				position[horizontalPaddingDirection] + horizontalPaddingDiff
			}px`;
		} else {
			...
			let horizontalPadding =
				mousePosition.x - popoverWidth + offsetWidth;

			if (horizontalDirection === 'right') {
				horizontalPadding = mousePosition.x + offsetWidth;
			}

			if (horizontalPadding + popoverWidth >= windowWidth) {
				horizontalPadding =
					mousePosition.x - popoverWidth + offsetWidth;
			}

			if (horizontalPaddingDirection === 'right') {
				horizontalPadding = windowWidth - mousePosition.x;
			}
			...
			popoverContent.style[horizontalPaddingDirection] = `${
				horizontalPadding + horizontalPaddingDiff
			}px`;

			if (popoverContent.style.left && popoverContent.style.right) {
				const directionToDelete =
					horizontalPaddingDirection === 'right' ? 'left' : 'right';
				popoverContent.style[directionToDelete] = '';
			}
		}
		...
		if (realTop + popoverContent.offsetHeight > window.innerHeight) {
			...
			popoverContent.className =
				'rc-popover__content rc-popover__content-scroll';
		}
