...
export function messageContext({ rid } = Template.instance()) {
	...
	const runAction = Layout.isEmbedded()
		? (msg, e) => {
	...
	return {
		...
		settings: {
			...
			Message_Enable_Reply_directly: settings.get(
				'Message_Enable_Reply_directly'
			),
			Message_Enable_Get_Link: settings.get('Message_Enable_Get_Link'),
			Message_Enable_Report: settings.get('Message_Enable_Report'),
			Message_Mark_As_Unread_Enabled: settings.get(
				'Message_Mark_As_Unread_Enabled'
			)
		}
	};
}
