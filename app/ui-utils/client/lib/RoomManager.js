...
export const RoomManager = new (function () {
	...
	const handleTrackSettingsChange = msg => {
		...
		Tracker.nonreactive(() => {
			if (msg.t === 'room_changed_privacy') {
				const type =
					FlowRouter.current().route.name === 'channel' ? 'c' : 'p';
	...
	const Cls = class {
		...
		closeAndDismiss(typeName) {
			RoomManager.close(typeName);
		...
		updateMentionsMarksOfRoom(typeName) {
			...
			const totalHeight =
				$(' > .wrapper > ul', messagesBox).height() + 40;
			if (!ticksBar) {
				return;
			}
			...
			ticksBar.innerHTML = Array.from(
				...
				.map(mentionLink => {
					...
					const className = [
						'tick',
						mentionLink.classList.contains('mention-link--me') &&
							'tick--me',
