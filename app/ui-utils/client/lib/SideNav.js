...
export const SideNav = new (class {
	...
	closeFlex(callback = null) {
		...
		if (!routesNamesForRooms.includes(FlowRouter.current().route.name)) {
			const subscription = Subscriptions.findOne({
				rid: Session.get('lastOpenedRoom')
			});
			if (subscription) {
	...
	focusInput() {
		const sideNavDivs = Array.from(this.sideNav[0].children).filter(
			el => el.tagName === 'DIV' && !el.classList.contains('hidden')
		);
		let highestZidx = 0;
		let highestZidxElem;
		...
		setTimeout(() => {
			const ref =
				highestZidxElem && highestZidxElem.querySelector('input');
