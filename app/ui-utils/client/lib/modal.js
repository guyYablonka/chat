...
const createModal = (config = {}, fn, onCancel) => {
	...
	config.closeOnConfirm =
		config.closeOnConfirm == null ? true : config.closeOnConfirm;
	config.showConfirmButton =
		config.showConfirmButton == null ? true : config.showConfirmButton;
	config.showFooter =
		config.showConfirmButton === true || config.showCancelButton === true;
	config.confirmOnEnter =
		config.confirmOnEnter == null ? true : config.confirmOnEnter;
	config.closeOnEscape =
		config.closeOnEscape == null ? true : config.closeOnEscape;
	config.showCloseButton =
		config.showCloseButton == null ? true : config.showCloseButton;
...
Template.rc_modal.helpers({
	...
	modalIcon() {
		...
		return modalTypes[this.type] || `modal-${this.type}`;
	}
});
...
Template.rc_modal.events({
	...
	'click .js-confirm'(event, instance) {
		...
		if (instance.data?.type === 'cargo') {
			this.confirm(
				document.open(settings.get('Cargo_URL'), '', 'noopener=true')
			);
			return;
		}
