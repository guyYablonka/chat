...
export function federationRequestToPeer(
	method,
	peerDomain,
	uri,
	body,
	...
	const ignoreErrors =
		peerDomain === getFederationDomain() ? false : options.ignoreErrors;
	...
	try {
		logger.web.debug(`federationRequestToPeer => url=${baseUrl}${uri}`);
		...
		logger.web.error(`${ignoreErrors ? '[IGNORED] ' : ''}Error ${err}`);
