...
export function searchHub(peerDomain) {
	try {
		...
		logger.dns.debug(
			`searchHub: found peerDomain=${peerDomain} url=${url}`
		);
...
export function search(peerDomain) {
	...
	if (!srvEntry || !protocol) {
		logger.dns.debug(
			`search: could not find valid SRV entry peerDomain=${peerDomain} srvEntry=${JSON.stringify(
				srvEntry
			)} protocol=${protocol}`
		);
	...
	logger.dns.debug(
		`search: found peerDomain=${peerDomain} srvEntry=${srvEntry.name}:${srvEntry.port} protocol=${protocol}`
	);
