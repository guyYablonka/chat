...
export function federationSearchUsers(query) {
	...
	const {
		data: { users }
	} = federationRequestToPeer('GET', peerDomain, uri);
...
export function requestEventsFromLatest(
	domain,
	fromDomain,
	contextType,
	contextQuery,
	...
	logger.client.debug(
		`requestEventsFromLatest => domain=${domain} contextType=${contextType} contextQuery=${JSON.stringify(
			contextQuery,
			null,
			2
		)} latestEventIds=${latestEventIds.join(', ')}`
	);

	const uri = '/api/v1/federation.events.requestFromLatest';

	federationRequestToPeer('POST', domain, uri, {
		fromDomain,
		contextType,
		contextQuery,
		latestEventIds
...
export function dispatchEvents(domains, events) {
	...
	logger.client.debug(
		`dispatchEvents => domains=${domains.join(
			', '
		)} events=${events.map(e => JSON.stringify(e, null, 2))}`
	);

	const uri = '/api/v1/federation.events.dispatch';

	for (const domain of domains) {
		federationRequestToPeer(
			'POST',
			domain,
			uri,
			{ events },
