...
export const getNameAndDomain = fullyQualifiedName =>
...
export function isRegisteringOrEnabled() {
	...
	return [STATUS_ENABLED, STATUS_REGISTERING].includes(
		status && status.value
	);
}
...
export const checkRoomDomainsLength = domains =>
...
export const getFederatedRoomData = room => {
	...
	if (room.t === 'd') {
		...
		subscriptions = Subscriptions.findByRoomIdWhenUsernameExists(
			room._id
		).fetch();
