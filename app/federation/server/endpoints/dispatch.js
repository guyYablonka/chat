			...
			case contextDefinitions.ROOM.type:
				...
				if (eventResult.success) {
					...
					if (persistedRoom) {
						...
						// Denormalize room
						const denormalizedRoom = normalizers.denormalizeRoom(
							room
						);
		...
		if (eventResult.success) {
			...
			try {
				if (persistedSubscription) {
					...
					const denormalizedSubscription = normalizers.denormalizeSubscription(
						subscription
					);
	...
	async [eventTypes.ROOM_MESSAGE](event) {
		...
		if (eventResult.success) {
			...
			if (persistedMessage) {
				...
				// Denormalize message
				const denormalizedMessage = normalizers.denormalizeMessage(
					message
				);
				...
				if (denormalizedMessage.file) {
					...
					Meteor.runAsUser(upload.userId, () =>
...
API.v1.addRoute(
	...
	},
	{
		async post() {
			...
			logger.server.debug(
				`federation.events.dispatch => events=${events.map(e =>
			...
			for (const event of events) {
				...
				if (!eventResult || !eventResult.success) {
					try {
						logger.server.debug(
							`federation.events.dispatch => Event has missing parents -> event=${JSON.stringify(
								event,
								null,
								2
							)}`
						...
						logger.server.error(
							`dispatch => event=${JSON.stringify(
								event,
								null,
								2
							)} eventResult=${JSON.stringify(
								eventResult,
								null,
								2
