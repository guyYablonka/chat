...
async function beforeDeleteRoom(roomId) {
	...
	logger.client.debug(
		`beforeDeleteRoom => room=${JSON.stringify(room, null, 2)}`
	);

	try {
		// Create the message event
		const event = await FederationRoomEvents.createDeleteRoomEvent(
			getFederationDomain(),
			room._id
		);
		...
		logger.client.error('beforeDeleteRoom => Could not remove room:', {
			err
		});
