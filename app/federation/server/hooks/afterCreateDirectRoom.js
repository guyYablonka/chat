...
async function afterCreateDirectRoom(room, extras) {
	logger.client.debug(
		`afterCreateDirectRoom => room=${JSON.stringify(
			room,
			null,
			2
	...
	try {
		...
		const events = await Promise.all(
			extras.members.map(member => {
				...
				const normalizedSourceSubscription = normalizers.normalizeSubscription(
					sourceSubscription
				);
		...
		// Dispatch the events
		dispatchEvents(normalizedRoom.federation.domains, [
			genesisEvent,
			...events
		]);
	} catch (err) {
		...
		logger.client.error(
			'afterCreateDirectRoom => Could not create federated room:',
			{ err }
		);
	}
