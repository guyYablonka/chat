...
async function afterRemoveFromRoom(involvedUsers, room) {
	...
	logger.client.debug(
		`afterRemoveFromRoom => involvedUsers=${JSON.stringify(
			involvedUsers,
			null,
			2
	...
	try {
		...
		const domainsBeforeRemoval = [
			...new Set(usersBeforeRemoval.map(u => u.federation.origin))
		];
		...
		logger.client.error('afterRemoveFromRoom => Could not remove user:', {
			err
		});
	}
...
export const definition = {
	...
	callback: (roomOwner, room) =>
		Promise.await(afterRemoveFromRoom(roomOwner, room)),
