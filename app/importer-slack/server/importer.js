...
export class SlackImporter extends Base {
	...
	prepareUsingLocalFile(fullFilePath) {
		...
		const prepareChannelsFile = (
			entry,
			typeName,
			filterInvalidCreators = true
		) => {
				...
				try {
					if (
						...
						return this.logger.debug(
							`Ignoring the file: ${entry.entryName}`
						);
					}
					...
					if (
						...
						try {
							...
							if (
								Base.getBSONSize(tempMessages) >
								Base.getMaxBSONSize()
							) {
								const tmp = Base.getBSONSafeArraysFromAnArray(
									tempMessages
								);
							...
							this.logger.warn(
								`${entry.entryName} is not a valid JSON file! Unable to import it.`
							);
						}
					}
				} catch (e) {
		...
		if (
			...
			this.logger.warn(
				`Loaded ${tempUsers.length} users, ${tempChannels.length} channels, ${tempGroups.length} groups, ${tempDMs.length} DMs, ${tempMpims.length} multi party IMs and ${messagesCount} messages`
			);
			super.updateProgress(ProgressStep.ERROR);
			return this.getProgress();
		}

		const selectionUsers = tempUsers.map(
			user =>
				new SelectionUser(
					user.id,
					user.name,
		...
		const selectionChannels = tempChannels.map(
			channel =>
		...
		const selectionGroups = tempGroups.map(
			channel =>
				new SelectionChannel(
					channel.id,
					channel.name,
		...
		const selectionMpims = tempMpims.map(
			channel =>
				new SelectionChannel(
					channel.id,
					channel.name,
		...
		return new Selection(
			this.name,
			selectionUsers,
			selectionChannels.concat(selectionGroups).concat(selectionMpims),
			selectionMessages
		);
	}
	...
	performUserImport(user, startedByUserId) {
		...
		Meteor.runAsUser(startedByUserId, () => {
			const existantUser =
				Users.findOneByEmailAddress(user.profile.email) ||
				Users.findOneByUsernameIgnoringCase(user.name);
			if (existantUser) {
				user.rocketId = existantUser._id;
				Users.update(
					{ _id: user.rocketId },
					{ $addToSet: { importIds: user.id } }
				);
				this._saveUserIdReference(
					user.id,
					existantUser._id,
					user.name,
					existantUser.username
				);
			} else {
				const userId = user.profile.email
					? Accounts.createUser({
							...
							password:
								Date.now() +
								user.name +
								user.profile.email.toUpperCase()
					  })
					: Accounts.createUser({
							username: user.name,
				...
				Meteor.runAsUser(userId, () => {
					...
					const url =
						user.profile.image_original || user.profile.image_512;
					try {
						Meteor.call(
						...
						this.logger.warn(
							`Failed to set ${user.name}'s avatar from url ${url}`
						);
						console.log(
							`Failed to set ${user.name}'s avatar from url ${url}`
						);
	...
	parseMentions(message) {
		...
		users.forEach((user_id, index, arr) => {
			...
			try {
				...
				this.logger.warn(
					`Failed to import user mention with name: ${user}`
				);
			}
		});
		...
		channels.forEach((channel_name, index, arr) => {
			...
			try {
				...
				this.logger.warn(
					`Failed to import channel mention with name: ${chan}`
				);
			}
		});
	...
	processMessageSubType(message, room, msgDataDefaults, missedTypes) {
		...
		switch (message.subtype) {
			...
			case 'me_message': {
				const msgObj = {
					...msgDataDefaults,
					msg: `_${this.convertSlackMessageToRocketChat(
						message.text
					)}_`
				};
			...
			case 'slackbot_response': {
				...
				const botUsername = this.bots[message.bot_id]
					? this.bots[message.bot_id].name
					: message.username;
				const msgObj = {
					...msgDataDefaults,
					...
					rid: room._id,
					bot: true,
					attachments: this.convertMessageAttachments(
						message.attachments
					),
				...
				if (message.edited) {
					msgObj.editedAt = new Date(
						parseInt(message.edited.ts.split('.')[0]) * 1000
					);
					const editedBy = this.getRocketUserFromUserId(
						message.edited.user
					);
					if (editedBy) {
						msgObj.editedBy = {
			...
			case 'pinned_item':
				if (message.attachments) {
					const msgObj = {
						...
						attachments: [
							{
								text: this.convertSlackMessageToRocketChat(
									message.attachments[0].text
								),
								author_name:
									message.attachments[0].author_subname,
					...
					this.logger.debug(
						'Pinned item with no attachment, needs work.'
					);
			...
			case 'file_share':
				if (
					message.file &&
					message.file.url_private_download !== undefined
				) {
					const details = {
					...
					this.uploadFile(
						details,
						message.file.url_private_download,
						rocketUser,
						room,
						new Date(parseInt(message.ts.split('.')[0]) * 1000)
					);
				}
				break;
			default:
				if (
					!missedTypes[message.subtype] &&
					!ignoreTypes[message.subtype]
				) {
			...
			message.reactions.forEach(reaction => {
				...
				if (reaction.users) {
					reaction.users.forEach(u => {
						...
						msgDataDefaults.reactions[reaction.name].usernames.push(
							rcUser.username
						);
					});
				}
		...
		if (message.type === 'message') {
			if (message.files) {
				...
				message.files.forEach(file => {
					fileIndex++;
					const msgObj = {
						_id: `slack-${slackChannel.id}-${message.ts.replace(
							/\./g,
							'-'
						)}-file${fileIndex}`,
			...
			if (message.subtype && message.subtype !== 'thread_broadcast') {
				...
				if (user) {
					...
					if (message.thread_ts) {
						if (message.thread_ts === message.ts) {
							if (message.reply_users) {
								...
								this.logger.warn(
									`Failed to import the parent comment, message: ${msgDataDefaults._id}. Missing replies/reply_users field`
								);
							}

							msgObj.tcount = message.reply_count;
							msgObj.tlm = new Date(
								parseInt(message.latest_reply.split('.')[0]) *
									1000
							);
						} else {
							msgObj.tmid = `slack-${
					...
					try {
						insertMessage(
							this.getRocketUserFromUserId(message.user),
							msgObj,
							room,
							this._anyExistingSlackMessage
						);
						...
						this.logger.warn(
							`Failed to import the message: ${msgDataDefaults._id}`
						);
					}
				}
			}
	...
	_importUsers(startedByUserId) {
		...
		this.users.users.forEach(user =>
			this.performUserImport(user, startedByUserId)
		);
	...
	_importChannels(startedByUserId, channelNames) {
			...
			Meteor.runAsUser(startedByUserId, () => {
				...
				if (existingRoom || channel.is_general) {
					if (
						channel.is_general &&
						existingRoom &&
						channel.name !== existingRoom.name
					) {
						Meteor.call(
							'saveRoomSettings',
							'GENERAL',
					...
					channel.rocketId = channel.is_general
						? 'GENERAL'
						: existingRoom._id;
					Rooms.update(
						{ _id: channel.rocketId },
						{ $addToSet: { importIds: channel.id } }
					);
				} else {
					const users = this._getChannelUserList(channel);
					const userId =
						this.getImportedRocketUserIdFromSlackUserId(
							channel.creator
						) || startedByUserId;
		...
		this.collection.update(
			...
			{ $set: { channels: this.channels.channels } }
		);
	}
	...
	_findExistingRoom(name) {
		...
		if (!existingRoom) {
			const newName = getValidRoomName(name, null, {
	...
	_updateImportedChannelTopicAndDescription(slackChannel) {
		...
		Rooms.update(
			{ _id: slackChannel.rocketId },
			{ $set: roomUpdate, $addToSet: { importIds: slackChannel.id } }
		);
	}
	...
	_importMpims(startedByUserId, channelNames) {
		...
		this.mpims.channels.forEach(channel => {
			...
			Meteor.runAsUser(startedByUserId, () => {
				...
				const existingRoom = Rooms.findOneDirectRoomContainingAllUserIDs(
					users,
					{ fields: { _id: 1 } }
				);
		...
		this.collection.update(
			{ _id: this.mpims._id },
			{ $set: { channels: this.mpims.channels } }
		);
	}
	...
	_importDMs(startedByUserId, channelNames) {
		...
		this.dms.channels.forEach(channel => {
			...
			Meteor.runAsUser(startedByUserId, () => {
				...
				if (existingRoom) {
					...
					if (!user1) {
						this.logger.error(
							`DM creation: User not found for id ${channel.members[0]} and channel id ${channel.id}`
						);
						return;
					}
					...
					if (!user2) {
						this.logger.error(
							`DM creation: User not found for id ${channel.members[1]} and channel id ${channel.id}`
						);
						return;
					}

					const roomInfo = Meteor.runAsUser(user1._id, () =>
						Meteor.call('createDirectMessage', user2.username)
					);
		...
		this.collection.update(
			{ _id: this.dms._id },
			{ $set: { channels: this.dms.channels } }
		);
	}
	...
	_importMessages(startedByUserId, channelNames) {
		...
		for (const channel of channelNames) {
			...
			const room = Rooms.findOneById(slackChannel.rocketId, {
			...
			const messagePacks = this.collection.find({
				import: this.importRecord._id,
				type: 'messages',
				channel
			});
			...
			Meteor.runAsUser(startedByUserId, () => {
				messagePacks.forEach(pack => {
					...
					this.updateRecord({
						messagesstatus: `${channel}/${packId} (${pack.messages.length})`
					});
					pack.messages.forEach(message =>
						this.performMessageImport(
							message,
							room,
							missedTypes,
							slackChannel
	...
	_applyChannelSelection(importSelection) {
		...
		if (this.groups && this.groups.channels) {
			this.collection.update(
				{ _id: this.groups._id },
				{ $set: { channels: this.groups.channels } }
			);
		}
	...
	startImport(importSelection) {
		const bots = this.collection.findOne({
			import: this.importRecord._id,
			type: 'bots'
		});
		if (bots) {
		...
		this.channels = RawImports.findOne({
			import: this.importRecord._id,
			type: 'channels'
		});
		this.groups = RawImports.findOne({
			import: this.importRecord._id,
			type: 'groups'
		});
		this.dms = RawImports.findOne({
			import: this.importRecord._id,
			type: 'DMs'
		});
		this.mpims = RawImports.findOne({
			import: this.importRecord._id,
			type: 'mpims'
		});

		this._userDataCache = {};
		this._anyExistingSlackMessage = Boolean(
			Messages.findOne({ _id: /slack\-.*/ })
		);
		this.reloadCount();
		...
		Meteor.defer(() => {
			try {
				...
				this.logger.log(
					`Import took ${Date.now() - start} milliseconds.`
				);
			} catch (e) {
				this.logger.error(e);
				super.updateProgress(ProgressStep.ERROR);
	...
	getSlackChannelFromName(channelName) {
		if (this.channels && this.channels.channels) {
			const channel = this.channels.channels.find(
		...
		if (this.groups && this.groups.channels) {
			const group = this.groups.channels.find(
				channel => channel.name === channelName
			);
			if (group) {
				return group;
		...
		if (this.mpims && this.mpims.channels) {
			const group = this.mpims.channels.find(
				channel => channel.name === channelName
			);
			if (group) {
				return group;
		...
		if (this.dms && this.dms.channels) {
			const dm = this.dms.channels.find(
