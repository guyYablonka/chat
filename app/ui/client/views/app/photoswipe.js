...
Meteor.startup(() => {
	...
	const initGallery = async (items, options) => {
		...
		const [PhotoSwipeImport, PhotoSwipeUI_DefaultImport] =
			await Promise.all([
				import('photoswipe'),
		...
		if (!currentGallery) {
			...
			currentGallery = await new PhotoSwipe(
	...
	const createEventListenerFor = className => event => {
		...
		const galleryOptions = {
			...
			addCaptionHTMLFn(item, captionEl) {
				captionEl.children[0].innerHTML = `${escapeHTML(
					item.title
		...
		const items = Array.from(document.querySelectorAll(className)).map(
			(element, i) => {
