...
const userCanDrop = _id =>
	...
	) && roomTypes.canSendMessage(_id);

const getFirstUnreadMessageByRoomId = rid => {
...
Template.roomOld.helpers({
	...
	messagesHistory() {
		...
		const hideSettings =
			settings.collection.findOne('Hide_System_Messages') || {};
	...
	messageboxData() {
		const { sendToBottomIfNecessaryDebounced, subscription } =
			Template.instance();
		const { _id: rid } = this;
		...
		return {
			...
			onResize: () =>
				sendToBottomIfNecessaryDebounced &&
				sendToBottomIfNecessaryDebounced(),
			onKeyUp: (...args) =>
				chatMessages[rid] &&
	...
	containerBarsShow(unreadData, uploading) {
		const hasUnreadData = !!(unreadData?.count && unreadData.since);
	...
	hideAvatar() {
		return getUserPreference(Meteor.userId(), 'hideAvatars')
			? 'hide-avatars'
			: undefined;
	},
	canPreview() {
...
export const dropzoneEvents = {
	'dragenter .dropzone'(e) {
		if (userCanDrop(this._id)) {
			...
			const isFileType =
				Array.isArray(types) &&
				types.some(type => !type.includes('text/'));
			...
			modal.open({
				title: t('error-cant-send-file'),
				text: t('error-cant-send-file-description'),
	...
	async 'dropped .dropzone-overlay'(event, instance) {
		...
		if (files.length < 1) {
			const transferData =
				e.dataTransfer.getData('text') || e.dataTransfer.getData('url');
...
Meteor.startup(() => {
	Template.roomOld.events({
		...
		'click .jump-recent'(e, template) {
			const { _id } = template?.data ?? {};
			e.preventDefault();

			callbacks.run('userClickedScrollToBottom');
			template.subscription.get().unread && readMessage.readNow();

			template.atBottom.set(true);
			if (_id && RoomHistoryManager.hasMoreNext(_id)) {
				RoomHistoryManager.clear(_id);
			} else {
		...
		'click .new-message'(event, instance) {
			callbacks.run('userClickedNewMessages');
			instance.atBottom.set(true);
			instance.sendToBottomIfNecessaryDebounced();
			instance.subscription.get().unread && readMessage.readNow();
			chatMessages[RoomManager.openedRoom].input.focus();
		},
		...
		'click .unread-bar > button.jump-to'(e, t) {
			...
			const message =
				room?.firstUnread?.get() || getFirstUnreadMessageByRoomId(_id);
			readMessage.enable();
			RoomHistoryManager.getSurroundingMessages(message, 50, false);
		},
		'scroll .wrapper': _.throttle(function (e, t) {
			...
			if (
				...
				if (hasMore === true && lastScrollTop <= height / 3) {
					...
					hasMoreNext === true &&
					Math.ceil(lastScrollTop) >= e.target.scrollHeight - height
				) {
		...
		'click .time a'(e) {
			...
			const repliedMessageId =
				msg.attachments[0].message_link.split('?msg=')[1];
	...
	Template.roomOld.onCreated(function () {
		...
		this.sendToBottomIfNecessaryDebounced = _.debounce(
			this.sendToBottomIfNecessary,
			10
		);
	}); // Update message to re-render DOM
	...
	Template.roomOld.onRendered(function () {
		...
		if (window.MutationObserver) {
			template.observer = new MutationObserver(() =>
				template.sendToBottomIfNecessaryDebounced()
			);
		...
		this.autorun(() => {
			const count =
				RoomHistoryManager.getRoom(rid).unreadNotLoaded.get() +
...
callbacks.add('enter-room', sub => {
	...
	const isAReplyInDMFromChannel =
		FlowRouter.getQueryParam('reply') && sub.t === 'd';
