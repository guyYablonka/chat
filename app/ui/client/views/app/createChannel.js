...
const filterChannelName = old => {
	if (settings.get('UI_Allow_room_names_with_special_chars') || !old.length) {
		return old;
	}

	const reg = new RegExp(settings.get('UTF8_Channel_Names_Validation'));

	return old.match(reg)?.[0] ?? Template.instance().name.get();
};
...
Template.createChannel.helpers({
	...
	roomTypesBeforeStandard() {
		const orderLow = roomTypes.roomTypesOrder.filter(
			roomTypeOrder => roomTypeOrder.identifier === 'c'
		)[0].order;
		return roomTypes.roomTypesOrder
			.filter(roomTypeOrder => roomTypeOrder.order < orderLow)
			.map(roomTypeOrder => roomTypes.getConfig(roomTypeOrder.identifier))
			.filter(roomType => roomType.creationTemplate);
	},
	roomTypesAfterStandard() {
		const orderHigh = roomTypes.roomTypesOrder.filter(
...
Template.createChannel.events({
	...
	'input [name="name"]'(e, t) {
		...
		const position = input.selectionEnd || input.selectionStart;
		const { length } = input.value;
		const modified = filterChannelName(input.value);
			...
			function (err, result) {
				if (err) {
					...
					if (err.error === 'error-invalid-room-name') {
						toastr.error(
							t('error-invalid-room-name', { room_name: name })
						);
						return;
					}
				...
				if (!isPrivate) {
					callbacks.run('aftercreateCombined', {
						_id: result.rid,
						name: result.name
					});
				}
...
Template.createChannel.onCreated(function () {
	...
	this.encrypted = new ReactiveVar(
...
Template.tokenpass.onCreated(function () {
	this.data.validations.tokenpass = instance => {
		const result =
			settings.get('API_Tokenpass_URL') !== '' &&
...
Template.tokenpass.events({
	...
	'click .rc-tags__tag'({ target }, t) {
		...
		t.selectedTokens.set(
			t.selectedTokens.get().filter(t => t.token !== token)
		);
		t.data.change();
	},
