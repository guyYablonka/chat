...
const showModal = config =>
	new Promise((resolve, reject) => modal.open(config, resolve, reject));
...
export class ChatMessages {
	...
	async restoreReplies() {
		...
		const message =
			Messages.findOne(mid) || (await call('getSingleMessage', mid));
		if (!message) {
			return;
		}
	...
	edit(element, isEditingTheNextOne) {
		...
		const editOwn =
			message && message.u && message.u._id === Meteor.userId();
	...
	async send(event, { rid, tmid, value, tshow }, done = () => {}) {
		...
		const originalMessage = ChatMessage.findOne(
			{ _id: this.editing.id },
	...
	async processTooLongMessage({ msg, rid, tmid }) {
		const tempMessage =
			messageProperties.messageWithoutEmojiShortnames(msg);
	...
	confirmDeleteMsg(message, done = () => {}) {
		...
		modal.open(
			...
			() => {
				modal.open({
					title: t('Deleted'),
					text: t('Your_message_has_been_deleted'),
					type: 'success',
					timer: 1000,
	...
	async deleteMsg({ _id, rid, ts, starred }) {
		...
		await call('deleteMessage', { _id, starred });
	}
