	...
	if (Meteor.status().status === 'waiting') {
		retryHandle =
			...
			setInterval(function () {
				const timeDiff =
					Meteor.status().retryTime - new Date().getTime();
				const _retryTime =
					(timeDiff > 0 && Math.round(timeDiff / 1000)) || 0;
...
Template.status.helpers({
	...
	extraMessage() {
		if (Meteor.status().status === 'waiting') {
			return TAPi18n.__('meteor_status_reconnect_in', {
				count: retryTime.get()
			});
		}
	},
	...
	reconnectLabel() {
		return TAPi18n.__('meteor_status_try_now', {
			context: Meteor.status().status
		});
	}
});
