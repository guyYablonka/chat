...
export const getRoomData = (roomId, ownUserId) => {
	...
	const roomName =
		roomData.name && roomData.t !== 'd' ? roomData.name : roomId;
	const [userId] =
		roomData.t === 'd'
	...
	if (msg.t) {
		...
		switch (msg.t) {
			...
			case 'livechat-close':
				...
				break;
			case 'cd':
				messageObject.msg = TAPi18n.__('room_changed_description', {
					room_description: msg.msg,
					user_by: username
				});
				break;
			case 'ct':
				messageObject.msg = TAPi18n.__('room_changed_topic', {
					room_topic: msg.msg,
					user_by: username
				});
				break;
			case 'ca':
				messageObject.msg = TAPi18n.__('room_changed_announcement', {
					room_announcement: msg.msg,
					user_by: username
				});
				break;
		}
...
async function processDataDownloads() {
	...
	if (operation.status === 'completed') {
		const file = operation.fileId
			? UserDataFiles.findOneById(operation.fileId)
			: UserDataFiles.findLastFileByUser(operation.userId);
		if (!file) {
			return;
		}
