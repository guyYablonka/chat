...
Template.videoFlexTab.onRendered(function () {
	...
	const update = async () => {
		const { jitsiTimeout } = Rooms.findOne(
			{ _id: rid },
			{ fields: { jitsiTimeout: 1 } }
		);

		if (
			jitsiTimeout &&
			TimeSync.serverTime() - new Date(jitsiTimeout) + CONSTANTS.TIMEOUT <
				CONSTANTS.DEBOUNCE
		) {
			return;
		}
	...
	modal.open(
		...
		dismiss => {
			...
			this.autorun(async () => {
				...
				const isEnabledTokenAuth = settings.get(
				...
				const accessToken =
					isEnabledTokenAuth &&
					(await call('jitsi:generateAccessToken', rid));
				...
				if (settings.get('Jitsi_Open_New_Window')) {
					return Tracker.nonreactive(async () => {
						...
						const queryString = accessToken
							? `?jwt=${accessToken}`
							: '';

						const newWindow = window.open(
							`${
								(noSsl ? 'http://' : 'https://') + domain
							}/${jitsiRoom}${queryString}`,
							jitsiRoom
						);
						if (newWindow) {
				...
				await new Promise(resolve => {
					if (typeof JitsiMeetExternalAPI === 'undefined') {
						const prefix =
							...
							'';
						return $.getScript(
							`${prefix}/packages/rocketchat_videobridge/client/public/external_api.js`,
							resolve
						);
					}
					resolve();
				...
				if (typeof JitsiMeetExternalAPI !== 'undefined') {
					...
					const name = Users.findOne(Meteor.userId(), {
						fields: { name: 1 }
					});
					if (!$('[id^=jitsiConference]').length) {
						Tracker.nonreactive(async () => {
							...
							setTimeout(
								() =>
									this.api.executeCommand('displayName', [
										name
									]),
								5000
							);
