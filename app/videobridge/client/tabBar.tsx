...
import {
    addAction,
    ToolboxActionConfig
} from '../../../client/views/room/lib/Toolbox';
...
addAction('bbb_video', ({ room }) => {
    ...
    const live =
        room && room.streamingOptions && room.streamingOptions.type === 'call';
        ...
        () =>
            enabled
                ? {
                      ...
                      order: live ? -1 : 0,
                      renderAction: (props): React.ReactNode => (
...
addAction('video', ({ room }) => {
    ...
    const groups = useStableArray(
        ['direct', 'group', 'live', enabledChannel && 'channel'].filter(
            Boolean
