...
export function process2faReturn({
	...
	if (
		!error ||
		(error.error !== 'totp-required' && error.errorType !== 'totp-required')
	) {
	...
	modal.open(
		{
			...
			inputActionText:
				method === 'email' &&
				emailOrUsername &&
				t('Send_me_the_code_again'),
			async inputAction(e) {
				const { value } = e.currentTarget;
				e.currentTarget.value = t('Sending');

				await APIClient.v1.post('users.2fa.sendEmailCode', {
					emailOrUsername
				});
		...
		process2faReturn({
			...
			onCode: (code, method) => {
				call(
					ddpMethod,
					...args,
					{ twoFactorCode: code, twoFactorMethod: method },
					(error, result) => {
