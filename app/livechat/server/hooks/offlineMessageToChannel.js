...
callbacks.add(
	...
	data => {
		...
		const room = Rooms.findOneByName(channelName, {
			fields: { t: 1, archived: 1 }
		});
		...
		let msg = `${TAPi18n.__('New_Livechat_offline_message_has_been_sent', {
			lng
		})}: \n`;
		if (host && host !== '') {
		...
		msg = msg.concat(
			`${TAPi18n.__('Visitor_Email', { lng })}: ${email} \n`
		);
		if (departmentName) {
			msg = msg.concat(
				`${TAPi18n.__('Department', { lng })}: ${departmentName} \n`
			);
		}
