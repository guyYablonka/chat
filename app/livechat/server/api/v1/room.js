	...
	get() {
		...
		try {
			...
			const room = Promise.await(
				getRoom({ guest, rid, agent, extraParams })
			);
	...
	post() {
		try {
			...
			for (const item of data) {
				if (
					(config.survey.items.includes(item.name) &&
						config.survey.values.includes(item.value)) ||
		...
		post() {
			API.v1.success(
				Meteor.runAsUser(this.userId, () =>
					Meteor.call('livechat:transfer', this.bodyParams)
				)
			);
		}
	}
