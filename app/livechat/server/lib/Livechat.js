...
export const Livechat = {
	...
	getRequiredDepartment(onlineRequired = true) {
		...
		return departments.fetch().find(dept => {
			...
			const onlineAgents =
				LivechatDepartmentAgents.getOnlineForDepartment(dept._id);
			return onlineAgents && onlineAgents.count() > 0;
		});
	},
	...
	updateMessage({ guest, message }) {
		...
		const editOwn =
			originalMessage.u && originalMessage.u._id === guest._id;
	...
	registerGuest({
		...
		if (user) {
			...
			if (
				s.trim(email) !== '' &&
				(existingUser =
					LivechatVisitors.findOneGuestByEmailAddress(email))
			) {
		...
		if (phone) {
			updateUser.$set.phone = [{ phoneNumber: phone.number }];
		}
	...
	saveGuest({ _id, name, email, phone, livechatData = {} }, userId) {
		...
		if (
			!userId ||
			hasPermission(userId, 'edit-livechat-room-customfields')
		) {
	...
	closeRoom({ user, visitor, room, comment, options = {} }) {
		...
		const serviceTimeDuration =
			servedBy && (now.getTime() - servedBy.ts) / 1000;
	...
	savePageHistory(token, roomId, pageInfo) {
		if (pageInfo.change === Livechat.historyMonitorType) {
			...
			if (!roomId) {
				...
				extraData.expireAt =
					new Date().getTime() + keepHistoryMiliseconds;
			}
		...
		try {
			...
			if (secretToken !== '' && secretToken !== undefined) {
				Object.assign(options, {
					headers: { 'X-RocketChat-Livechat-Token': secretToken }
				});
			}
			return HTTP.post(settings.get('Livechat_webhookUrl'), options);
		} catch (e) {
			Livechat.logger.webhook.error(
				`Response error on ${trying} try ->`,
				e
			);
			// try 10 times after 10 seconds each
			if (trying < 10) {
				Livechat.logger.webhook.warn(
	...
	saveDepartment(_id, departmentData, departmentAgents) {
		...
		Object.keys(departmentData).forEach(field => {
			if (!defaultValidations.hasOwnProperty(field)) {
				defaultValidations[field] = Match.OneOf(
					String,
					Match.Integer,
					Boolean
				);
	...
	sendTranscript({ token, rid, email, subject, user }) {
		...
		const userLanguage =
			(visitor && visitor.language) || settings.get('Language') || 'en';
		...
		messages.forEach(message => {
			...
			const datetime = moment(message.ts)
				.locale(userLanguage)
				.format('LLL');
			const singleMessage = `
		...
		const mailSubject =
			subject ||
			TAPi18n.__('Transcript_of_your_livechat_conversation', {
				lng: userLanguage
			});
	...
	sendOfflineMessage(data = {}) {
		...
		const subject = `Livechat offline message from ${name}: ${`${emailMessage}`.substring(
			0,
			20
		)}`;
