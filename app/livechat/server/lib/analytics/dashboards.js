...
const getProductivityMetricsAsync = async ({
	...
	const sumOfWaitingTime = averageWaitingTime.departments.reduce(
		(acc, serviceTime) => {
	...
	const totalOfAvarageWaitingTime =
		totalOfWaitingTime === 0 ? 0 : sumOfWaitingTime / totalOfWaitingTime;
...
const getAgentsProductivityMetricsAsync = async ({
	...
	const totalOfAverageAvailableServiceTime = averageOfAvailableServiceTime
		...
		: 0;
	const totalOfAverageServiceTime =
		totalOfServiceTime === 0 ? 0 : sumOfServiceTime / totalOfServiceTime;
...
const getChatsMetricsAsync = async ({
	...
	const totalOfChatDurationTime =
		averageOfChatDurationTime.departments.length;
	...
	const totalOfAverageAbandonedRooms =
		totalOfAbandonedRooms === 0
			? 0
			: sumOfPercentageOfAbandonedRooms / totalOfAbandonedRooms;
	const totalOfAverageChatDurationTime =
		totalOfChatDurationTime === 0
			? 0
			: sumOfChatDurationTime / totalOfChatDurationTime;
...
const getConversationsMetricsAsync = async ({ start, end, departmentId }) => {
	if (!start || !end) {
		throw new Error('"start" and "end" must be provided');
	}
	const totalizers = Livechat.Analytics.getAnalyticsOverviewData({
...
const findAllAgentsStatusAsync = async ({ departmentId = undefined }) =>
...
const findAllResponseTimeMetricsAsync = async ({
	...
	const responseTimes = (
		await LivechatRooms.calculateResponseTimingsBetweenDates({
			start,
			end,
			departmentId
		})
	...
	const reactionTimes = (
		await LivechatRooms.calculateReactionTimingsBetweenDates({
			start,
			end,
			departmentId
		})
	...
	const durationTimings = (
		await LivechatRooms.calculateDurationTimingsBetweenDates({
			start,
			end,
			departmentId
		})
...
export const getProductivityMetrics = ({
	...
	departmentId = undefined
}) => Promise.await(getProductivityMetricsAsync({ start, end, departmentId }));
export const getAgentsProductivityMetrics = ({
	...
	Promise.await(
		getAgentsProductivityMetricsAsync({ start, end, departmentId })
	);
export const getConversationsMetrics = ({
	start,
	end,
	departmentId = undefined
}) => Promise.await(getConversationsMetricsAsync({ start, end, departmentId }));
...
export const findAllChatMetricsByDepartment = ({
	...
	Promise.await(
		findAllChatMetricsByDepartmentAsync({ start, end, departmentId })
	);
export const findAllResponseTimeMetrics = ({
	...
	Promise.await(
		findAllResponseTimeMetricsAsync({ start, end, departmentId })
	);
export const getChatsMetrics = ({ start, end, departmentId = undefined }) =>
	Promise.await(getChatsMetricsAsync({ start, end, departmentId }));
export const findAllAgentsStatus = ({ departmentId = undefined }) =>
	Promise.await(findAllAgentsStatusAsync({ departmentId }));
