...
export const Analytics = {
	getAgentOverviewData(options) {
		...
		if (!this.AgentOverviewData[options.chartOptions.name]) {
			console.log(
				`Method RocketChat.Livechat.Analytics.AgentOverviewData.${options.chartOptions.name} does NOT exist`
			);
			return;
		}
		...
		return this.AgentOverviewData[options.chartOptions.name](
			from,
			to,
			departmentId
		);
	...
	getAnalyticsChartData(options) {
		...
		if (!this.ChartData[options.chartOptions.name]) {
			console.log(
				`Method RocketChat.Livechat.Analytics.ChartData.${options.chartOptions.name} does NOT exist`
			);
			return;
		}
		...
		if (from.diff(to) === 0) {
			...
			for (
				...
				data.dataLabels.push(
					`${moment(hour, ['H']).format('hA')}-${moment(
						(parseInt(hour) + 1) % 24,
						['H']
					).format('hA')}`
				);
				...
				data.dataPoints.push(
					this.ChartData[options.chartOptions.name](
						date,
						departmentId
					)
				);
	...
	getAnalyticsOverviewData(options) {
		...
		if (!this.OverviewData[options.analyticsOptions.name]) {
			console.log(
				`Method RocketChat.Livechat.Analytics.OverviewData.${options.analyticsOptions.name} does NOT exist`
			);
			return;
		}

		return this.OverviewData[options.analyticsOptions.name](
			from,
			to,
			departmentId
		);
	...
	ChartData: {
		...
		Best_first_response_time(date, departmentId) {
			...
			LivechatRooms.getAnalyticsMetricsBetweenDate('l', date, {
				...
				if (metrics && metrics.response && metrics.response.ft) {
					maxFrt = maxFrt
						? Math.min(maxFrt, metrics.response.ft)
						: metrics.response.ft;
				}
			});
	...
	OverviewData: {
		...
		Conversations(from, to, departmentId) {
			...
			const summarize = m => ({ metrics, msgs }) => {
				...
				totalMessagesOnWeekday.set(
					weekday,
					totalMessagesOnWeekday.has(weekday)
						? totalMessagesOnWeekday.get(weekday) + msgs
						: msgs
				);
			};
			...
			for (
				...
				const result = Promise.await(
					LivechatRooms.getAnalyticsBetweenDate(date, {
						departmentId
					}).toArray()
				);
			...
			for (
				...
				for (
					...
					Promise.await(
						...
						totalMessagesInHour.set(
							dayHour,
							totalMessagesInHour.has(dayHour)
								? totalMessagesInHour.get(dayHour) + msgs
								: msgs
						);
					});
				}
			...
			const busiestHour = this.getKeyHavingMaxValue(
				totalMessagesInHour,
				-1
			);
			...
			const data = [
				...
				},
				{
					...
					value:
						busiestHour > 0
							? `${moment(busiestHour, ['H']).format(
									'hA'
							  )}-${moment((parseInt(busiestHour) + 1) % 24, [
									'H'
							  ]).format('hA')}`
							: '-'
				}
				...
				if (servedBy && metrics && metrics.chatDuration) {
					if (agentChatDurations.has(servedBy.username)) {
						agentChatDurations.set(servedBy.username, {
							chatDuration:
								agentChatDurations.get(servedBy.username)
				...
				if (
					...
					if (agentAvgRespTime.has(servedBy.username)) {
						agentAvgRespTime.set(servedBy.username, {
							frt:
								agentAvgRespTime.get(servedBy.username).frt +
								metrics.response.ft,
							total:
								agentAvgRespTime.get(servedBy.username).total +
								1
						});
					} else {
						agentAvgRespTime.set(servedBy.username, {
			...
			LivechatRooms.getAnalyticsMetricsBetweenDate('l', date, {
				...
				if (
					...
					if (agentFirstRespTime.has(servedBy.username)) {
						agentFirstRespTime.set(
							servedBy.username,
							Math.min(
								agentFirstRespTime.get(servedBy.username),
								metrics.response.ft
							)
						);
					} else {
			...
			LivechatRooms.getAnalyticsMetricsBetweenDate('l', date, {
				...
				if (
					...
					if (agentAvgRespTime.has(servedBy.username)) {
						agentAvgRespTime.set(servedBy.username, {
							avg:
								agentAvgRespTime.get(servedBy.username).avg +
								metrics.response.avg,
							total:
								agentAvgRespTime.get(servedBy.username).total +
								1
				...
				if (
					...
					if (agentAvgReactionTime.has(servedBy.username)) {
						agentAvgReactionTime.set(servedBy.username, {
							frt:
								agentAvgReactionTime.get(servedBy.username)
