...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			if (department) {
				const departmentFromDB = LivechatDepartment.findOneByIdOrName(
					department
				);
				if (departmentFromDB) {
		...
		post() {
			try {
				...
				if (
					...
					!Users.findOneById(this.bodyParams.userId, {
						fields: { _id: 1 }
					})
				) {
