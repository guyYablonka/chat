...
const getInquiriesFromAPI = async () => {
	const { inquiries } = await APIClient.v1.get(
		'livechat/inquiries.queued?sort={"ts": 1}'
	);
	return inquiries;
};

const removeListenerOfDepartment = departmentId => {
	inquiryDataStream.removeListener(
...
const addListenerForeachDepartment = async (departments = []) => {
	const cleanupFunctions = departments.map(department =>
		appendListenerToDepartment(department)
	);
