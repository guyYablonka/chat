...
Migrations._collection = new Mongo.Collection(
	Migrations.options.collectionName
);
...
function makeABox(message, color = 'red') {
	...
	const len =
		_(message).reduce(function (memo, msg) {
			return Math.max(memo, msg.length);
		}, 0) + 4;
	const text = message
		.map(msg => '|'[color] + s.lrpad(msg, len)[color] + '|'[color])
		.join('\n');
...
Migrations.migrateTo = function (command) {
	...
	for (let attempts = 1; attempts <= maxAttempts; attempts++) {
		if (version === 'latest') {
			...
			migrated = this._migrateTo(
				parseInt(version),
				subcommands.includes('rerun')
			);
		}
		if (migrated) {
			break;
			...
			console.log(
				`Not migrating, control is locked. Attempt ${attempts}/${maxAttempts}.${willRetry}`
					.yellow
			);
		}
	}
		...
		console.log(
			makeABox([
				...
				`Database target version: ${
					version === 'latest' ? _.last(this._list).version : version
				}`,
				'',
...
Migrations._migrateTo = function (version, rerun) {
	...
	log.info(
		`Migrating from version ${this._list[startIdx].version} -> ${this._list[endIdx].version}`
	);
	...
	function lock() {
		const date = new Date();
		const dateMinusInterval = moment(date)
			.subtract(self.options.lockExpiration, 'minutes')
			.toDate();
