	...
	insertOrUpdateUserStatus(userStatusData) {
		...
		if (!s.trim(userStatusData.name)) {
			throw new Meteor.Error(
				...
				{ method: 'insertOrUpdateUserStatus', field: 'Name' }
			);
		}
		...
		if (userStatusData._id) {
			matchingResults = CustomUserStatus.findByNameExceptId(
				userStatusData.name,
				userStatusData._id
			).fetch();
		} else {
			matchingResults = CustomUserStatus.findByName(
				userStatusData.name
			).fetch();
		}
		...
		if (
			userStatusData.statusType &&
			validStatusTypes.indexOf(userStatusData.statusType) < 0
		) {
			throw new Meteor.Error(
				'error-input-is-not-a-valid-field',
		...
		if (userStatusData.statusType !== userStatusData.previousStatusType) {
			CustomUserStatus.setStatusType(
				userStatusData._id,
				userStatusData.statusType
			);
		}
