...
export class CsvImporter extends Base {
	...
	prepareUsingLocalFile(fullFilePath) {
		...
		zip.forEach(entry => {
			...
			if (entry.entryName.indexOf('/') > -1) {
				...
				if (Base.getBSONSize(data) > Base.getMaxBSONSize()) {
					Base.getBSONSafeArraysFromAnArray(data).forEach(
						(splitMsg, i) => {
		...
		const selectionUsers = tempUsers.map(
			u =>
				new SelectionUser(u.id, u.username, u.email, false, false, true)
		);
	...
	startImport(importSelection) {
		...
		Meteor.defer(() => {
			...
			try {
				...
				for (const u of this.users.users) {
					...
					Meteor.runAsUser(startedByUserId, () => {
						...
						if (!existantUser) {
							existantUser = Users.findOneByUsernameIgnoringCase(
								u.username
							);
						}
				...
				if (this.channels.channels.length === 0) {
					const channelNames = distinct('channel', {
						import: this.importRecord._id,
						type: 'messages',
						channel: { $ne: 'directMessages' }
					});
				...
				if (this.users.users.length === 0) {
					const usernames = distinct('messages.username', {
						import: this.importRecord._id,
						type: 'messages'
					});
				...
				messagePacks.forEach(pack => {
					...
					if (csvChannel.isDirect) {
						this._importDirectMessagesFile(
							msgGroupData,
							pack,
							startedByUserId
						);
					...
					const room = Rooms.findOneById(csvChannel.rocketId, {
					...
					Meteor.runAsUser(startedByUserId, () => {
						super.updateRecord({
							messagesstatus: `${ch}/${msgGroupData}.${pack.messages.length}`
						});
						for (const msg of pack.messages) {
							if (isNaN(new Date(parseInt(msg.ts)))) {
								this.logger.warn(
									`Timestamp on a message in ${ch}/${msgGroupData} is invalid`
								);
								super.addCountCompleted(1);
								continue;
							}

							const creator = this.getUserFromUsername(
								msg.username
							);
							if (creator) {
								let suffix = '';
			...
			this.logger.info(`CSV Import took ${timeTook} milliseconds.`);
		});
	...
	_importDirectMessagesFile(msgGroupData, msgs, startedByUserId) {
		...
		Meteor.runAsUser(startedByUserId, () => {
			...
			let rid;
			super.updateRecord({
				messagesstatus: `${t('Direct_Messagest')}/${msgGroupData}.${
			...
			for (const msg of msgs.messages) {
				if (isNaN(new Date(parseInt(msg.ts)))) {
					this.logger.warn(
						`Timestamp on a message in ${t(
							'Direct_Messagest'
						)}/${msgGroupData} is invalid`
					);
					super.addCountCompleted(1);
					continue;
				}
				...
				if (creator && targetUser) {
					if (!rid) {
						const roomInfo = Meteor.runAsUser(creator._id, () =>
							Meteor.call(
								'createDirectMessage',
								targetUser.username
							)
					...
					if (!room) {
						this.logger.warn(
							`DM room not found for users ${msg.username} and ${msg.otherUsername}`
						);
						super.addCountCompleted(1);
						continue;
					}
	...
	getUserFromUsername(username) {
		for (const u of this.users.users) {
			if (u.username === username) {
				return Users.findOneById(u.rocketId, {
					fields: { username: 1 }
				});
			}
		}
