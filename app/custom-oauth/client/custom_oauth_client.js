...
export class CustomOAuth {
	...
	async overrideOptionsByEnv() {
		const webAuthName = 'oidc';

		if (this.name.toLowerCase() === webAuthName) {
			const { serverURL: newServerUrl } = await APIClient.v1.get(
				'oidcUrl'
			);

			this.authorizePath = this.authorizePath.replace(
				this.serverURL,
				newServerUrl
			);
	...
	async requestCredential(options, credentialRequestCompleteCallback) {
		...
		await this.overrideOptionsByEnv();

		const separator = this.authorizePath.indexOf('?') !== -1 ? '&' : '?';

		const loginUrl =
			`${this.authorizePath}${separator}client_id=${
				config.clientId
