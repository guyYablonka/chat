...
export const logger = new Logger('OAuth', {}, __filename);
...
export function mapSSOGroupsToChannels(
	user,
	identity,
	groupClaimName,
	channelsMap,
		...
		for (const ssoGroup in channelsMap) {
			if (typeof ssoGroup === 'string') {
				...
				for (const channel of channels) {
					...
					if (
						Array.isArray(groupsFromSSO) &&
						groupsFromSSO.includes(ssoGroup)
					) {
