...
export const theme = new (class {
	constructor() {
		...
		settings.onload(
			'css',
			Meteor.bindEnvironment((key, value, initialLoad) => {
				if (!initialLoad) {
		...
		Meteor.startup(() => {
			settings.onAfterInitialLoad(() => {
				settings.get(
					/^theme-./,
					Meteor.bindEnvironment((key, value) => {
	...
	compile() {
		...
		return less.render(content, options, function (err, data) {
			logger.info(`Theme: Stop Rendering ${Date.now() - start}`);
			if (err != null) {
...
Meteor.startup(() => {
	settings.get('css', (key, value = '') => {
		...
		injectIntoHead(
			'css-theme',
			`<link rel="stylesheet" type="text/css" href="${getURL(
				`/theme.css?${currentHash}`
			)}">`
		);
	});
});
