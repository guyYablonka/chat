...
class GoogleVision {
	constructor() {
		...
		settings.get('GoogleVision_Block_Adult_Images', (key, value) => {
			if (value) {
				callbacks.add(
					'beforeSaveMessage',
					this.blockUnsafeImages.bind(this),
					callbacks.priority.MEDIUM,
					'googlevision-blockunsafe'
				);
			} else {
				callbacks.remove(
					'beforeSaveMessage',
	...
	incCallCount(count) {
		...
		const maxMonthlyCalls =
			settings.get('GoogleVision_Max_Monthly_Calls') || 0;
		if (maxMonthlyCalls > 0) {
			if (settings.get('GoogleVision_Current_Month') !== currentMonth) {
				...
				count +
					(settings.get('GoogleVision_Current_Month_Calls') || 0) >
				maxMonthlyCalls
			) {
				return false;
			}
				...
				if (this.incCallCount(1)) {
					...
					if (results && results.adult === true) {
						...
						throw new Meteor.Error(
							'GoogleVisionError: Image blocked'
						);
					}
				} else {
		...
		if (
			...
			if (
				...
				if (this.incCallCount(visionTypes.length)) {
					...
					const bucketFile = bucket.file(file.GoogleStorage.path);
					this.visionClient.detect(
						bucketFile,
						visionTypes,
						Meteor.bindEnvironment((error, results) => {
							if (!error) {
								Messages.setGoogleVisionData(
									message._id,
									this.getAnnotations(visionTypes, results)
								);
							} else {
								console.trace(
									'GoogleVision error: ',
		...
		for (const index in visionData) {
			if (visionData.hasOwnProperty(index)) {
				switch (index) {
					...
					case 'similar':
					case 'logos':
						results[index] = (results[index] || []).concat(
							visionData[index] || []
						);
						break;
					case 'safeSearch':
