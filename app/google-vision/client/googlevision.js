const getVisionAttributes = attachment => {
	...
	if (
		attachment.safeSearch &&
		attachment.safeSearch &&
		attachment.safeSearch.adult === true
	) {
	...
	if (attachment.faces && attachment.faces.length > 0) {
		...
		attachment.faces.forEach(face => {
			...
			if (faceAttributes.length > 0) {
				labels.push({
					label: `Face ${++faceCount}: ${faceAttributes.join(', ')}`
				});
			}
		});
	}
...
export const createGoogleVisionMessageRenderer = () => message => {
...
export const createGoogleVisionMessageStreamHandler = () => message => {
	if (!message.attachments?.length) {
		return message;
	}
