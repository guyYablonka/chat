...
Template.CreateDiscussion.helpers({
	...
	createIsDisabled() {
		...
		return parentChannel.get() && discussionName.get().trim()
			? ''
			: 'disabled';
	},
	parentChannel() {
...
Template.CreateDiscussion.events({
	...
	async 'submit #create-discussion, click .js-save-discussion'(
		event,
		instance
	) {
		...
		if (!prid) {
			const errorText = TAPi18n.__(
				'Invalid_room_name',
				`${parentChannel}...`
			);
			return toastr.error(errorText);
		}
		const result = await call('createDiscussion', {
			prid,
			pmid,
			t_name,
			users,
...
Template.CreateDiscussion.onCreated(function () {
	...
	const room =
		parentRoom && parentRoom.prid
			? ChatSubscription.findOne({ rid: parentRoom.prid })
			: parentRoom;
	...
	this.onClickTagUser = ({ username }) => {
		this.selectedUsers.set(
			this.selectedUsers.get().filter(user => user.username !== username)
		);
	};
	...
	const { parentChannel, reply } =
		callbacks.run('openDiscussionCreationScreen') || {};
