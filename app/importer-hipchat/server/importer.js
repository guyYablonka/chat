...
export class HipChatImporter extends Base {
	...
	prepare(dataURI, sentContentType, fileName, skipTypeCheck) {
		...
		zipEntries.forEach(entry => {
			...
			if (entry.entryName.indexOf(this.roomPrefix) > -1) {
				...
				if (roomName === 'list.json') {
					...
					try {
						tempMessages[roomName][msgGroupData] = JSON.parse(
				...
				return this.logger.warn(
					`Unexpected file in the ${this.name} import: ${entry.entryName}`
				);
			}
		});
		...
		Object.keys(tempMessages).forEach(channel => {
			...
			Object.keys(messagesObj).forEach(date => {
				...
				if (Base.getBSONSize(msgs) > Base.getMaxBSONSize()) {
					Base.getBSONSafeArraysFromAnArray(msgs).forEach(
						(splitMsg, i) => {
		...
		if (
			...
			this.logger.warn(
				`The loaded users count ${tempUsers.length}, the loaded channels ${tempRooms.length}, and the loaded messages ${messagesCount}`
			);
			...
			try {
				this.users.users.forEach(user => {
					...
					Meteor.runAsUser(startedByUserId, () => {
						const existantUser = Users.findOneByEmailAddress(
							user.email
						);
						if (existantUser) {
				...
				this.channels.channels.forEach(channel => {
					...
					Meteor.runAsUser(startedByUserId, () => {
						...
						if (existantRoom) {
							...
							if (userId === '') {
								this.logger.warn(
									`Failed to find the channel creator for ${channel.name}, setting it to the current running user.`
								);
				...
				for (const channel of channelNames) {
					const hipchatChannel = this.getHipChatChannelFromName(
						channel
					);
						...
						messagePacks.forEach(pack => {
							...
							pack.messages.forEach(message => {
								if (message.from != null) {
									const user = this.getRocketUser(
										message.from.user_id
									);
									if (user != null) {
										const msgObj = {
											msg: this.convertHipChatMessageToRocketChat(
												message.message
											),
				...
				this.logger.warn('The following did not have users:', {
					nousers
				});
	...
	getHipChatChannelFromName(channelName) {
		return this.channels.channels.find(
	...
	getRocketUser(hipchatId) {
		...
		return user
			? Users.findOneById(user.rocketId, {
					fields: {
						username: 1,
						name: 1
					}
