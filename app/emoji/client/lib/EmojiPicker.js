...
export const EmojiPicker = {
	...
	setPosition() {
		...
		const { top, left } = $(this.source).offset();
		const windowHeight = window.innerHeight;
		const windowWidth = window.innerWidth;
		const horizontalPaddingDirection = isRtl ? 'right' : 'left';
		const horizontalPaddingSize = isRtl
			? windowWidth -
			  this.source.clientWidth -
			  left -
		...
		const cssProperties = {
			top,
			[horizontalPaddingDirection]: horizontalPaddingSize
		};
		...
		if (horizontalPaddingSize < windowBorder) {
			cssProperties[horizontalPaddingDirection] = isLargerThanWindow
				? 0
				: windowBorder;
		}

		if (horizontalPaddingSize + this.width >= windowWidth) {
			cssProperties[horizontalPaddingDirection] = isLargerThanWindow
				? 0
