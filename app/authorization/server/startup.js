...
Meteor.startup(function () {
	...
	const permissions = [
		...
		{ _id: 'add-user-to-any-p-room', roles: ['aman-proxy'] },
		...
		{ _id: 'bulk-register-user', roles: ['admin'] },
		{
			...
			roles: ['admin', 'livechat-manager', 'livechat-agent']
		},
		{ _id: 'create-c', roles: ['admin', 'user', 'bot', 'app'] },
		{ _id: 'create-d', roles: ['admin', 'user', 'bot', 'app'] },
		{
			_id: 'create-p',
			roles: ['admin', 'user', 'bot', 'app', 'aman-proxy']
		},
		...
		{
			_id: 'edit-room',
			roles: ['admin', 'owner', 'moderator', 'aman-proxy', 'aman-user']
		},
		...
		{
			_id: 'view-c-room',
			roles: ['admin', 'user', 'bot', 'app', 'anonymous']
		},
		{ _id: 'view-any-room', roles: ['admin'] },
		...
		{
			_id: 'view-p-room',
			roles: ['admin', 'user', 'anonymous', 'guest', 'aman-proxy']
		},
		...
		{ _id: 'use-as-user-routes', roles: ['admin', 'aman-proxy'] }
	];
	...
	const defaultRoles = [
		...
		},
		{
			name: 'aman-proxy',
			scope: 'Users',
			description: 'aman-proxy'
		},
		{
			name: 'aman-user',
			scope: 'Users',
			description: 'aman-user'
		}
	];
	...
	const createSettingPermission = function (
		...
		if (setting.group) {
			permission.groupPermissionId = getSettingPermissionId(
				setting.group
			);
		}
		if (setting.section) {
			permission.sectionPermissionId = getSettingPermissionId(
				setting.section
			);
		}
