...
export const canDeleteMessageAsync = async (uid, { u, rid, ts }) => {
	const forceDelete = await hasPermissionAsync(
		uid,
	...
	const allowedToDeleteAny = await hasPermissionAsync(
		uid,
		'delete-message',
		rid
	);
	...
	const allowed =
		allowedToDeleteAny ||
		(uid === u._id &&
			(await hasPermissionAsync(uid, 'delete-own-message')));
	if (!allowed) {
		return false;
	}
	const blockDeleteInMinutes = await getValue(
	...
	const room = await Rooms.findOneById(rid, {
		fields: { ro: 1, unmuted: 1 }
	});
	if (
		room.ro === true &&
		!(await hasPermissionAsync(uid, 'post-readonly', rid))
	) {
...
export const canDeleteMessage = (uid, { u, rid, ts }) =>
