...
export class HipChatEnterpriseImporter extends Base {
	...
	async storeTempMessages(
		...
		const name = subIndex
			? `${roomIdentifier}/${index}/${subIndex}`
			: `${roomIdentifier}/${index}`;
	...
	async storeUserTempMessages(tempMessages, roomIdentifier, index) {
		this.logger.debug(
			`dumping ${tempMessages.length} messages from room ${roomIdentifier} to database`
		);
	...
	async prepareUserMessagesFile(file, roomIdentifier, index) {
		...
		for (const m of file) {
			if (m.PrivateUserMessage) {
				...
				if (
					this.preparedMessages[m.PrivateUserMessage.id] !== undefined
				) {
					continue;
				}
				...
				const skipAttachment =
					skipMessage &&
					(m.PrivateUserMessage.attachment_path
						? this._checkIfMessageExists(`${newId}-attachment`)
						: true);
				...
				if (!skipMessage || !skipAttachment) {
					msgs.push({
						...
						text:
							m.PrivateUserMessage.message.indexOf('/me ') === -1
								? m.PrivateUserMessage.message
								: `${m.PrivateUserMessage.message.replace(
										/\/me /,
										'_'
								  )}_`,
						ts: new Date(
							m.PrivateUserMessage.timestamp.split(' ')[0]
						),
	...
	async loadExistingMessagesIfNecessary() {
		...
		if (!this._previewsMessagesIds) {
			...
			await Messages.model
				.rawCollection()
				.find({}, { fields: { _id: 1 } })
				.forEach(i => this._previewsMessagesIds.add(i._id));
		}
	}
	...
	async prepareRoomMessagesFile(file, roomIdentifier, id, index) {
			...
			if (m.UserMessage) {
				...
				if (!skipMessage || !skipAttachment) {
					roomMsgs.push({
						...
						text:
							m.UserMessage.message.indexOf('/me ') === -1
								? m.UserMessage.message
								: `${m.UserMessage.message.replace(
										/\/me /,
										'_'
								  )}_`,
				...
				const text =
					m.NotificationMessage.message.indexOf('/me ') === -1
						? m.NotificationMessage.message
						: `${m.NotificationMessage.message.replace(
								/\/me /,
								'_'
						  )}_`;
				...
				const skipAttachment =
					skipMessage &&
					(m.NotificationMessage.attachment_path
						? this._checkIfMessageExists(`${newId}-attachment`)
						: true);
				...
				if (!skipMessage || !skipAttachment) {
					roomMsgs.push({
						...
						text:
							m.NotificationMessage.message_format === 'html'
								? turndownService.turndown(text)
								: text,
						ts: new Date(
							m.NotificationMessage.timestamp.split(' ')[0]
						),
				...
				if (!skipMessage) {
					roomMsgs.push({
						...
						ts: new Date(
							m.TopicRoomMessage.timestamp.split(' ')[0]
						),
		...
		if (roomMsgs.length > 0) {
			await this.storeTempMessages(
				...
				roomIdentifier,
				index,
				subIndex > 0 ? subIndex + 1 : undefined,
				id
			);
		}
	...
	async prepareMessagesFile(file, info) {
		...
		switch (type) {
			case 'users':
				...
				return this.prepareUserMessagesFile(
					file,
					roomIdentifier,
					userMessageGroupIndex
				);
			case 'rooms':
				messageGroupIndex++;
				return this.prepareRoomMessagesFile(
					file,
					roomIdentifier,
					id,
					messageGroupIndex
	...
	async prepareFile(info, data, fileName) {
		...
		switch (info.base) {
			...
			case 'emoticons.json':
				this.logger.error(
					"HipChat Enterprise importer doesn't import emoticons.",
					{ info }
				);
				break;
			default:
				this.logger.error(
					`HipChat Enterprise importer doesn't know what to do with the file "${fileName}" :o`,
					{ info }
				);
				break;
		}
		...
		for (const fileName of files) {
			try {
				...
				if (fs.statSync(fullFilePath).isDirectory()) {
					await this._prepareFolderEntry(
						fullFilePath,
						fullRelativePath
					); // eslint-disable-line no-await-in-loop
					continue;
	...
	prepareUsingLocalFolder(fullFolderPath) {
		this.logger.debug(
			'start preparing import operation using local folder'
		);
		this.collection.remove({});
		this.emailList = [];

		this._hasAnyImportedMessage = Boolean(
			Messages.findOne({ _id: /hipchatenterprise\-.*/ })
		);
	...
	async _finishPreparationProcess(resolve, reject) {
		...
		super.updateRecord({
			'count.messages': this.messagesCount,
			messagesstatus: null
		});
		...
		if (this.emailList.length > 0) {
			const conflictingUsers = Users.find({
				'emails.address': { $in: this.emailList }
			});
		...
		if (!this.usersCount && !this.channelsCount && !this.messagesCount) {
			this.logger.info(
				`users: ${this.usersCount}, channels: ${this.channelsCount}, messages = ${this.messagesCount}`
			);
		...
		const selectionUsers = tempUsers.users.map(
			u =>
				new SelectionUser(
					u.id,
					u.username,
					...
					false,
					u.do_import !== false,
					u.is_email_taken === true
				)
		);
		const selectionChannels = tempChannels.channels.map(
			r =>
		...
		const promise = new Promise((resolve, reject) => {
			this.extract.on(
				...
				Meteor.bindEnvironment((header, stream, next) => {
					this.logger.debug(
						`new entry from import file: ${header.name}`
					);
					...
					stream.on(
						...
						Meteor.bindEnvironment(async () => {
							this.logger.info(
								`Processing the file: ${header.name}`
							);
		...
		Meteor.runAsUser(existingUserId, () => {
			...
			if (userToImport.avatar) {
				Meteor.call(
					...
					`data:image/png;base64,${userToImport.avatar}`
				);
			}
		});
	}
	...
	_importUser(userToImport, startedByUserId) {
		Meteor.runAsUser(startedByUserId, () => {
			let existingUser = Users.findOneByUsernameIgnoringCase(
	...
	_applyUserSelections(importSelection) {
		...
		const usersToImport = importSelection.users
			.filter(user => user.do_import !== false)
			.map(user => user.user_id);
		const usersNotToImport = importSelection.users
			.filter(user => user.do_import === false)
			.map(user => user.user_id);
		...
		const channelsToImport = importSelection.channels
			.filter(channel => channel.do_import !== false)
			.map(channel => channel.channel_id);
		const channelsNotToImport = importSelection.channels
			.filter(channel => channel.do_import === false)
			.map(channel => channel.channel_id);
	...
	startImport(importSelection) {
		...
		Meteor.defer(async () => {
			...
			this.logger.log(
				`HipChat Enterprise Import took ${timeTook} milliseconds.`
			);
			this._userDataCache = {};
	...
	_importUsers(startedByUserId) {
		...
		userLists.forEach(list => {
			...
			list.users.forEach(u => {
				this.logger.debug(
					`Starting the user import: ${u.username} and are we importing them? ${u.do_import}`
				);
	...
	_createSubscriptions(channelToImport, roomOrRoomId) {
		...
		channelToImport.members.forEach(hipchatUserId => {
			...
			if (!user) {
				this.logger.error(
					`User ${hipchatUserId} not found on Rocket.Chat database.`
				);
				return;
			}
			...
			if (
				Subscriptions.find(
					{ rid: room._id, 'u._id': user._id },
					{ limit: 1 }
				).count() === 0
			) {
				this.logger.info(
					`Creating user's subscription to room ${room._id}, rocket.chat user is ${user._id}, hipchat user is ${hipchatUserId}`
				);
	...
	_importChannel(channelToImport, startedByUserId) {
		Meteor.runAsUser(startedByUserId, () => {
			const existingRoom = Rooms.findOneByName(
				limax(channelToImport.name)
			);
			// If the room exists or the name of it is 'general', then we don't need to create it again
			if (
				existingRoom ||
				channelToImport.name.toUpperCase() === 'GENERAL'
			) {
				channelToImport.rocketId =
					channelToImport.name.toUpperCase() === 'GENERAL'
						? 'GENERAL'
						: existingRoom._id;
				this._saveRoomIdReference(
					channelToImport.id,
					channelToImport.rocketId
				);
				Rooms.update(
					{ _id: channelToImport.rocketId },
					{ $push: { importIds: channelToImport.id } }
				);

				this._createSubscriptions(
					channelToImport,
					existingRoom || 'general'
				);
			} else {
				// Find the rocketchatId of the user who created this channel
				const creatorId =
					this._getUserRocketId(channelToImport.creator) ||
					startedByUserId;

				// Create the channel
				Meteor.runAsUser(creatorId, () => {
					try {
						const roomInfo = Meteor.call(
							channelToImport.isPrivate
								? 'createPrivateGroup'
								: 'createChannel',
							channelToImport.name,
							[]
						);
						this._saveRoomIdReference(
							channelToImport.id,
						...
						this.logger.error(
							`Failed to create channel, using userId: ${creatorId};`,
							e
						);
					}
				});
	...
	_importChannels(startedByUserId) {
		...
		channelLists.forEach(list => {
			...
			list.channels.forEach(c => {
				this.logger.debug(
					`Starting the channel import: ${c.name} and are we importing them? ${c.do_import}`
				);
	...
	_importSingleMessage(msg, roomIdentifier, room) {
		if (isNaN(msg.ts)) {
			this.logger.error(
				`Timestamp on a message in ${roomIdentifier} is invalid`
			);
			return;
		}
		...
		try {
			...
			if (creator) {
				...
				this.addMessageError(
					new Meteor.Error('error-message-sender-is-invalid'),
					`Hipchat user not found: ${msg.userId}`
				);
			}
		} catch (e) {
			this.logger.error(e);
	...
	async _importMessageList(startedByUserId, messageListId) {
		...
		if (!rid) {
			this.logger.debug(
				`Ignoring room ${roomIdentifier} ( ${name} ), as there's no rid to use.`
			);
			return;
		}

		const room = await Rooms.findOneById(rid, {
			fields: { usernames: 1, t: 1, name: 1 }
		});
	...
	async _importMessages(startedByUserId) {
		const messageListIds = this.collection
			.find(
				{
					import: this.importRecord._id,
					importer: this.name,
	...
	_importDirectMessages() {
		...
		this.logger.info(
			`${messageListIds.length} lists of messages to import.`
		);
		...
		messageListIds.forEach(item => {
			...
			if (!this.getRocketUserFromRoomIdentifier(roomIdentifier)) {
				this.logger.error(
					`Skipping ${list.messages.length} messages due to missing room ( ${roomIdentifier} ).`
				);
				return;
			}
			...
			list.messages.forEach(msg => {
				...
				if (isNaN(msg.ts)) {
					this.logger.error(
						`Timestamp on a message in ${list.name} is invalid`
					);
					return;
				}
				...
				if (!(msg.senderId in roomUsers)) {
					roomUsers[msg.senderId] = this.getRocketUserFromUserId(
						msg.senderId
					);
				}
				...
				if (!roomUsers[msg.senderId]) {
					this.logger.error(
						`Skipping message due to missing sender ( ${msg.senderId} ).`
					);
					return;
				}
				...
				if (!(msg.receiverId in roomUsers)) {
					roomUsers[msg.receiverId] = this.getRocketUserFromUserId(
						msg.receiverId
					);
				}
				...
				if (!roomUsers[msg.receiverId]) {
					this.logger.error(
						`Skipping message due to missing receiver ( ${msg.receiverId} ).`
					);
					return;
				}
	...
	_getBasicUserData(userId) {
		...
		this._userDataCache[userId] = Users.findOneById(userId, {
			fields: { username: 1 }
		});
		return this._userDataCache[userId];
	}
