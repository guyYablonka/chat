...
const fetchUsersFromServer = _.throttle(
	async (filterText, useRegex = true, records, rid, cb) => {
		const usernames = [];
		...
		const roomType = roomTypes.getRoomTypeLabel(rid);
			...
			.forEach(
				({ username, nickname, name, status, avatarETag, outside }) => {
					...
					records.push({
						...
						outside,
						sort: 3,
						notIn: outside ? t(`Not_in_${roomType}`) : ''
					});
					// }
				}
			);
	...
	popupUserConfig() {
		...
		return {
			...
			getFilter: (collection, filter = '', cb) => {
				...
				if (filterText !== '') {
					fetchUsersFromServer(filterText, true, items, rid, cb);
				}
