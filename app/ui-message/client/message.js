...
createTemplateForComponent('messageLocation', () =>
	import('../../../client/views/location/MessageLocation')
);

const renderBody = (msg, room, settings) => {
...
Template.message.helpers({
	...
	body() {
		...
		return Tracker.nonreactive(() => renderBody(msg, room, settings));
	},
	i18nReplyCounter() {
		const { msg } = this;
		if (msg.tcount === 1) {
	...
	isIgnored() {
		...
		const isIgnored =
			typeof ignored !== 'undefined' ? ignored : msg.ignored;
		return isIgnored;
	},
	ignoredClass() {
		const { ignored, msg } = this;
	...
	getName() {
		...
		const cleanName = name => {
			const partialName = name.split('/').pop();
			return partialName || name;
		};

		if (msg.alias) {
			return cleanName(msg.alias);
		}
		...
		return (
			(settings.UI_Use_Real_Name && cleanName(msg.u.name)) ||
			msg.u.username
		);
	},
	showUsername() {
	...
	date() {
		...
		const thatDay = new Date(msg.ts).setHours(0, 0, 0, 0);
	...
	reactions() {
		...
		return Object.entries(reactions).map(([emoji, reaction]) => {
			...
			if (
				displayNames.some(displayName => displayName === myDisplayName)
			) {
