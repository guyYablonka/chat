...
import { call, isRTL, keyCodes, messageBox, popover } from '../../../ui-utils';
import { getUserPreference, roomTypes, t } from '../../../utils/client';
import './messageBox.html';
import './messageBoxActions';
import './messageBoxAudioMessage';
import { setupAutogrow } from './messageBoxAutogrow';
import { applyFormatting, formattingButtons } from './messageBoxFormatting';
...
Template.messageBox.onRendered(function () {
	...
	this.autorun(() => {
		...
		const isBlocked =
			room && room.t === 'd' && subscription && subscription.blocked;
		const isBlocker =
			room && room.t === 'd' && subscription && subscription.blocker;
...
Template.messageBox.helpers({
	...
	isWritable() {
		...
		const isBlockedOrBlocker =
			Template.instance().state.get('isBlockedOrBlocker');
...
const handleSubmit = (event, instance) => {
	...
	const isSubmitKey =
		keyCode === keyCodes.CARRIAGE_RETURN || keyCode === keyCodes.NEW_LINE;
...
Template.messageBox.events({
	...
	'click .js-emoji-picker'(event, instance) {
		...
		EmojiPicker.open(instance.source, ({ emoji }) => {
			...
			input.selectionStart = caretPos + emoji.length;
			input.selectionEnd = caretPos + emoji.length;
		});
	},
	...
	'keydown .js-input-message'(event, instance) {
		const isEventHandled =
			handleFormattingShortcut(event, instance) ||
