...
import validateFileName from '../methods/validateFileName';
...
export const FileUpload = {
	...
	validateFileUpload({ file, content }) {
		...
		const { isValid, reason } = validateFileName(file.name, language);

		if (!isValid) {
			throw new Meteor.Error('error-invalid-file-name', reason);
		}

		// App IPreFileUpload event hook
		try {
			Promise.await(
				Apps.triggerEvent(AppEvents.IPreFileUpload, { file, content })
			);
		} catch (error) {
	...
	validateAvatarUpload({ file }) {
		...
		const user = file.uid
			? Meteor.users.findOne(file.uid, { fields: { language: 1 } })
			: null;
	...
	defaultUserDataFiles() {
		return {
			...
			getPath(file) {
				return `${settings.get('uniqueID')}/uploads/userData/${
					file.userId
				}`;
			},
			onValidate: FileUpload.uploadsOnValidate,
	...
	resizeImagePreview(file) {
		...
		const image = FileUpload.getStore('Uploads')._store.getReadStream(
			file._id,
			file
		);
	...
	uploadsOnValidate(file) {
		...
		s.metadata(
			Meteor.bindEnvironment((err, metadata) => {
				...
				const rotated =
					typeof metadata.orientation !== 'undefined' &&
	...
	requestCanAccessFiles({ headers = {}, query = {} }) {
		...
		const isAuthorizedByCookies =
			rc_uid &&
			rc_token &&
			Users.findOneByIdAndLoginToken(rc_uid, rc_token);
		const isAuthorizedByHeaders =
			headers['x-user-id'] &&
			headers['x-auth-token'] &&
			Users.findOneByIdAndLoginToken(
				headers['x-user-id'],
				headers['x-auth-token']
			);
		const isAuthorizedByRoom =
			rc_room_type &&
			roomTypes
				.getConfig(rc_room_type)
				.canAccessUploadedFile({ rc_uid, rc_rid, rc_token });
	...
	addExtensionTo(file) {
		...
		if (file.type !== 'application/octet-stream') {
			...
			if (
				ext &&
				new RegExp(`\\.${ext}$`, 'i').test(file.name) === false
			) {
	...
	proxyFile(fileName, fileParams, forceDownload, request, req, res) {
		res.setHeader(
			'Content-Disposition',
			`${forceDownload ? 'attachment' : 'inline'}; filename="${encodeURI(
				fileName
			)}"`
		);

		request.get(fileParams, fileRes => fileRes.pipe(res));
	},
