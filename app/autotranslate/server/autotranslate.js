...
export class AutoTranslate {
	...
	translateMessage(message, room, targetLanguage) {
		...
		if (targetLanguage) {
			...
			targetLanguages =
				Subscriptions.getAutoTranslateLanguagesByRoomAndNotUser(
					room._id,
				...
				for (const index in message.attachments) {
					if (message.attachments.hasOwnProperty(index)) {
						...
						if (attachment.description || attachment.text) {
							const translations =
								this._translateAttachmentDescriptions(
									attachment,
									targetLanguages
								);
							if (!_.isEmpty(translations)) {
	...
	_getProviderMetadata() {
		Logger.warn('_getProviderMetadata must be implemented by subclass!');
	}
	...
	getSupportedLanguages(target) {
		Logger.warn('getSupportedLanguages must be implemented by subclass!', {
			target
		});
	}
	...
	_translateMessage(message, targetLanguages) {
		Logger.warn('_translateMessage must be implemented by subclass!', {
			message,
			targetLanguages
		});
	}
	...
	_translateAttachmentDescriptions(attachment, targetLanguages) {
		Logger.warn(
			'_translateAttachmentDescriptions must be implemented by subclass!',
			{ attachment, targetLanguages }
		);
	}
}
