...
import {
    useUserId,
    useUserSubscription
} from '../../../../client/contexts/UserContext';
import {
    useEndpoint,
    useMethod
...
const useThreadMessage = (tmid: string): IMessage => {
    ...
    const getMessageParsed = useCallback<
        (params: Mongo.Query<IMessage>) => Promise<IMessage>
    >(
        async params => {
    ...
    useEffect(() => {
        const computation = Tracker.autorun(async computation => {
            const msg =
                ChatMessage.findOne({ _id: tmid }) ||
                (await getMessageParsed({ msgId: tmid }));
...
const ThreadComponent: FC<{
    ...
    const handleClose = useCallback(() => {
        channelRoute.push(
            room.t === 'd' ? { rid: room._id } : { name: room.name }
        );
    ...
    useEffect(() => {
        setViewData(viewData => {
            if (
                !threadMessage ||
                viewData.mainMessage?._id === threadMessage._id
            ) {
                return viewData;
            }
