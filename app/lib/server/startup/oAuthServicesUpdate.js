	...
	services.forEach(service => {
		...
		if (service.value === true) {
			...
			if (/Accounts_OAuth_Custom-/.test(service.key)) {
				...
				data.serverURL =
					process.env.WEB_AUTH_URL ||
					settings.get(`${service.key}-url`);

				data.tokenPath = settings.get(`${service.key}-token_path`);
				data.identityPath = settings.get(
					`${service.key}-identity_path`
				);
				data.authorizePath = settings.get(
					`${service.key}-authorize_path`
				);
				data.scope = settings.get(`${service.key}-scope`);
				data.accessTokenParam = settings.get(
					`${service.key}-access_token_param`
				);
				data.buttonLabelText = settings.get(
					`${service.key}-button_label_text`
				);
				data.buttonLabelColor = settings.get(
					`${service.key}-button_label_color`
				);
				data.loginStyle = settings.get(`${service.key}-login_style`);
				data.buttonColor = settings.get(`${service.key}-button_color`);
				data.tokenSentVia = settings.get(
					`${service.key}-token_sent_via`
				);
				data.identityTokenSentVia = settings.get(
					`${service.key}-identity_token_sent_via`
				);
				data.usernameField = settings.get(
					`${service.key}-username_field`
				);
				...
				data.channelsMap = settings.get(
					`${service.key}-groups_channel_map`
				);
				data.channelsAdmin = settings.get(
					`${service.key}-channels_admin`
				);
