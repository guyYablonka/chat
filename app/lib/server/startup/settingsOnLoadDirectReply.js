...
import {
	...
	POP3Helper,
	POP3
} from '../lib/interceptDirectReplyEmails.js';

let IMAP;
let _POP3Helper;

const startEmailIntercepter = _.debounce(
		...
		if (
			...
			if (settings.get('Direct_Reply_Protocol') === 'IMAP') {
				...
				if (IMAP && IMAP.isActive()) {
					console.log(
						'Disconnecting already running IMAP instance...'
					);
					IMAP.stop(
						Meteor.bindEnvironment(function () {
					...
					console.log(
						'Disconnecting already running POP instance...'
					);
					_POP3Helper.stop(
						Meteor.bindEnvironment(function () {
							console.log('Starting new IMAP instance......');
