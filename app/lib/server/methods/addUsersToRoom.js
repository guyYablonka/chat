...
Meteor.methods({
	addUsersToRoom(data = {}) {
		...
		data.users.forEach(userToAdd => {
			const newUser = Users.findOneByUsernameIgnoringCase(
				userToAdd.username || userToAdd
			);
			if (!newUser) {
				throw new Meteor.Error(
			...
			if (!subscription) {
				addUserToRoom(data.rid, newUser, user, false, data.extraData);
			} else {
