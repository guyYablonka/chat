...
const logger = new Logger('getFullUserData', {}, __filename);
...
settings.get('Accounts_DefultUsersInfoFieldsToShow', (key, value) => {
	if (!value.trim()) {
		return;
	}

	try {
		defaultFields = JSON.parse(value.trim());
	} catch (e) {
		logger.warn(
			`The JSON specified for "Accounts_DefultUsersInfoFieldsToShow" is invalid. The following error was thrown`,
			{ e }
		);
	}
});
...
export function getFullUserDataByIdOrUsername({
	...
	const canViewAllInfo =
		myself || hasPermission(userId, 'view-full-other-user-info');
...
export const getFullUserData = function ({ userId, filter, limit: l }) {
	...
	const userToRetrieveFullUserData =
		username &&
	...
	const isMyOwnInfo =
		userToRetrieveFullUserData && userToRetrieveFullUserData._id === userId;
	...
	if (limit === 1) {
		return Users.findByUsername(
			userToRetrieveFullUserData.username,
			options
		);
	}
