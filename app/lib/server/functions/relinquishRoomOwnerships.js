...
export const relinquishRoomOwnerships = function (
	userId,
	subscribedRooms,
	...
	subscribedRooms
		...
		.forEach(({ newOwner, rid }) =>
			Roles.addUserRoles(newOwner, ['owner'], rid)
		);

	const roomIdsToRemove = subscribedRooms
	...
	if (removeDirectMessages) {
		Rooms.find1On1ByUserId(userId, {
			fields: { _id: 1 }
		}).forEach(({ _id }) => roomIdsToRemove.push(_id));
	}
