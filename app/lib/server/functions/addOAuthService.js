...
export function addOAuthService(name, values = {}) {
	...
	settings.add(`Accounts_OAuth_Custom-${name}`, values.enabled || false, {
		type: 'boolean',
		group: 'OAuth',
	...
	settings.add(`Accounts_OAuth_Custom-${name}-url`, values.serverURL || '', {
		type: 'string',
		group: 'OAuth',
		section: `Custom OAuth: ${name}`,
		i18nLabel: 'URL',
	...
	settings.add(`Accounts_OAuth_Custom-${name}-id`, values.clientId || '', {
		type: 'string',
		group: 'OAuth',
		section: `Custom OAuth: ${name}`,
