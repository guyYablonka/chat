...
function validateUserData(userId, userData) {
	...
	if (
		...
		throw new Meteor.Error(
			'error-action-not-allowed',
			'The field Roles consist invalid role name',
			{
				method: 'insertOrUpdateUser',
				action: 'Assign_role'
			}
	...
	try {
		nameValidation = new RegExp(
			`^${settings.get('UTF8_Names_Validation')}$`
		);
	} catch (e) {
		nameValidation = new RegExp('^[\\-.\'"\\w א-ת]+$');
	}
...
export const saveUser = function (userId, userData) {
	...
	if (!userData._id) {
		...
		if (typeof userData.requirePasswordChange !== 'undefined') {
			updateUser.$set.requirePasswordChange =
	...
	const userBefore = Users.findOneById(userData._id);

	Meteor.users.update({ _id: userData._id }, updateUser);

	if (userData.roles) {
		notifyRolesUpdated(userData._id, userBefore.roles, userData.roles);
	}
