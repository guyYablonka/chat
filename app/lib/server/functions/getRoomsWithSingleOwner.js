...
export function shouldRemoveOrChangeOwner(subscribedRooms) {
	return subscribedRooms.some(
		({ shouldBeRemoved, shouldChangeOwner }) =>
			shouldBeRemoved || shouldChangeOwner
	);
}
...
export function getSubscribedRoomsForUserWithDetails(userId) {
	...
	Subscriptions.findByUserIdExceptType(userId, 'd').forEach(subscription => {
		...
		if (subscriptionHasRole(subscription, 'owner')) {
			...
			const numOwners = Subscriptions.findByRoomIdAndRoles(
				subscription.rid,
				['owner']
			).count();
			...
			if (numOwners === 1) {
				...
				const subscribersCursor = Subscriptions.findByRoomId(
					subscription.rid,
					options
				);
			...
			roomData.shouldBeRemoved =
				Subscriptions.findByRoomId(roomData.rid).count() === 1;
		}
