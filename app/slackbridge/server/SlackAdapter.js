	...
	populateMembershipChannelMapByChannels() {
		...
		for (const slackChannel of channels) {
			const rocketchat_room =
				Rooms.findOneByName(slackChannel.name, {
					fields: { _id: 1 }
				}) ||
	...
	**/
					...
					if (err) {
						...
						if (/^image\/.+/.test(file.type)) {
							...
							attachment.image_dimensions =
								file.identify && file.identify.size;
						}
	...
	copyChannelInfo(rid, channelMap) {
		...
		if (channel) {
			...
			if (topic) {
				const creator =
					this.rocket.findUser(topic_creator) ||
		...
		if (rocketchat_room) {
			if (this.getSlackChannel(rid)) {
				...
				let results = this.importFromHistory(
					...
					{ channel: this.getSlackChannel(rid).id, oldest: 1 }
				);
