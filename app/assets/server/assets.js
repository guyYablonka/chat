...
const assets = {
	chat_logo: {
		label: 'chat-logo (svg, png, jpg)',
		defaultUrl: 'images/logo/chat-logo.png',
		constraints: {
			type: 'image',
	...
	create_group_button: {
		label: 'create group button image',
		constraints: {
			type: 'image',
			extensions: ['svg', 'png', 'jpg', 'jpeg'],
			width: undefined,
...
settings.get(/^Assets_/, (key, value) =>
	RocketChatAssets.processAsset(key, value)
);
...
WebApp.connectHandlers.use(
	...
	Meteor.bindEnvironment(function (req, res, next) {
		...
		if (!file) {
			const defaultUrl =
				assets[params.asset] && assets[params.asset].defaultUrl;
			if (defaultUrl) {
				const assetUrl =
