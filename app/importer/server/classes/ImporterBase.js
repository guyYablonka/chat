...
export class Base {
	...
	static getBSONSafeArraysFromAnArray(theArray) {
		...
		const maxSize = Math.floor(
			theArray.length / Math.ceil(BSONSize / Base.getMaxBSONSize())
		);
		const safeArrays = [];
		let i = 0;
	...
	constructor(info, importRecord) {
		...
		this.logger = new Logger(`${this.info.name} Importer`, {}, __filename);
	...
	prepare(dataURI, sentContentType, fileName, skipTypeCheck) {
		...
		if (!skipTypeCheck) {
			const fileType = this.getFileType(
				new Buffer(dataURI.split(',')[1], 'base64')
			);
			this.logger.debug('Uploaded file information:', { fileType });
			this.logger.debug('Expected file type:', {
				fileType: this.info.mimeType
			});
	...
	uploadFile(details, fileUrl, user, room, timeStamp) {
		this.logger.debug(
			`Uploading the file ${details.name} from ${fileUrl}.`
		);
