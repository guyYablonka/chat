...
createTemplateForComponent('accountSecurity', () =>
	import('../../../client/views/account/security/AccountSecurityPage')
);

callbacks.add(
	'afterLogoutCleanUp',
	() => customScriptsOnLogout(),
...
Template.main.helpers({
	removeSidenav() {
		return (
			Layout.isEmbedded() &&
			!/^\/admin/.test(FlowRouter.current().route.path)
		);
	},
	siteName() {
	...
	hasUsername() {
		...
		const user =
			uid && Users.findOne({ _id: uid }, { fields: { username: 1 } });
		return (
			(user && user.username) ||
	...
	showSetupWizard() {
		...
		return (
			(!userId && Show_Setup_Wizard === 'pending') ||
			(userId &&
				hasRole(userId, 'admin') &&
				Show_Setup_Wizard === 'in_progress')
		);
	}
});
