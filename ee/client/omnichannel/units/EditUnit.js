...
export function UnitEditWithData({ unitId, reload, allUnits }) {
	...
	if (
		[
			state,
			availableDepartmentsState,
			availableMonitorsState,
			unitMonitorsState
		].includes(AsyncStatePhase.LOADING)
	) {
		return <FormSkeleton />;
	}
	...
	const filteredDepartments = {
		departments: availableDepartments.departments.filter(
			department =>
				...
				!allUnits.units.find(
					unit => unit._id === department.ancestors[0]
				)
		)
	};
...
export function UnitNew({ reload, allUnits }) {
	...
	if (
		[availableDepartmentsState, availableMonitorsState].includes(
...
export function UnitEdit({
	...
	const depOptions = useMemo(
		() =>
			availableDepartments && availableDepartments.departments
				? availableDepartments.departments.map(({ _id, name }) => [
						_id,
						name || _id
				  ])
				: [],
	...
	const monOptions = useMemo(
		() =>
			availableMonitors && availableMonitors.monitors
				? availableMonitors.monitors.map(({ _id, name }) => [
						_id,
						name || _id
				  ])
				: [],
	...
	const nameError = useMemo(
		() =>
			!name || name.length === 0
				? t('The_field_is_required', t('name'))
				: undefined,
		[name, t]
	);
	...
	const unitMonitorsError = useMemo(
		() =>
			!monitors || monitors.length === 0
				? t('The_field_is_required', t('monitors'))
				: undefined,
		[monitors, t]
	);
