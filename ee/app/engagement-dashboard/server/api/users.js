	...
	{ authRequired: true },
	{
		get() {
			...
			const data = Promise.await(
				findWeeklyUsersRegisteredData(transformDatesForAPI(start, end))
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const data = Promise.await(
				findActiveUsersMonthlyData(transformDatesForAPI(start, end))
			);
			return API.v1.success(data);
		}
	}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const data = Promise.await(
				findBusiestsChatsInADayByHours(transformDatesForAPI(start))
			);
			return API.v1.success(data);
		}
	}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const data = Promise.await(
				findBusiestsChatsWithinAWeek(transformDatesForAPI(start))
			);
			return API.v1.success(data);
		}
	}
