...
export const handleMessagesDeleted = (message, room) => {
	...
	Promise.await(
		AnalyticsRaw.saveMessageDeleted({
			date: convertDateToInt(message.ts),
			room
		})
	);
...
export const fillFirstDaysOfMessagesIfNeeded = async date => {
	...
	if (!messagesFromAnalytics.length) {
		const startOfPeriod = moment(convertIntToDate(date))
			.subtract(90, 'days')
			.toDate();
...
export const findWeeklyMessagesSentData = async ({ start, end }) => {
	...
	const yesterdayMessages =
		(currentPeriodMessages.find(item => item._id === yesterday) || {})
			.messages || 0;
	const todayMessages =
		(currentPeriodMessages.find(item => item._id === today) || {})
			.messages || 0;
...
export const findMessagesSentOrigin = async ({ start, end }) => {
	...
	const missingTypes = roomTypesToShow.filter(
		type => !responseTypes.includes(type)
	);
