...
export function ActiveUsersSection() {
	...
	const [
		...
		for (const { users, day, month, year } of data.month) {
			const i = moment
				.utc([year, month - 1, day, 0, 0, 0, 0])
				.diff(period.start, 'days');
	...
	const downloadData = () => {
		...
		downloadCsvAs(
			data,
			`ActiveUsersSection_start_${params.start}_end_${params.end}`
		);
	};
