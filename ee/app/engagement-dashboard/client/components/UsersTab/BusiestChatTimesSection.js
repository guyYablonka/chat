...
function ContentForHours({
	...
	const params = useMemo(() => ({ start: currentDate.toISOString() }), [
		currentDate
	]);
...
function ContentForDays({
	displacement,
	onPreviousDateClick,
	onNextDateClick
}) {
	const currentDate = useMemo(
		() => moment.utc().subtract(displacement, 'weeks'),
		[displacement]
	);
...
export function BusiestChatTimesSection() {
	...
	const handlePreviousDateClick = () =>
		setDisplacement(displacement => displacement + 1);
	const handleNextDateClick = () =>
		setDisplacement(displacement => displacement - 1);

	const Content =
		(timeUnit === 'hours' && ContentForHours) ||
		(timeUnit === 'days' && ContentForDays);
