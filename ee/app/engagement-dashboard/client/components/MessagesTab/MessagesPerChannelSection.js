...
export function MessagesPerChannelSection() {
	...
	const { value: pieData } = useEndpointData(
	...
	const [pie, table] = useMemo(() => {
		...
		const pie = pieData.origins.reduce(
	...
	const downloadData = () => {
		...
		downloadCsvAs(
			data,
			`MessagesPerChannelSection_start_${params.start}_end_${params.end}`
		);
	};
