...
const getWaitingQueueMessage = departmentId => {
	const department =
		departmentId && LivechatDepartment.findOneById(departmentId);
...
export const setPredictedVisitorAbandonmentTime = room => {
	if (
		...
		!settings.get('Livechat_auto_close_abandoned_rooms')
	) {
		return;
	}
	...
	const department =
		room.departmentId && LivechatDepartment.findOneById(room.departmentId);
