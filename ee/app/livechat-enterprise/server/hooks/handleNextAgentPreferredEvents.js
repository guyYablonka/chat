...
const getDefaultAgent = username =>
	...
	normalizeDefaultAgent(
		Users.findOneOnlineAgentByUsername(username, {
...
const checkDefaultAgentOnNewRoom = (defaultAgent, defaultGuest) => {
	...
	const guestManager =
		contactManagerPreferred && getDefaultAgent(contactManager?.username);
	if (guestManager) {
		return guestManager;
	}
	...
	const lastRoomAgent = normalizeDefaultAgent(
		Users.findOneOnlineAgentByUsername(usernameByRoom, {
			fields: { _id: 1, username: 1 }
		})
	);
