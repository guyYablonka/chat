...
import {
    SettingsEvents,
    settings
} from '../../../../app/settings/server/functions/settings';
import {
    isEnterprise,
    hasLicense,
    onValidateLicenses
...
export function changeSettingValue(
    record: ISetting
): undefined | { value: SettingValue } {
...
SettingsEvents.on(
    'store-setting-value',
    (record: ISetting, newRecord: { value: SettingValue }) => {
...
function updateSettings(): void {
    ...
    enterpriseSettings.forEach((record: ISetting) =>
        settings.storeSettingValue(record, false)
    );
}
