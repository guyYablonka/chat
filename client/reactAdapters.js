...
export const mountRoot = async () => {
	...
	render(
		createElement(Suspense, { fallback: null }, createElement(LazyAppRoot)),
		rootNode
	);
};
...
export const renderRouteComponent = (
	...
	Tracker.autorun(async computation => {
		...
		if (!Template[routeName]) {
			const blazeTemplate = new Blaze.Template(routeName, () =>
				HTML.DIV()
			...
			blazeTemplate.onRendered(async function () {
				...
				const handleMainContentDestroyed = () => {
					...
					document.removeEventListener(
						'main-content-destroyed',
						handleMainContentDestroyed
					);
				};
