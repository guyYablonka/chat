...
export function ContactNewEdit({ id, data, reload, close }) {
	...
	const { value: allCustomFields, phase: state } = useEndpointData(
	...
	useComponentDidUpdate(() => {
		setEmailError(
			email && !isEmail(email) ? t('Validate_email_address') : null
		);
	}, [t, email]);
	useComponentDidUpdate(() => {
	...
	const formIsValid =
		name && !emailError && !phoneError && customFieldsError.length === 0;
	...
	return (
		<>
			<VerticalBar.ScrollableContent is="form">
				...
				{ContactManager && (
					<ContactManager value={username} handler={handleUsername} />
				)}
