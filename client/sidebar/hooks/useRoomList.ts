...
export const useRoomList = (): Array<ISubscription> => {
    ...
    useEffect(() => {
        setRoomList(() => {
            ...
            showOmnichannel &&
                inquiries.enabled &&
                groups.set('Omnichannel', []);
            showOmnichannel &&
                !inquiries.enabled &&
                groups.set('Omnichannel', omnichannel);
            showOmnichannel &&
                inquiries.enabled &&
                inquiries.queue.length &&
                groups.set('Incoming_Livechats', inquiries.queue);
            showOmnichannel &&
                inquiries.enabled &&
                omnichannel.size &&
            ...
            sidebarShowUnread &&
                unread.size &&
                groups.set('Unread_Messages', unread);
            favoritesEnabled && favorite.size && groups.set('Pinned', favorite);
            showDiscussion &&
                discussion.size &&
                groups.set('Discussions', discussion);
            sidebarGroupByType &&
                _public.size &&
                groups.set('Channels', _public);
            sidebarGroupByType &&
                _private.size &&
                groups.set('Groups', _private);
            sidebarGroupByType &&
                direct.size &&
                groups.set('Private_Chats', direct);
            !sidebarGroupByType &&
                rooms.length &&
                groups.set('Conversations', conversation);
        ...
        showOmnichannel,
        inquiries.enabled,
        inquiries.enabled && inquiries.queue,
        sidebarShowUnread,
        favoritesEnabled,
        showDiscussion,
        sidebarGroupByType
