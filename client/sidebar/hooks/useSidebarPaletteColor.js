...
const useTheme = () => {
	...
	const result = useMemo(() => {
		const n900 = customColors.find(
			({ _id }) => _id === 'theme-color-rc-color-primary-darkest'
		);
		const n800 = customColors.find(
			({ _id }) => _id === 'theme-color-rc-color-primary-dark'
		);
		...
		const n600 = customColors.find(
			({ _id }) => _id === 'theme-color-rc-color-primary-light'
		);
		const n500 = customColors.find(
			({ _id }) => _id === 'theme-color-rc-primary-light-medium'
		);

		const n400 = customColors.find(({ _id }) => _id === '');
		const n300 = customColors.find(({ _id }) => _id === '');

		const n200 = customColors.find(
			({ _id }) => _id === 'theme-color-rc-color-primary-lightest'
		);
...
const getStyle = (
	selector => colors =>
		...
		${selector} {
			...
			--rcx-button-colors-ghost-success-active-background-color: ${toVar(
				colors.n800
			)};
			...
			--rcx-button-colors-ghost-success-background-color: ${toVar(
				colors.sibebarSurface
			)};
			...
			--rcx-sidebar-item-background-color-selected: ${h2r(
				toVar(colors.n700 || colors.n800),
				0.3
			)};
			--rcx-badge-colors-ghost-background-color: ${toVar(colors.b500)};
		...
		(async () => {
			...
			try {
				...
				const fuselageStyleRules = sidebarStyle.innerText
					.match(/(.|\n)*?\{((.|\n)*?)\}(.|\n)*?/gi)
					.filter(text =>
						/\.rcx-(sidebar|button|divider|input)/.test(text)
					);
				...
				const filterSelectors = selector =>
