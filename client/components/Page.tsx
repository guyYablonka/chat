...
const PageScrollableContent: FC<PageScrollableContentProps> = React.forwardRef(
    ({ onScrollContent, ...props }, ref) => (
        <Box display='flex' flexShrink={1} flexGrow={1} overflow='auto'>
            <ScrollableContentWrapper onScroll={onScrollContent} ref={ref}>
                <Box
                    p='x16'
                    display='flex'
                    flexDirection='column'
...
const PageScrollableContentWithShadow: FC<PageScrollableContentProps> = ({
    onScrollContent,
    ...props
}) => {
