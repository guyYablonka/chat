...
const HeaderIcon: FC<{
    icon: JSX.Element | { name: string; color?: string } | null;
}> = ({ icon }) =>
    icon && (
        <Box
            display='flex'
...
const Button: FC = (props: any) => (
    <Box mi='x4' display='flex' alignItems='center' {...props} />
);
const Avatar: FC = (props: any) => <Button width='x36' {...props} />;

const HeaderDivider: FC = () => (
    <Divider {...({ mbs: 'neg-x2', mbe: 0 } as any)} />
);

const Header: FC & { ToolBoxAction: FC; Badge: FC<BadgeProps> } = (
    props: any
) => (
    <Box
        rcx-room-header
