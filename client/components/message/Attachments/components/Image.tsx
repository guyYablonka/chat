...
export const Retry: FC<BoxProps & { retry: () => void }> = ({
    retry,
    ...props
}) => {
    ...
    `;
    return (
        <ImageBox className={clickable} {...props} onClick={retry}>
...
export const Load: FC<BoxProps & { load: () => void }> = ({
    load,
    ...props
}) => {
    const t = useTranslation();
...
const getDimensions = (
    ...
    if (
        height >= width ||
        Math.min(width, limits.width) * ratio > limits.height
    ) {
        return {
            width: (width * Math.min(height, limits.height)) / height,
            height: 'auto'
        };
    }
...
const Image: FC<ImageProps> = ({
    ...
    const background =
        previewUrl &&
        `url(${previewUrl}) center center / cover no-repeat fixed`;
