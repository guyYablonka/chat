...
const Action: FC<ButtonProps & { icon: string }> = props => (
    <ActionButton mi='x2' mini ghost {...props} />
);
const Collapse: FC<ButtonProps & { collapsed?: boolean }> = ({
    collapsed = false,
    ...props
}) => {
...
const Content: FC<BoxProps> = ({ ...props }) => (
    <Box rcx-attachment__content width='full' mb='x4' {...props} />
);
const Details: FC<BoxProps> = ({ ...props }) => (
    <Box
...
const AuthorName: FC<BoxProps> = props => (
    <Box withTruncatedText fontScale='p2' mi='x8' {...props} />
);

const Thumb: FC<{ url: string }> = memo(({ url }) => (
    <Box mis='x8'>
