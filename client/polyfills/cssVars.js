...
const findDeclarations = code =>
	(code.match(/(--[^:; ]+:..*?;)/g) ?? []).map(declaration => {
		...
		return [
			...
			value.indexOf('var(') >= 0
				? variables =>
						value.replace(/var\((--.*?)\)/gm, (_, name) =>
							variables[name]?.call(null, variables)
						)
				: () => value
		];
	});
...
const update = _.debounce(() => {
	...
	originalCodes.forEach((originalCode, element) => {
		...
		if (
			!patchedElement ||
			!patchedElement.classList.contains('patched-css-variables')
		) {
...
const findAndPatchFromLinkElements = () => {
	Array.from(
		document.querySelectorAll('link[type="text/css"].__meteor-css__')
	).forEach(async linkElement => {
