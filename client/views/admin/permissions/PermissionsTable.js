...
const RoleCell = React.memo(
	({
		...
		const [granted, setGranted] = useState(
			() => !!grantedRoles.includes(_id)
		);
		const [loading, setLoading] = useState(false);

		const isRestrictedForRole = AuthorizationUtils.isPermissionRestrictedForRole(
			permissionId,
			_id
		);
...
const PermissionRow = React.memo(
	({ permission, t, roleList, onGrant, onRemove, ...props }) => {
...
const PermissionsTable = () => {
	...
	const permissionsData = usePermissionsAndRoles(
		type,
		filter,
		params.limit,
		params.skip
				...
				<Page.Content mb="neg-x8">
					<Margins block="x8">
						...
						<GenericTable
							header={
								<>
									<GenericTable.HeaderCell width="x120">
										{t('Name')}
									</GenericTable.HeaderCell>
									{roleList.map(({ _id, description }) => (
										<RoleHeader
