...
const DeploymentCard = React.memo(function DeploymentCard({
	...
	const handleInstancesModal = useMutableCallback(() => {
		setModal(
			<InstancesModal instances={instances} onClose={() => setModal()} />
		);
	});
			...
			<Card.Body>
				<Card.Col>
					...
					<Card.Col.Section>
						...
						{s(
							() =>
								`${statistics.mongoVersion} / ${
									statistics.mongoStorageEngine
								} (oplog ${
									statistics.oplogEnabled
										? t('Enabled')
										: t('Disabled')
								})`
						)}
