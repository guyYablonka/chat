                ...
                >
                    <Margins inline='x8'>
                        ...
                        <Field.Row>
                            ...
                            />
                            <Field.Label htmlFor={idOmnichannel}>
                                {t('Omnichannel')}
                            </Field.Label>
                        </Field.Row>
                        <Field.Row>
                            ...
                            />
                            <Field.Label htmlFor={idDiscussions}>
                                {t('Discussions')}
                            </Field.Label>
                        </Field.Row>
                    </Margins>
...
const useQuery = (
    { text, types, itemsPerPage, current },
    [column, direction]
) =>
    useMemo(
        () => ({
    ...
    if (sort[0] === 'name' && data.rooms) {
        data.rooms = data.rooms.sort((a, b) => {
            const aName =
                a.type === 'd'
                    ? a.usernames.join(' x ')
                    : roomTypes.getRoomName(a.t, a);
            const bName =
                b.type === 'd'
                    ? b.usernames.join(' x ')
                    : roomTypes.getRoomName(b.t, b);
            if (aName === bName) {
                return 0;
            }
        ...
        ({
            ...
            return (
                ...
                >
                    ...
                    {mediaQuery && (
                        <Table.Cell style={style}>{msgs}</Table.Cell>
                    )}
                    {mediaQuery && (
                        <Table.Cell style={style}>
                            {isDefault ? t('True') : t('False')}
                        </Table.Cell>
                    )}
                    {mediaQuery && (
                        <Table.Cell style={style}>
