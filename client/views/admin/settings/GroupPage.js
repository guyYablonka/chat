...
function GroupPage({
	...
	const save = useMutableCallback(async () => {
		const changes = changedEditableSettings.map(
			({ _id, value, editor }) => ({ _id, value, editor })
		);
		...
		try {
			...
			if (changes.some(({ _id }) => _id === 'Language')) {
				const lng =
					user?.language ||
					changes.filter(({ _id }) => _id === 'Language').shift()
						?.value ||
					'en';
				...
				dispatchToastMessage({
					type: 'success',
					message: t('Settings_updated', { lng })
				});
				return;
			}
	...
	const cancel = useMutableCallback(() => {
		dispatchToEditing(
			changedEditableSettings
				.map(({ _id }) =>
					originalSettings.find(setting => setting._id === _id)
				)
				.map(setting => {
					if (!setting) {
						return;
