...
const useQuery = ({ text, itemsPerPage, current }, sortFields, searchFields) =>
	useMemo(
		() => ({
			...
			query: JSON.stringify({
				$or: searchFields?.map(field => ({
					[field.trim()]: { $regex: text, $options: 'i' }
				}))
			}),
			sort: JSON.stringify(
...
export function UsersTable() {
	const t = useTranslation();
	const searchFields = useSetting('Accounts_SearchFields')?.trim().split(',');
	...
	const query = useQuery(debouncedParams, debouncedSort, searchFields);
