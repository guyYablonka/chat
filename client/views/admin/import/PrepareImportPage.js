...
function PrepareImportPage() {
	...
	const usersCount = useMemo(
		() => users.filter(({ do_import }) => do_import).length,
		[users]
	);
	...
	const getCurrentImportOperation = useEndpoint(
		'GET',
	...
	useEffect(() => {
		const loadImportFileData = async () => {
			try {
				...
				setUsers(
					data.users.map(user => ({ ...user, do_import: true }))
				);
				setChannels(
		...
		const loadCurrentOperation = async () => {
			try {
				const { operation } = await waitFor(
					...
					({ operation }) =>
						operation.valid &&
						!ImportWaitingStates.includes(operation.status)
				);
