...
function NewImportPage() {
	...
	const downloadPublicImportFile = useEndpoint(
		'POST',
							...
							reader.onloadend = async () => {
								try {
									await uploadImportFile({
										binaryContent: reader.result.split(
											';base64,'
										)[1],
									...
									handleError(
										error,
										t('Failed_To_upload_Import_File')
									);
								} finally {
									resolve();
								}
								...
								{fileType === 'upload' && (
									<>
										{maxFileSize > 0 ? (
											...
											>
												{t(
													'Importer_Upload_Unlimited_FileSize'
												)}
											</Callout>
										)}
										<Field>
											...
											{files?.length > 0 && (
												<Field.Row>
													{files.map((file, i) => (
														<Chip
															key={i}
															onClick={handleFileUploadChipClick(
																file
															)}
														>
															{file.name}
