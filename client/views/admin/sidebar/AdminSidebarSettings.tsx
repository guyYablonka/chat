...
const useSettingsGroups = (filter: string): ISetting[] => {
    ...
    const filterPredicate = useMemo(() => {
        ...
        try {
            ...
            return (setting: ISetting): boolean =>
                getMatchableStrings(setting).some(text =>
                    filterRegex.test(text)
                );
        } catch (e) {
            return (setting: ISetting): boolean =>
                getMatchableStrings(setting).some(
                    text => text.slice(0, filter.length) === filter
                );
        }
    }, [filter, t]);
    ...
    return useMemo(() => {
        ...
        return settings
            ...
            .sort((a, b) =>
                t(a.i18nLabel || a._id).localeCompare(t(b.i18nLabel || b._id))
            );
...
const AdminSidebarSettings: FC<AdminSidebarSettingsProps> = ({
    currentPath
}) => {
    const t = useTranslation();
