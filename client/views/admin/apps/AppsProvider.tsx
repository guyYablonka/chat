...
const AppsProvider: FC = ({ children }) => {
    ...
    const [finishedLoading, setFinishedLoading] = useState<boolean>(
        () => false
    );
    ...
    useEffect(() => {
        ...
        const handleAppAddedOrUpdated = async (
            appId: string
        ): Promise<void> => {
            try {
        ...
        const listeners = {
            ...
            APP_REMOVED: (appId: string): void => {
                ...
                if (updatedData[index].marketplace !== false) {
                    ...
                    updatedData[index].version =
                        updatedData[index].marketplaceVersion;
                }
        ...
        const fetchData = async (): Promise<void> => {
            try {
                ...
                const marketplaceApps =
                    (await Apps.getAppsFromMarketplace()) as App[];

                const appsData = marketplaceApps.length
                    ? marketplaceApps.map<App>(app => {
                          const appIndex = installedApps.findIndex(
                              ({ id }) => id === app.id
                          );
                          ...
                          const installedApp = installedApps
                              .splice(appIndex, 1)
                              .pop();
                          return {
                              ...app,
