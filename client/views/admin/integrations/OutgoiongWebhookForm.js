...
export default function OutgoingWebhookForm({
	...
	const eventOptions = useMemo(
		() =>
			Object.entries(outgoingEvents).map(([key, val]) => [
				key,
				t(val.label)
			]),
		[t]
	...
	const showChannel = useMemo(() => outgoingEvents[event].use.channel, [
		event
	]);
	const showTriggerWords = useMemo(
		() => outgoingEvents[event].use.triggerWords,
		[event]
	);
	const showTargetRoom = useMemo(() => outgoingEvents[event].use.targetRoom, [
		event
	]);
	...
	const hilightedExampleJson = useHighlightedCode(
		'json',
						...
						{useMemo(
							() => (
								<Field>
									...
									<Field.Hint>
										{t(
											'You_can_change_a_different_avatar_too'
										)}
									</Field.Hint>
									<Field.Hint>
							...
							{useMemo(
								() => (
									<Field>
										...
										>
											{t(
												'Integration_Retry_Failed_Url_Calls'
											)}
											<ToggleSwitch
										...
										<Field.Hint>
											{t(
												'Integration_Retry_Failed_Url_Calls_Description'
											)}
										</Field.Hint>
									</Field>
								),
							...
							{useMemo(
								() => (
									<Field>
										...
										<Field.Hint>
											{t(
												'Integration_Retry_Count_Description'
											)}
										</Field.Hint>
									</Field>
								),
