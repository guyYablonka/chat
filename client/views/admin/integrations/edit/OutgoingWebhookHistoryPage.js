...
function HistoryItem({ data, ...props }) {
	...
	const createdAt =
		typeof _createdAt === 'string' ? _createdAt : _createdAt.toISOString();
	const updatedAt =
		typeof _updatedAt === 'string' ? _updatedAt : _updatedAt.toISOString();
	...
	const httpCallDataCode = useHighlightedCode(
		'json',
		JSON.stringify(httpCallData || '', null, 2)
	);
	const httpErrorCode = useHighlightedCode(
		'json',
		JSON.stringify(httpError || '', null, 2)
	);
	const httpResultCode = useHighlightedCode(
		'json',
		JSON.stringify(httpResult || '', null, 2)
	);
	const errorStackCode = useHighlightedCode(
		'json',
			...
			<FieldGroup>
				...
				{dataSentToTrigger && (
					<Field>
						<Field.Label>
							{t(
								'Integration_Outgoing_WebHook_History_Data_Passed_To_Trigger'
							)}
						</Field.Label>
						<Field.Row>
							<Box withRichContent w="full">
				...
				{errorStack && (
					<Field>
						<Field.Label>
							{t(
								'Integration_Outgoing_WebHook_History_Error_Stacktrace'
							)}
						</Field.Label>
						<Field.Row>
							<Box withRichContent w="full">
...
function OutgoingWebhookHistoryPage(props) {
	...
	useEffect(() => {
		...
		return () =>
			integrationHistoryStreamer.removeListener(id, handleDataChange);
