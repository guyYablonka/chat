...
const useFields = () => {
	...
	const [fields, dispatch] = useReducer((fields, { type, payload }) => {
		...
		if (type === setValue) {
			...
			return fields.map(field =>
				field._id === _id ? { ...field, value } : field
			);
		}
	...
	const resetFields = useCallback(
		fields => dispatch({ type: reset, payload: fields }),
		[]
	);
...
function SettingsBasedStep({ step, title, active }) {
	...
	useEffect(() => {
		resetFields(
			settings
				...
				.filter(({ type }) =>
					['string', 'select', 'language', 'boolean'].includes(type)
				)
				.sort(
					({ wizard: { order: a } }, { wizard: { order: b } }) =>
						a - b
				)
				.map(({ value, ...field }) => ({
					...field,
	...
	const hasEmptyRequiredFields = useMemo(
		() =>
			!!fields.find(
				field =>
					field.requiredOnWizard && String(field.value).trim() === ''
			),
		[fields]
	);
