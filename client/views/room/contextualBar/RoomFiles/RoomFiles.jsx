...
export const createItemData = memoize(
    (items, onClickDelete, isDeletionAllowed) => ({
        items,
        onClickDelete,
        isDeletionAllowed
    })
...
export const RoomFiles = function RoomFiles({
    ...
    const { ref, contentBoxSize: { blockSize = 780 } = {} } = useResizeObserver(
        { debounceDelay: 100 }
    );
    ...
    return (
        <>
            <VerticalBar.Header>
                ...
                <VerticalBar.Text>{t('Files_List')}</VerticalBar.Text>
            ...
            <VerticalBar.Content p='x12'>
                ...
                {!loading && filesItems.length <= 0 && (
                    <Box p='x12'>{t('No_results_found')}</Box>
                )}
...
export default ({ rid }) => {
    ...
    const handleDelete = useMutableCallback(_id => {
        ...
        setModal(
            <DeleteFileWarning onConfirm={onConfirm} onCancel={closeModal} />
        );
    }, []);
