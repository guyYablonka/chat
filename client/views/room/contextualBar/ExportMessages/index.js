...
const MailExportForm = ({ onCancel, rid }) => {
	...
	const reset = useMutableCallback(() => {
		setSelected([]);
		$(`#chat-window-${rid}.messages-box .message.selected`).removeClass(
			'selected'
		);
	});
			...
			<Field>
				...
				>
					...
					{selectedMessages.length === 0 && (
						<Box is="p">
							{t(
								'Click_the_messages_you_would_like_to_send_by_email'
							)}
						</Box>
					)}
				</Callout>
	...
	return (
		<>
			...
			<VerticalBar.ScrollableContent>
				...
				{type && type === 'file' && (
					<FileExport rid={rid} onCancel={close} />
				)}
				{type && type === 'email' && (
					<MailExportForm rid={rid} onCancel={close} />
				)}
			</VerticalBar.ScrollableContent>
		</>
	);
