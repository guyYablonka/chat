			...
			<VerticalBar.ScrollableContent>
				...
				<Field>
					<Field.Row>
						...
						/>
						<Field.Label htmlFor={inclusiveCheckboxId}>
							{t('Inclusive')}
						</Field.Label>
					</Field.Row>
				</Field>
				...
				<Field>
					<Field.Row>
						...
						/>
						<Field.Label htmlFor={attachedCheckboxId}>
							{t('Files_only')}
						</Field.Label>
					</Field.Row>
				</Field>

				{callOutText && !validateText && (
					<Callout type="warning">{callOutText}</Callout>
				)}
				{validateText && (
					<Callout type="warning">{validateText}</Callout>
				)}
			</VerticalBar.ScrollableContent>
			<VerticalBar.Footer>
				<ButtonGroup stretch>
...
export default ({ rid, tabBar }) => {
	...
	const onChangeUsers = useMutableCallback((value, action) => {
		if (!action) {
			if (usernames.find(current => current.includes(value.username))) {
				return;
			}
			return handleUsers([...users, value]);
		}
		handleUsers(usernames.filter(current => current !== value));
	});
	...
	const handlePrune = useMutableCallback(async () => {
		...
		try {
			...
			dispatchToastMessage({
				type: 'success',
				message: `${result} ${t('messages_pruned')}`
			});
			closeModal();
			reset();
		} catch (error) {
