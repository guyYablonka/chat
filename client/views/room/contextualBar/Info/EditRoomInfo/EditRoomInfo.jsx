...
const useInitialValues = (room, settings) => {
    ...
    const retentionEnabledDefault = useSetting(
        `RetentionPolicy_AppliesTo${typeMap[room.t]}`
    );
        ...
        () => ({
            roomName:
                t === 'd'
                    ...
                    : roomTypes.getRoomName(t, { type: t, ...room }),
            roomType: t,
            readOnly: !!ro,
            ...
            ...(retentionPolicyEnabled && {
                ...
                retentionMaxAge:
                    Math.min(retention.maxAge, maxAgeDefault) || maxAgeDefault,
                retentionExcludePinned:
                    retention.excludePinned ?? excludePinnedDefault,
...
const getCanChangeType = (room, canCreateChannel, canCreateGroup, isAdmin) =>
    ...
    ((room.t === 'p' && canCreateChannel) ||
        (room.t === 'c' && canCreateGroup));
...
function EditChannel({ room, onClickClose, onClickBack }) {
    ...
    const maxAgeDefault =
        useSetting(`RetentionPolicy_MaxAge_${typeMap[room.t]}`) || 30;

    const roomNameRegex = new RegExp(
        useSetting('UTF8_Channel_Names_Validation')
    );

    const roomTopicRegex = new RegExp(
        useSetting('UTF8_Channel_Topic_Validation')
    );

    const roomAnnouncementRegex = new RegExp(
        useSetting('UTF8_Channel_Announcment_Validation')
    );
    ...
    const [
        ...
        const isAllowed =
            roomTypes.getConfig(room.t)?.allowRoomSettingChange || (() => {});
        return [
    ...
    const canSetReactWhenRo = usePermission(
        'set-react-when-readonly',
        room._id
    );
    const canEditRoomRetentionPolicy = usePermission(
    ...
    const canToggleEncryption =
        usePermission('toggle-room-e2e-encryption', room._id) &&
        (room.encrypted || e2e.isReady());
    ...
    const handleSave = useMutableCallback(async () => {
        const { hideSysMes, ...data } = saveData.current;
    ...
    const handleTopicChange = useMutableCallback(e => {
        const { value } = e.currentTarget;
        handleRoomTopic(value.match(roomTopicRegex)?.[0]);
    });

    const handleAnnouncementChange = useMutableCallback(e => {
        const { value } = e.currentTarget;
        handleRoomAnnouncement(value.match(roomAnnouncementRegex)?.[0]);
    });
                            ...
                            <FieldGroup>
                                ...
                                <Field>
                                    ...
                                    >
                                        <Field.Label>
                                            {t(
                                                'RetentionPolicyRoom_OverrideGlobal'
                                            )}
                                        </Field.Label>
                                        <Field.Row>
                                            <ToggleSwitch
                    ...
                    <Field.Row>
                        ...
                        >
                            <ButtonGroup stretch flexGrow={1}>
                                ...
                                <Button
                                    flexGrow={1}
                                    disabled={!hasUnsavedChanges || !roomName}
                                    onClick={handleSave}
                                >
                                    {t('Save')}
                                </Button>
