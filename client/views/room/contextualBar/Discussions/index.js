...
const Row = memo(function Row({
	...
	return (
		<Discussion
			...
			style={style}
			following={
				discussion.replies && discussion.replies.includes(userId)
			}
...
export function DiscussionList({
	...
	const isItemLoaded = useCallback(
		index => index < discussionsRef.current.length,
		[]
	);
	const {
		ref,
		contentBoxSize: { inlineSize = 378, blockSize = 750 } = {}
		...
		<>
			...
			<VerticalBar.Content paddingInline={0}>
				...
				>
					...
					{total === 0 && (
						<Box p="x24">{t('No_Discussions_found')}</Box>
					)}
					{!error && total > 0 && (
						<InfiniteLoader
