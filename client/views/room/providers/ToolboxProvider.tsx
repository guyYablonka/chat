...
const VirtualAction = React.memo(
    ({
        ...
        const visible =
            config &&
            (!config.groups ||
                (groupsDict[room.t] && config.groups.includes(group as any)));

        useLayoutEffect(() => {
            handleChange((list: Store<ToolboxAction>) => {
                visible && config
                    ? list.get(id) !== config && list.set(id, config)
                    : list.delete(id);
            });
            return (): void => {
...
const useToolboxActions = (
    ...
    listen: (handler: Handler<any>) => Function;
    actions: Array<[string, ToolboxAction]>;
} => {
    const { listen, actions } = useContext(ToolboxContext);
    const [state, setState] = useState<Array<[string, ToolboxAction]>>(
...
export const ToolboxProvider = ({
    ...
    const [activeTabBar, setActiveTabBar] = useState<
        [ToolboxActionConfig | undefined, string?]
    >([undefined]);
    const [list, setList] = useSafely(
        useDebouncedState<Store<ToolboxAction>>(new Map(), 5)
    );
    ...
    const open = useMutableCallback((actionId, context) => {
        ...
        callbacks.run(`userClickedOn-${actionId}`);
        router.push({
            ...params,
            tab: actionId,
            context
    ...
    useLayoutEffect(() => {
        ...
        setActiveTabBar([
            list.get(tab as string) as ToolboxActionConfig,
            context
        ]);
...
export const useTabBarOpenUserInfo = (): Function =>
    useContext(ToolboxContext).openUserInfo;
