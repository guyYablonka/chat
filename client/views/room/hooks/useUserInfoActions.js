...
export const useUserHasRoomRole = (uid, rid, role) =>
	useReactiveValue(
		useCallback(
			() => !!RoomRoles.findOne({ rid, 'u._id': uid, roles: role }),
			[uid, rid, role]
		)
	);
...
const getUserIsMuted = (room, user, userCanPostReadonly) => {
	if (room && room.ro) {
		if (
			Array.isArray(room.unmuted) &&
			room.unmuted.indexOf(user && user.username) !== -1
		) {
			return false;
		}
...
export const useUserInfoActionsSpread = (actions, size) =>
	useMemo(() => {
		const actionsEntries = Object.entries(actions);
		const menuOptions = actionsEntries
			.slice(size ?? actionsEntries.length, actionsEntries.length)
			.map(mapOptions);
		if (menuOptions.length) {
			const menu = Object.fromEntries(menuOptions);
			return { actions: actionsEntries, menu };
		}

		return { actions: actionsEntries };
	}, [actions]);
...
export const useUserInfoActions = (user = {}, rid, reload) => {
	...
	const getWebRTCInstance = useCallback(
		() => WebRTC.getInstanceByRoomId(rid),
		[rid]
	);
	...
	const roomName =
		room && room.t && escapeHTML(roomTypes.getRoomName(room.t, room));
	...
	const callInProgress = useReactiveValue(
		useCallback(
			() => webRTCInstance?.callInProgress.get(),
			[webRTCInstance]
		)
	);
	...
	const videoCallOption = useMemo(() => {
		...
		return (
			shouldAllowCalls && {
				label: t(
					callInProgress ? 'Join_video_call' : 'Start_video_call'
				),
				icon: 'video',
				action
			}
	...
	const audioCallOption = useMemo(() => {
		...
		return (
			shouldAllowCalls && {
				label: t(
					callInProgress ? 'Join_audio_call' : 'Start_audio_call'
				),
				icon: 'mic',
				action
			}
	...
	const changeModeratorEndpoint = isModerator
		? 'removeModerator'
		: 'addModerator';
	...
	const changeModeratorOption = useMemo(
		() =>
			...
			userCanSetModerator && {
				label: t(
					isModerator ? 'Remove_as_moderator' : 'Set_as_moderator'
				),
				icon: 'shield',
				action: changeModeratorAction
			},
		[
			changeModeratorAction,
			isModerator,
			roomCanSetModerator,
			t,
			userCanSetModerator
	...
	const onSuccessUserKick = useCallback(() => {
		if (uid !== ownUserId) {
			RoomRoles.remove({ rid, 'u._id': uid });
