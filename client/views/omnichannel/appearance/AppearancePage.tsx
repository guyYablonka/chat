...
const reduceAppearance = (
    settings: LivechatAppearanceEndpointData['appearance']
): AppearanceSettings =>
    settings.reduce<Partial<LivechatAppearanceSettings>>(
        (acc, { _id, value }) => {
            acc = { ...acc, [_id]: value };
            return acc;
        },
...
const AppearancePageContainer: FC = () => {
    ...
    const {
        ...
        phase: state,
        error
    } = useEndpointData<LivechatAppearanceEndpointData>('livechat/appearance');
...
const AppearancePage: FC<AppearancePageProps> = ({ settings }) => {
    ...
    const handleSave = useMutableCallback(async () => {
        const mappedAppearance = Object.entries(values).map(([_id, value]) => ({
            _id,
            value
        }));
