...
type OnToggleProps = {
    onToggle: (
        ...
        setSubscribed: Dispatch<boolean>
    ) => void;
};
...
const FacebookPageContainer: FC = () => {
    ...
    const { enabled, hasToken } = initialStateData || {
        enabled: false,
        hasToken: false
    };
    ...
    const onToggle = useMutableCallback(
        async (id, isSubscribed, setSubscribed) => {
    ...
    if (
        ...
        listState === AsyncStatePhase.LOADING
    ) {
        return <PageSkeleton />;
    }
...
const PageToggle: FC<PageToggleProps> = ({
    name,
    id,
    subscribed,
    onToggle,
    ...
    const handleToggle = useMutableCallback(() =>
        onToggle(id, isSubscribed, setIsSubscribed)
    );
    return (
...
const PageToggleAssembler: FC<PageToggleAssemblerProps> = ({
    pages,
    onToggle,
    className
}) => (
                    ...
                    {enabled && (
                        <>
                            ...
                            {pages?.length ? (
                                ...
                                t(
                                    'No_pages_yet_Try_hitting_Reload_Pages_button'
                                )
                            )}
                            <Box w='full' mb='x16'>
                                <Divider />
