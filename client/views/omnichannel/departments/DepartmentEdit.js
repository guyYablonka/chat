...
const useQuery = ({ name }) =>
	useMemo(() => ({ selector: JSON.stringify({ name }) }), [name]);
...
export function EditDepartment({ data, id, title, reload }) {
	...
	const [tags, setTags] = useState(
		(department && department.chatClosingTags) || []
	);
	...
	const { values, handlers, hasUnsavedChanges } = useForm({
		...
		maxNumberSimultaneousChat:
			(department && department.maxNumberSimultaneousChat) || undefined,
		...
		requestTagBeforeClosingChat:
			(department && department.requestTagBeforeClosingChat) || false,
		offlineMessageChannelName:
			(department && department.offlineMessageChannelName) || '',
		visitorInactivityTimeoutInSeconds:
			(department && department.visitorInactivityTimeoutInSeconds) ||
			undefined,
		waitingQueueMessage:
			(department && department.waitingQueueMessage) || '',
	...
	const handleSubmit = useMutableCallback(async e => {
		...
		const payload = {
			...
			departmentsAllowedToForward:
				departmentsAllowedToForward &&
				departmentsAllowedToForward.join()
		};
		...
		const agentListPayload = {
			...
			remove: initialAgents.current.filter(
				initialAgent =>
					!agentList.some(agent => initialAgent._id === agent._id)
			)
		};
				...
				<Page.Header title={title}>
					<ButtonGroup>
						{id &&
							...
							cannedResponsesEnabled && (
								<Button
									onClick={handleOpenCannedResponses}
									title={t('Canned Responses')}
								>
									<Icon
										name="baloon-exclamation"
										size="x16"
									/>
								</Button>
							)}
						...
						<Button
							...
							form={formId}
							primary
							disabled={invalidForm && hasNewAgent}
						>
							{t('Save')}
						</Button>
					</ButtonGroup>
				...
				<Page.ScrollableContentWithShadow>
					...
					>
						...
						<Field>
							<Field.Label>
								{t(
									'Livechat_DepartmentOfflineMessageToChannel'
								)}
							</Field.Label>
							<Field.Row>
								<SelectFiltered
						...
						{DepartmentBusinessHours && (
							<Field>
								<DepartmentBusinessHours
									bhId={
										department && department.businessHourId
									}
								/>
							</Field>
						)}
