...
const AccountProfilePage = () => {
	...
	const handleLogoutOtherLocationsResponse = useCallback(error => {
	...
	const logoutOtherClients = () =>
		Meteor.logoutOtherClients(handleLogoutOtherLocationsResponse);
	...
	const canChangeUsername = allowUsernameChange;
	...
	const { handleAvatar, handlePassword, handleConfirmationPassword } =
		handlers;
	...
	const handleLogoutOtherLocations = useCallback(() => {
		setLoggingOut(true);
		try {
			logoutOtherClients();
		} catch (error) {
				...
				if (error.error === 'user-last-owner') {
					...
					return setModal(() => (
						<ConfirmOwnerChangeWarningModal
							onConfirm={() => {
								deleteOwnAccount(
									SHA256(passwordOrUsername),
									true
								);
							}}
							onCancel={closeModal}
							contentTitle={t(
								`Delete_User_Warning_${erasureType}`
							)}
	...
	return (
		<Page>
			<Page.Header title={t('Profile')}></Page.Header>
			<Page.ScrollableContentWithShadow>
				<Box maxWidth='600px' w='full' alignSelf='center'>
					...
					/>
					<ButtonGroup stretch align='center' mb='x24'>
						<Button
