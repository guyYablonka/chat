...
const TwoFactorTOTP = props => {
	...
	const handleDisableTotp = useCallback(async () => {
		...
		setModal(
			<VerifyCodeModal onVerify={onDisable} onCancel={closeModal} />
		);
	...
	const handleVerifyCode = useCallback(async () => {
		try {
			...
			setModal(
				<BackupCodesModal codes={result.codes} onClose={closeModal} />
			);
		} catch (error) {
			dispatchToastMessage({ type: 'error', message: error });
		}
	...
	const handleRegenerateCodes = useCallback(() => {
		...
		setModal(
			<VerifyCodeModal onVerify={onRegenerate} onCancel={closeModal} />
		);
