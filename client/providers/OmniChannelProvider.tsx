...
const OmnichannelManualSelectionProvider: FC<{
    value: OmnichannelContextValue;
}> = ({ value, children }) => {
    const queue = useOmnichannelInquiries();
    const showOmnichannelQueueLink =
        (useSetting('Livechat_show_queue_list_link') as boolean) &&
        value.agentAvailable;
    ...
    return (
        <OmnichannelContext.Provider value={contextValue} children={children} />
    );
};
...
const OmnichannelEnabledProvider: FC = ({ children }) => {
    ...
    const {
        ...
        phase: status,
        reload
    } = useMethodData<OmichannelRoutingConfig>(
    ...
    if (
        ...
        return (
            <OmnichannelManualSelectionProvider
                value={contextValue}
                children={children}
            />
        );
