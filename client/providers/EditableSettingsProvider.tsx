...
const EditableSettingsProvider: FunctionComponent<EditableSettingsProviderProps> =
    ({ children, query = defaultQuery }) => {
        const settingsCollectionRef = useRef<
            Mongo.Collection<IEditableSetting>
        >(null) as MutableRefObject<Mongo.Collection<IEditableSetting>>;
        ...
        useEffect(() => {
            ...
            settingsCollection.remove({
                _id: { $nin: persistedSettings.map(({ _id }) => _id) }
            });
            ...
            () =>
                createReactiveSubscriptionFactory(
                    (_id: SettingId): IEditableSetting | undefined => {
                        ...
                        return {
                            ...editableSetting,
                            disabled: !queries.every(
                                query =>
                                    settingsCollection.find(query).count() > 0
                            )
                        };
                    }
                ),
        ...
        const dispatch = useMutableCallback(
            (changes: Partial<IEditableSetting>[]): void => {
