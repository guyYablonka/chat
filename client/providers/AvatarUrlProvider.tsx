...
const AvatarUrlProvider: FC = ({ children }) => {
    ...
    const externalProviderUrl = String(
        useSetting('Accounts_AvatarExternalProviderUrl') || ''
    );
    const contextValue = useMemo(
        () => ({
            ...
            getRoomPathAvatar: ({ type, ...room }: any): string =>
                roomTypes
                    .getConfig(type || room.t)
                    .getAvatarPath({ username: room._id, ...room })
        }),
        [externalProviderUrl, cdnAvatarUrl]
    );

    return (
        <AvatarUrlContext.Provider children={children} value={contextValue} />
    );
};
