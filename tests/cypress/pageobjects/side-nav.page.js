...
class SideNav extends Page {
	...
	get channelType() {
		return browser.element(
			'.create-channel__content [name=type]~.rc-switch__button'
		);
	}

	get channelReadOnly() {
		return browser.elements('.create-channel__switches .rc-switch__button')
			.value[1];
	}
	...
	get channelHoverIcon() {
		return browser.element(
			'.rooms-list > .wrapper > ul [title="general"] .icon-eye-off'
		);
	}
	...
	get preferencesClose() {
		return browser.element(
			'.sidebar-flex__close-button[data-action="close"]'
		);
	}
	...
	openChannel(channelName) {
		cy.contains('[data-qa="sidebar-item-title"]', channelName)
			.scrollIntoView()
			.click();
	...
	searchChannel(channelName) {
		...
		cy.get(
			`[data-qa='sidebar-item'][aria-label='${channelName}']:first-child`
		).click();
	...
	createChannel(channelName, isPrivate /* isReadOnly*/) {
		...
		cy.get('.rc-modal__content [data-button="create"]').should(
			'be.enabled'
		);
