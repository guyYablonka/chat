...
describe('[Users]', function () {
	...
	describe('[/users.info]', () => {
		...
		it('should return fullName for all users regardless of permissions', done => {
			request
				...
				.expect(res => {
					...
					expect(res.body).to.have.property(
						'fullName',
						targetUser.name.split('/').pop()
					);
				});
		});

		it('should return firstName and lastName combined for user with custom fields', async () => {
			let user = await createUser(customFieldsUsername, ['user'], {
				firstName: 'עידו',
				lastName: 'אבני'
			});
			let userCredentials = await login(customFieldsUsername, password);
			const expectedName =
			...
			await request
				...
				.expect(res => {
					expect(res.body).to.have.property('success', true);
					expect(res.body).to.have.property('fullName', expectedName);
				});
		});
