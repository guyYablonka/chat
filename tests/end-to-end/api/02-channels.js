			...
			request
				...
				.expect(res => {
					...
					const lastMessage = messages.filter(
						message => message._id === channelMessage._id
					)[0];
					expect(lastMessage)
						.to.have.property('starred')
						.and.to.be.an('array');
			...
			updatePermission('manage-incoming-integrations', ['admin']).then(
				() => {
					request
						...
						.expect(res => {
							expect(res.body).to.have.property('success', true);
							const integrationCreated =
								res.body.integrations.find(
									createdIntegration =>
					...
					() => {
						request
							...
							.expect(res => {
								...
								expect(integrationCreated).to.be.equal(
									undefined
								);
								expect(res.body).to.have.property('offset');
								expect(res.body).to.have.property('total');
		...
		it('set customFields with multiple nested fields', async () => {
			...
			return request
				...
				.expect(res => {
					...
					expect(res.body).to.have.nested.property('channel.t', 'c');
					expect(res.body).to.have.nested.property(
						'channel.customFields.field2',
						'value2'
					);
					expect(res.body).to.have.nested.property(
						'channel.customFields.field3',
						'value3'
					);
					expect(res.body).to.have.nested.property(
						'channel.customFields.field4',
						'value4'
					);
					expect(res.body).to.have.not.nested.property(
						'channel.customFields.field1',
			...
			request
				...
				.expect(res => {
					...
					expect(res.body)
						...
						.that.is.an('array')
						.that.has.lengthOf(1);
					expect(res.body.moderators[0].username).to.be.equal(
						'rocket.cat'
					);
				})
				.end(done);
				...
				request
					...
					.expect(res => {
						...
						expect(res.body.error).to.be.equal(
							'Enable "Allow Anonymous Read" [error-not-allowed]'
						);
					})
					.end(done);
			});
