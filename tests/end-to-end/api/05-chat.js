				...
				request
					...
					.end((err, res) => {
						...
						request
							...
							.expect(200)
							.expect(res => {
								userCredentials['X-Auth-Token'] =
									res.body.data.authToken;
								userCredentials['X-User-Id'] =
					...
					request
						...
						.send({
							message: {
								rid: readOnlyChannel._id,
								msg:
									'Sample message overwriting readonly status'
							}
						})
						.expect('Content-Type', 'application/json')
						.expect(200)
		...
		describe('when execute successfully', () => {
			it("should return the statusCode 200 and 'receipts' property and should be equal an array", done => {
				request
					.get(
						api(
							`chat.getMessageReadReceipts?messageId=${message._id}`
						)
					)
					.set(credentials)
					.expect('Content-Type', 'application/json')
	...
	describe('[/chat.getSnippetedMessageById]', () => {
		it('should return an error when the snippeted messages is disabled', done => {
			updateSetting('Message_AllowSnippeting', false).then(() => {
				request
					.get(
						api('chat.getSnippetedMessageById?messageId=invalid-id')
					)
					.set(credentials)
					.expect('Content-Type', 'application/json')
					.expect(400)
...
describe('Threads', () => {
	after(done => {
		updateSetting('API_Upper_Count_Limit', 100)
			.then(() =>
				updatePermission('view-c-room', ['admin', 'user', 'bot'])
			)
			.then(done);
	});
	...
	describe('[/chat.getThreadsList]', () => {
		...
		before(done => {
			createRoom({
				type: 'c',
				name: `channel.test.threads.${Date.now()}`
			}).end((err, channel) => {
			...
			updateSetting('Threads_enabled', false).then(() => {
				request
					...
					.expect(res => {
						...
						expect(res.body).to.have.property(
							'error',
							'Threads Disabled [error-not-allowed]'
						);
					})
					.end(done);
			});
			...
			updatePermission('view-c-room', ['admin', 'user']).then(() => {
				request
					...
					.expect(res => {
						expect(res.body).to.have.property('success', true);
						expect(res.body)
							.to.have.property('threads')
							.and.to.be.an('array');
						expect(res.body).to.have.property('total');
						expect(res.body).to.have.property('offset');
						expect(res.body).to.have.property('count');
						expect(res.body.threads).to.have.lengthOf(1);
						expect(res.body.threads[0]._id).to.be.equal(
							threadMessage.tmid
						);
					})
					.end(done);
			...
			updatePermission('view-c-room', ['admin', 'user']).then(() => {
				request
					...
					.expect(res => {
						...
						expect(res.body.threads.update[0]._id).to.be.equal(
							threadMessage.tmid
						);
					})
					.end(done);
			...
			updatePermission('view-c-room', ['admin', 'user']).then(() => {
				request
					...
					.expect(res => {
						...
						expect(res.body.messages[0].tmid).to.be.equal(
							createdThreadMessage._id
						);
					})
					.end(done);
			});
			...
			updatePermission('view-c-room', ['admin', 'user']).then(() => {
				request
					...
					.expect(res => {
						...
						expect(res.body.messages.update[0].id).to.be.equal(
							createdThreadMessage.tmid
						);
					})
					.end(done);
			});
